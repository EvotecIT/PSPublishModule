...
public sealed class PowerShellCompilationProviderPackageBuilder
{
        ...
        string? repositoryCommit)
    {
        ...
        var repository = hasRepositoryUrl
            ...
            : string.Empty;
        return $"<?xml version=\"1.0\" encoding=\"utf-8\"?><package><metadata><id>{Xml(manifest.PackageId)}</id><version>{Xml(manifest.PackageVersion)}</version><authors>{Xml(manifest.Publisher)}</authors><description>PowerForge PowerShell compilation provider metadata.</description><license type=\"expression\">{Xml(manifest.LicenseExpression)}</license><licenseUrl>{Xml(licenseUrl)}</licenseUrl>{repository}<dependencies><group targetFramework=\"net8.0\">{dependencies}</group></dependencies></metadata></package>";
    }
