...
public sealed partial class PowerForgeStudioReleasePublishExecutionServiceTests
{
    ...
    public void BuildPendingTargets_PublishReadyItem_ReturnsGroupedTargetsFromSigningCheckpoint()
    {
        ...
        try
        {
            ...
            foreach (var artifact in signingResult.Receipts)
            {
                if (artifact.ArtifactKind == "Directory") Directory.CreateDirectory(artifact.ArtifactPath);
                else
                {
                    Directory.CreateDirectory(Path.GetDirectoryName(artifact.ArtifactPath)!);
                    File.WriteAllText(artifact.ArtifactPath, "fixture");
                }
            }
