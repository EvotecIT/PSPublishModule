...
public static partial class WebSiteBuilder
{
    ...
    private static void WriteApacheRedirects(string path, IReadOnlyList<RedirectSpec> redirects)
    {
        ...
        {
            var notFoundPath = Path.Combine(outputRoot, "404.html");
        ...
        foreach (var redirect in redirects)
        {
            ...
            var flags = new List<string>
            {
                ...
                "L",
                redirect.PreserveQuery && !hasSourceQuery ? "QSA" : "QSD"
            };
    ...
    private static void WriteIisWebConfigRedirects(string path, IReadOnlyList<RedirectSpec> redirects)
    {
        ...
        foreach (var redirect in redirects)
        {
            ...
            if (hasSourceQuery)
            {
                conditions = new XElement("conditions",
                    new XElement("add",
                        new XAttribute("input", "{QUERY_STRING}"),
                        new XAttribute("pattern", "^" + Regex.Escape(sourceQuery) + "$")));
            }
            ...
            rules.Add(
                new XElement("rule",
                    ...
                    new XElement("action",
                        ...
                        new XAttribute("appendQueryString", redirect.PreserveQuery && !hasSourceQuery ? "true" : "false"),
    ...
    private static void WriteNginxRedirects(string path, IReadOnlyList<RedirectSpec> redirects)
    {
        ...
        foreach (var redirect in redirects)
        {
            ...
            if (hasSourceQuery)
            {
                var requestMatcher = BuildNginxRequestUriMatcher(redirect.From, sourceQuery);
                if (string.IsNullOrWhiteSpace(requestMatcher))
                    continue;

                var statusWithQuery = ResolveRedirectStatus(redirect);
                lines.Add($"if ($request_uri ~ {requestMatcher}) {{ return {statusWithQuery} {destination}; }}");
                ruleCount++;
                continue;
            }
    ...
    {
        var normalized = NormalizeRedirectSourcePath(source);
        if (string.IsNullOrWhiteSpace(normalized))
            return string.Empty;
    ...
    private static string NormalizeRedirectSourcePath(string? source)
    {
            ...
            trimmed.StartsWith("//", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (Uri.TryCreate(absoluteValue, UriKind.Absolute, out var absolute))
                trimmed = absolute.AbsolutePath + absolute.Query;
        }
    ...
    private static bool TryGetRedirectSourceQuery(string? source, out string query)
    {
        query = string.Empty;
        if (string.IsNullOrWhiteSpace(source))
            return false;
