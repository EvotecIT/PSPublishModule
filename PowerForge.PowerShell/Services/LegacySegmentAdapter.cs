...
/// <summary>
/// Adapts legacy PSPublishModule DSL outputs (hashtables/dictionaries) and settings script blocks into typed configuration segments.
/// </summary>
public static class LegacySegmentAdapter
{
    /// <summary>
    /// Resolves the module name from the legacy configuration dictionary.
    /// </summary>
    /// <param name="configuration">Legacy configuration dictionary.</param>
    /// <returns>Module name.</returns>
    /// <exception cref="PSArgumentException">Thrown when ModuleName is missing.</exception>
    public static string ResolveModuleNameFromLegacyConfiguration(IDictionary configuration)
    {
        if (configuration is null) return string.Empty;

        var info = GetDictionary(configuration, "Information");
        var moduleName = info is null ? null : GetString(info, "ModuleName");
    ...
    /// <summary>
    /// Collects typed configuration segments from a legacy DSL settings script block.
    /// </summary>
    /// <param name="settings">Settings script block that returns typed segments and/or dictionaries.</param>
    /// <returns>Typed configuration segments.</returns>
    ...
    /// <summary>
    /// Collects typed configuration segments from the legacy configuration dictionary.
    /// </summary>
    /// <param name="configuration">Legacy configuration dictionary.</param>
    /// <returns>Typed configuration segments.</returns>
        ...
        if (manifest is not null)
        {
            ...
            segments.Add(new ConfigurationManifestSegment
            {
                Configuration = new ManifestConfiguration
                {
                    ...
                    ProcessorArchitecture = GetString(manifest, "ProcessorArchitecture"),
                    Tags = tags,
                    IconUri = iconUri,
        ...
        if (string.Equals(type, "Manifest", StringComparison.OrdinalIgnoreCase))
        {
            ...
            output.Add(new ConfigurationManifestSegment
            {
                Configuration = new ManifestConfiguration
                {
                    ...
                    ProcessorArchitecture = GetString(conf, "ProcessorArchitecture"),
