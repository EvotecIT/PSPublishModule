namespace PowerForge.Generated.Runtime
{
    ...
    public static class PowerShellNativeFunctionHost
    {
            ...
            Action<PowerShellNativeFunctionContext>? clean = null)
        {
            ...
            var metadata = Parse(parameterDeclaration);
            ...
            var source = metadata.ParamBlock.Extent.Text;
            ...
            if (end != null || begin == null && process == null)
                source += "\nend { throw 'Compiled end callback was not installed.' }";
            if (clean != null) source += "\nclean { throw 'Compiled clean callback was not installed.' }";
            var script = Parse(source).GetScriptBlock();
        ...
        private static ScriptBlockAst Parse(string source)
        {
        ...
        private static void Run(Action<PowerShellNativeFunctionContext> callback, object nativeContext)
        {
            using (var context = new PowerShellNativeFunctionContext(nativeContext)) callback(context);
        }
        ...
        internal sealed class NativeContract
        {
            ...
            internal readonly FieldInfo ExecutionContext, OutputPipe;
