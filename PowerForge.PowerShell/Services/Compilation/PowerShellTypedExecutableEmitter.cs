...
internal static class PowerShellTypedExecutableEmitter
{
    ...
    private const string ProviderProcessIsolationTemplate = "PowerForge.PowerShell.Compilation.ProviderProcessIsolation.cs.template";
    ...
    {
        var commandProviderArray = commandProviders?.ToArray();
        var compilation = PowerShellTypedExecutableCompiler.Compile(
            ...
            targetFramework,
            semanticProfileId,
            commandProviderArray);
        var contract = compilation.EntryPoint;
        var method = compilation.EntryPointMethod;
        var reachableCommandProviders = compilation.ReachableCommandProviders;
        ...
        var compiledSource = new StringBuilder()
            ...
            .AppendLine("}")
            .AppendLine(GenerateProviderProcessIsolationSource(reachableCommandProviders))
            .ToString();
        ...
        var programSource = ReadTemplate(ProgramTemplate)
            ...
            .Replace("{{PROVIDER_WORKER_INITIALIZATION}}", GenerateProviderWorkerInitialization(reachableCommandProviders))
    ...
    private static string GenerateProviderWorkerInitialization(
        IEnumerable<PowerShellCompilationCommandProviderContract>? providers)
        => providers?.Any(static provider =>
               provider.Adapter.Cancellation == PowerShellCompilationProviderCancellation.ProcessIsolated) == true
            ? "            var providerWorkerExitCode = PowerForgeProviderProcessIsolation.TryRunWorker(args);" + Environment.NewLine +
              "            if (providerWorkerExitCode.HasValue) return providerWorkerExitCode.Value;"
            : string.Empty;

    private static string GenerateProviderProcessIsolationSource(
        IEnumerable<PowerShellCompilationCommandProviderContract> providers)
    {
        var isolated = providers
            .Where(static provider => provider.Adapter.Cancellation == PowerShellCompilationProviderCancellation.ProcessIsolated)
            .OrderBy(static provider => provider.ProviderId, StringComparer.Ordinal)
            .ToArray();
        if (isolated.Length == 0) return string.Empty;
        var cases = string.Join(Environment.NewLine, isolated.Select(provider =>
            "            " + PowerShellCSharpLiteral.QuoteString(provider.ProviderId) + " => global::" +
            EscapeQualifiedProviderIdentifier(provider.Adapter.EntryPoint!.TypeName) + "." +
            EscapeProviderIdentifier(provider.Adapter.EntryPoint.MethodName) +
            "(request, global::System.Threading.CancellationToken.None),"));
        return ReadTemplate(ProviderProcessIsolationTemplate).Replace("{{PROVIDER_WORKER_CASES}}", cases);
    }
