...
internal sealed partial class PowerShellTypedLowerer
{
    ...
    private static readonly string[] ModuleStateReadHostParameterNames = { "__readPowerShellModuleVariable" };
    private static readonly string[] ModuleStateWriteHostParameterNames = { "__writePowerShellModuleVariable" };
        ...
        bool requiresPowerShellModuleStateWrite)
    {
        ...
        if (requiresPowerShellModuleStateRead) generated.UnionWith(ModuleStateReadHostParameterNames);
        if (requiresPowerShellModuleStateWrite) generated.UnionWith(ModuleStateWriteHostParameterNames);
    ...
    private static bool ContainsBoundParameterPresence(PowerShellBoundBlock block)
        => PowerShellSemanticAnalyzer.EnumerateStatements(block)
            ...
            .Any(static expression => expression is PowerShellBoundParameterPresenceExpression);
    ...
    private static bool ContainsPowerShellModuleStateRead(PowerShellBoundBlock block)
        => PowerShellSemanticAnalyzer.EnumerateStatements(block)
            ...
            .SelectMany(PowerShellSemanticAnalyzer.EnumerateExpressions)
            .OfType<PowerShellBoundRuntimeStateExpression>()
            .Any(static expression => expression.Kind == PowerShellRuntimeStateIntrinsicKind.ModuleVariable);

    private static bool ContainsPowerShellModuleStateWrite(PowerShellBoundBlock block)
        => PowerShellSemanticAnalyzer.EnumerateStatements(block)
            .Any(static statement => statement is PowerShellBoundModuleVariableAssignmentStatement);
}
