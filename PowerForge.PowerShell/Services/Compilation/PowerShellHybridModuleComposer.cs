...
internal static class PowerShellHybridModuleComposer
{
        ...
        bool manifestControlsExports = false)
    {
        ...
        var moduleStateVariables = typed.Methods
        ...
        var requiresModuleState = moduleStateVariables.Length > 0;
        if (requiresDispatcher || requiresModuleState)
        {
            ...
            AppendRuntimeRegionGuardStart(import, typed, runtimeVariables, requiresDispatcher, requiresModuleState);
        }
        ...
        if (moduleStateVariables.Length > 0)
        {
            var hostType = "[" + typed.NamespaceName + "." + PowerShellBinaryCmdletSourceGenerator.GetRuntimeRegionHostTypeName(typed) + "]";
            foreach (var variable in moduleStateVariables)
            {
        ...
        AppendRuntimeRegionGuardEnd(builder, typed, runtimeVariables, requiresDispatcher, requiresModuleState);
        ...
        bool requiresModuleState)
    {
        ...
        AppendRuntimeRegionClear(builder, hostType, variables, requiresDispatcher, requiresModuleState, "    ");
