...
internal static class PowerShellHybridModuleComposer
{
        ...
        IEnumerable<string>? retainedSourcePaths = null)
    {
        ...
        import.Append("Microsoft.PowerShell.Core\\Import-Module -Name (Microsoft.PowerShell.Management\\Join-Path -Path $PSScriptRoot -ChildPath '" + EscapePowerShellSingleQuotedString(assemblyFileName) + "') -Force -ErrorAction Stop");
        if (requiresRuntimeState)
            import.Append(" -PassThru | Microsoft.PowerShell.Core\\ForEach-Object { [").Append(typed.NamespaceName).Append('.')
