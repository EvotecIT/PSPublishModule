...
internal static partial class PowerShellStrictDependencyClosureVerifier
{
    ...
    internal static PowerShellCompilationDependencyClosure Verify(PowerShellStrictDependencyClosureRequest request)
    {
        ...
        foreach (var file in request.Files.OrderBy(static file => file.Path, StringComparer.OrdinalIgnoreCase))
        {
            ...
            if (extension.Equals(".dll", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (pe.HasMetadata)
                {
                    VerifyManagedAssembly(
                        pe,
                        file.Path,
                        managedAssemblies,
                        ComputeSha256(file.Path),
                        request.AllowCompilerOwnedProcessIsolation &&
                        (file.Role.Equals("Primary", StringComparison.Ordinal) ||
                         request.Files.Any(primary =>
                             primary.Role.Equals("Primary", StringComparison.Ordinal) &&
                             Path.GetFileNameWithoutExtension(primary.Path).Equals(
        ...
        ICollection<NativeLibraryInspection> nativeLibraries)
    {
        ...
        try
        {
            ...
            if (pe.HasMetadata)
            {
                VerifyManagedAssembly(
                    ...
                    managedAssemblies,
                    ComputeSha256(path),
                    request.AllowCompilerOwnedProcessIsolation && request.Files.Any(file =>
                        file.Role.Equals("Primary", StringComparison.Ordinal) &&
                        PowerShellCompilationPathSafety.PathEquals(file.Path, path)));
                result.ManagedAssemblies++;
                return true;
            }
        }
        ...
        foreach (var entry in manifest.Entries)
            VerifyBundleEntry(
                ...
                nativeLibraries,
                request.AllowCompilerOwnedProcessIsolation &&
                Path.GetFileNameWithoutExtension(entry.RelativePath).Equals(
        ...
        bool allowProcessLaunch)
    {
        ...
        switch (entry.Type)
        {
            case DotNetBundleFileType.Assembly:
                VerifyManagedAssembly(bytes, bundlePath + "!" + entry.RelativePath, managedAssemblies, allowProcessLaunch);
                result.ManagedAssemblies++;
                break;
    ...
    private static void VerifyManagedAssembly(
        ...
        string displayPath,
        ICollection<ManagedAssemblyInspection> managedAssemblies,
        bool allowProcessLaunch = false)
    {
        using var stream = new MemoryStream(bytes, writable: false);
        using var pe = new PEReader(stream);
        if (!pe.HasMetadata)
            throw new InvalidDataException($"Expected managed assembly '{displayPath}' does not contain CLR metadata.");
        VerifyManagedAssembly(pe, displayPath, managedAssemblies, ComputeSha256(bytes), allowProcessLaunch);
    }
