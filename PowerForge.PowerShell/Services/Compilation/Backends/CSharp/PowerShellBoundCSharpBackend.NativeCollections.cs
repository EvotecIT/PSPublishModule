...
internal sealed partial class PowerShellBoundCSharpBackend
{
    private string EmitNativeCollection(PowerShellLoweredNativeCollectionExpression collection)
    {
        ...
        {
            body.Append("try { ").Append(PowerShellCSharpSymbolRenderer.TypeName(item.Value.ClrType)).Append(' ')
            ...
            if (item.Enumerate)
                body.Append("foreach (object? ").Append(item.RecordTemporary).Append(" in ").Append(item.ValueTemporary)
                    .Append(") ").Append(result).Append(".Add(").Append(item.RecordTemporary).Append("); ");
            else
                body.Append(result).Append(".Add(").Append(item.ValueTemporary).Append("); ");
