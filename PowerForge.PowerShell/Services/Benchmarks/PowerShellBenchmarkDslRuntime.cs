...
public static class PowerShellBenchmarkDslRuntime
{
    ...
    private static Dictionary<string, string> CreateFunctionBodies()
    {
        ...
        const string InputBody = "param([Parameter(Position=0, Mandatory=$true)] [string] $Name, [Parameter(Position=1)] [string] $Default, [switch] $Required) $value = $BenchmarkVariables[$Name]; if ([string]::IsNullOrWhiteSpace([string] $value)) { if ($Required.IsPresent) { throw \"Benchmark variable '$Name' is required.\" }; return $Default }; [string] $value";
...
""",
            ...
            ["Get-BenchmarkIntInput"] = InputIntBody,
            ["Get-BenchmarkBoolInput"] = InputBoolBody,
