...
public sealed partial class ModulePipelineRunner
{
    internal void SyncSourceProjectVersionIfRequested(ModulePipelinePlan plan)
    {
        ...
        var updated = CsprojVersionEditor.UpdateVersionText(content, synchronizedVersion, out var hadVersionTag);
        if (string.Equals(content, updated, StringComparison.Ordinal))
        {
            _logger.Info($"Build: source project version already matches resolved module version '{synchronizedVersion}' ({fullPath}).");
            return;
        }

        File.WriteAllText(fullPath, updated);
        var action = hadVersionTag ? "synchronized" : "inserted VersionPrefix for";
        _logger.Info($"Build: {action} source project version '{synchronizedVersion}' in '{fullPath}'.");
    }
}
