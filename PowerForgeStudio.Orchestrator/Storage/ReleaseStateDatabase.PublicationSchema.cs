...
public sealed partial class ReleaseStateDatabase
{
    ...
    private async Task MigratePublicationReceiptsAsync(CancellationToken cancellationToken)
    {
        ...
        await database.RunInTransactionAsync(async (transaction, token) =>
        {
            ...
            if (!columns.Any(column => column.Name == "github_assets_json"))
                await transaction.ExecuteNonQueryAsync("ALTER TABLE release_publish_receipt ADD COLUMN github_assets_json TEXT NULL;", cancellationToken: token).ConfigureAwait(false);
