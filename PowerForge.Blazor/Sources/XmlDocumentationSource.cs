...
/// <summary>
/// Documentation source that loads from C# XML documentation files.
/// </summary>
public class XmlDocumentationSource : IDocumentationSource
{
    ...
    public XmlDocumentationSource(string xmlPath, XmlDocSourceOptions? options = null)
    {
    ...
    private async Task LoadApiDocAsync(CancellationToken cancellationToken)
    {
        ...
        await using var stream = File.OpenRead(_xmlPath);
        ...
        foreach (var member in members.Elements("member"))
        {
            ...
            switch (prefix)
            {
                case 'T': // Type
                    ...
                    if (!_options.IncludeInternals && type.Modifiers.Contains("internal")) continue;
    ...
    private static ApiTypeKind InferTypeKind(string name)
    {
        ...
        return ApiTypeKind.Class; // Default, could be enhanced with reflection
    }
    ...
    {
        // Get inner text, preserving inline elements like <see>, <c>, etc.
        ...
        foreach (var ns in _apiDoc.Namespaces)
        {
            ...
            foreach (var type in ns.Types)
            {
                nsItem.Children.Add(new DocNavItem
                {
                    ...
                    Badge = type.Kind == ApiTypeKind.Interface ? "I" : null,
                    BadgeClass = type.Kind == ApiTypeKind.Interface ? "badge-interface" : null
                });
            }
...
/// <summary>
/// Options for XmlDocumentationSource.
/// </summary>
public class XmlDocSourceOptions
{
    /// <summary>
    ...
    /// <summary>
    /// Display name. Defaults to "API Reference".
    /// </summary>
    ...
    /// <summary>
    /// Sort order. Defaults to 100.
    /// </summary>
    public int Order { get; set; } = 100;

    /// <summary>
    /// Whether to include internal types/members. Defaults to false.
    /// </summary>
