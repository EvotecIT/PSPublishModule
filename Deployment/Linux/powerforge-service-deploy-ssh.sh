...
head -c "$((max_payload_bytes + 1))" >"$payload_path"
...
mapfile -t archive_entries < <(tar -tf "$payload_path")
(( ${#archive_entries[@]} == 2 )) || fail 'Deployment payload must contain exactly two files.'
sorted_entries="$(printf '%s\n' "${archive_entries[@]}" | LC_ALL=C sort)"
[[ "$sorted_entries" == $'artifact.tar\ndeployment.json' ]] ||
  fail 'Deployment payload contains an unexpected path.'
