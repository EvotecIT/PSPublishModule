Describe 'Get-PowerShellAssemblyMetadata' {
    It 'loads cmdlets when runtime contains a different assembly version' {
        ...
        $runtimeAssemblyPath = Join-Path $projRuntime 'bin/Release/net8.0/DummyDependency.dll'
        $runtimeAssemblies = Get-ChildItem -Path $runtimeDir -Filter '*.dll'
        Mock Get-ChildItem -ParameterFilter { $Path -eq $runtimeDir -and $Filter -eq '*.dll' } -MockWith {
            $runtimeAssemblies + (Get-Item $runtimeAssemblyPath)
        }
