...
/// <para type="description">Stores Personal Access Tokens for GitHub and/or Azure DevOps under the current user profile so that PowerGuardian can access private repositories when rendering documentation from a repository. On Windows, tokens are protected with DPAPI; on other platforms they are stored as Base64 (best effort).</para>
/// <example>
...
public sealed class SetModuleDocumentationCommand : PSCmdlet
{
    ...
    /// <summary>
    /// Saves or clears stored tokens used by PowerGuardian to access private repositories when fetching documentation.
    /// </summary>
    protected override void ProcessRecord()
    {
        if (Clear)
        {
            if (ShouldProcess("PowerGuardian token store", "Clear"))
            {
                TokenStore.Clear();
        ...
        if (ShouldProcess("PowerGuardian token store", "Save"))
        {
