...
`CompleteWorkflow_PinnedSavedStateInFullModule` compares the same 18 observations within that full source context on PowerShell 5.1, 7.4.19, and 7.6.5. Both saved-state functions compile while the original loader discovers the surrounding functions. This extends the closure proof to module initialization and source discovery; dependency integration, cancellation, removal/reimport, and multiple-region lifecycle remain separate qualification work.
