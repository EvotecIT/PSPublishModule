...
- `SecurityGate` promotes Authenticode install/save compatibility into a named suite scenario. The first PowerShell 7 and Windows PowerShell 5.1 proof for `ThreadJob` 2.1.0 with `-AuthenticodeCheck` passed with strict managed-rank gates. PowerShell 7 measured managed install/save at 1364.09/996.65 ms versus PSResourceGet at 3041.44/2475.88 ms. Windows PowerShell 5.1 measured managed install/save at 1399.85/1285.63 ms versus PSResourceGet at 4190.17/2662.24 ms. Each managed suite row now reports `ManagedAuthenticodeCheckedFiles=4`, covering the signable `.psd1`, `.psm1`, and dependency `.dll`/manifest files before promotion. This is initial file-signature evidence, not full catalog/timestamp/short-lived-certificate parity.
