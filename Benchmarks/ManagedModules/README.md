...
- Provider-matrix scenarios now use `ModuleFastSource = ProviderDefault`, which makes the harness omit ModuleFast `-Source` instead of forcing the same-source URL. A PowerShell 7 `Az.Accounts.ProviderMatrix` smoke succeeded for all four engines: managed 1272.75 ms, ModuleFast 2094.90 ms, PSResourceGet 2339.58 ms, and PowerShellGet 7799.18 ms. This proves the provider-default harness path; use `Az.Full.ProviderMatrix` for the heavyweight full Az follow-up.
