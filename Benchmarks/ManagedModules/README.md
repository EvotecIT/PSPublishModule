...
Suite runs write `suite-summary.csv`, `suite-summary.json`, `suite-host-comparison.csv`, `suite-host-comparison.json`, `suite-hosts.csv`, optional `suite-gate.csv`, and per-scenario child run folders. Suite summaries carry the managed detail medians from each child comparison row so heavy Graph/Az/Teams/Exchange runs can be inspected from one file before drilling into package detail artifacts. `suite-host-comparison.csv` joins matching managed rows from PowerShell 7 and Windows PowerShell 5.1 so host deltas are visible without manually combining CSV files. Prefer the unique package columns for optimization planning; use the tree package columns when investigating repeated dependency traversal.
