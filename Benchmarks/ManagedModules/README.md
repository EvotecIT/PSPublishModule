...
- `HeavySaveCacheGate` suite aggregation now preserves first/last managed phase timings into `suite-summary.csv`, `suite-optimization-targets.csv`, and `suite-notes.md`. A full Graph/Az rerun on PowerShell 7 and Windows PowerShell 5.1 showed every warm second-iteration row using zero package requests, zero downloaded bytes, full package-cache hits, and full extraction-cache hits. The suite-level `LastBottleneck` was `RootDependency` for all four rows, so the next optimization pass should investigate dependency fan-out/materialization after cache hits before changing archive copy primitives again.
