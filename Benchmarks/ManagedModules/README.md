...
Use `-ManagedMaxWindowsPowerShellVsPowerShell7` when a suite should fail if Windows PowerShell 5.1 drifts too far behind the matching PowerShell 7 managed row. The host gate is intentionally separate from provider ranking: provider scoreboards decide whether managed beats equivalent engines, while the host gate decides whether the same managed path remains acceptably close across hosts.
