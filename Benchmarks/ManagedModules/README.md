...
Run the public comparison gate before making README benchmark claims. This is
the curated table lane: ThreadJob, Microsoft.Graph.Authentication, full
Microsoft.Graph, Az.Accounts, and full Az across find, install, save, and
update operations where the operation has an equivalent provider command.
Repair planning is intentionally not part of the public race because the
comparison tools do not expose an equivalent estate-repair planner.

```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\Benchmarks\ManagedModules\Invoke-ManagedModuleBenchmarkSuite.ps1 -Suite PublicComparisonGate -HostName PowerShell7,WindowsPowerShell -RepeatCount 1 -RotateEngineOrder -CacheMode Warm -UseScenarioGates -RemoveOutputRoots -SkipBuild
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\Benchmarks\ManagedModules\Export-ManagedModuleBenchmarkReadmeTable.ps1 -PublicComparison -LatestPerScenario -SplitByOperation -ReadmePath .\README.MD
```
...
Run the lifecycle evidence lane across PowerShell 7 and Windows PowerShell 5.1. This lane covers no-op and force semantics for install and save, keeps ModuleFast in the install comparison, and uses compact suite paths so Windows PowerShell 5.1 does not trip over legacy path-length limits. With `-UseScenarioGates`, the named lifecycle rows carry both provider-rank gates and a conservative Windows PowerShell 5.1 versus PowerShell 7 managed drift gate:
...
Run the exact-version Graph and Az install lifecycle gates when optimizing force/no-op install behavior beyond small modules. These scenarios compare Managed with ModuleFast and PSResourceGet under a strict managed-rank gate and a scenario-owned PowerShell host drift gate. PowerShellGet remains available in provider-matrix compatibility runs, but it is not included in this speed gate because its install path can dominate runtime without being the fastest competitor. Setup retries apply only to the preseed step outside the measured operation:

```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\Benchmarks\ManagedModules\Invoke-ManagedModuleBenchmarkSuite.ps1 -Suite LifecycleGate -ScenarioName Graph.Authentication.InstallExact.NoOpForce,Az.Accounts.InstallExact.NoOpForce -HostName PowerShell7,WindowsPowerShell -RepeatCount 3 -RotateEngineOrder -CacheMode Warm -UseScenarioGates -RemoveOutputRoots -SkipBuild
```
...
Use `-ManagedMaxWindowsPowerShellVsPowerShell7` when a suite should fail if Windows PowerShell 5.1 drifts too far behind the matching PowerShell 7 managed row with a custom threshold. Named gate scenarios can also carry their own host drift threshold; `-UseScenarioGates` applies those defaults without requiring the command line to repeat them. The host gate is intentionally separate from provider ranking: provider scoreboards decide whether managed beats equivalent engines, while the host gate decides whether the same managed path remains acceptably close across hosts.

### Security And Signature Checks

Run the Authenticode compatibility gate when validating certificate/signature behavior for install and save. The current lane compares managed delivery with PSResourceGet because both expose an equivalent signature-check switch for these operations. Its scenario-owned gate requires managed to rank first and to report at least one checked Authenticode file, so a fast run that silently stops inspecting signable files fails. Catalog files are reported separately as `ManagedAuthenticodeCatalogFiles`; use `-ManagedMinAuthenticodeCatalogFiles` only with a catalog-specific fixture so catalog evidence is not confused with ordinary file-signature checks:
...
Start with `managed-module-comparison.csv` for a quick child-run scoreboard, then inspect `managed-module-summary.csv` for every participating engine median and `managed-module-results.csv` when a competitor is skipped or failed.
...
Suite runs write `suite-notes.md`, `suite-summary.csv`, `suite-summary.json`, `suite-engine-summary.csv`, `suite-engine-summary.json`, `suite-scoreboard.csv`, `suite-scoreboard.json`, `suite-host-comparison.csv`, `suite-host-comparison.json`, `suite-optimization-targets.csv`, `suite-optimization-targets.json`, `suite-hosts.csv`, optional `suite-gate.csv`, optional `suite-host-gate.csv`, and per-scenario child run folders. Start with `suite-notes.md` when a suite mixes scoreboards and diagnostics: it groups selected scenarios by `BenchmarkRole`, repeats the plain-language interpretation, summarizes the managed result rows, includes a wide provider scoreboard, includes engine median rows, includes host comparisons, and includes the top optimization target questions without requiring a spreadsheet. `suite-scoreboard.csv` is the README-ready provider table: each scenario/host/operation has one row with `Managed`, `ModuleFast`, `PSResourceGet`, and `PowerShellGet` columns formatted as median milliseconds plus `x` ratio against the fastest successful engine. Skipped and failed engine cells stay visible, which keeps ModuleFast install evidence in the right lanes and prevents ModuleFast save skips from reading like managed save losses. `suite-engine-summary.csv` is the normalized suite-level provider view: it preserves each child run's operation/engine median, first/last timing, success/failure/skip counts, output-size medians, and output throughput columns (`MedianOutputMB`, `MedianOutputMBPerSecond`, and `MedianOutputFilesPerSecond`) so save/install scoreboards can still be pivoted without opening each child folder. `suite-summary.csv` stays managed-focused and carries the managed detail medians from each child comparison row so heavy Graph/Az/Teams/Exchange runs can be inspected from one file before drilling into package detail artifacts. `ManagedAuthenticodeCheckedFiles` and `ManagedAuthenticodeCatalogFiles` are also promoted into direct comparison and suite summary rows so signature-check gates show whether managed actually inspected signable files and whether catalog-file evidence was present. `GateManagedMinAuthenticodeCheckedFiles` and `GateManagedMinAuthenticodeCatalogFiles` record active minimums when a security scenario requires that proof. `BenchmarkRole` separates real provider scoreboards from managed-only diagnostic rows, and `ComparisonScope` names the comparable engine set, such as `InstallSameSource`, `SaveCapableProviders`, or `ManagedOnlySaveCache`. `BenchmarkInterpretation` carries the plain-language reading for the row into scenario lists, suite notes, suite summaries, engine summaries, scoreboards, host comparisons, optimization targets, metadata, and gate failures so managed-only cache/materialization diagnostics are not mistaken for provider races. Suite summary gate columns show the effective gate for the row: explicit command-line thresholds first, scenario-owned thresholds when `-UseScenarioGates` is supplied, or zero when no gate was active. `suite-host-comparison.csv` joins matching managed rows from PowerShell 7 and Windows PowerShell 5.1 so host deltas are visible without manually combining CSV files. Its median columns feed `suite-host-gate.csv`, while `BaselineFirstMs`/`ComparisonFirstMs` and `BaselineLastMs`/`ComparisonLastMs` show cold-start and warm-cache behavior separately. `suite-host-gate.csv` is written when `-ManagedMaxWindowsPowerShellVsPowerShell7` is supplied or when `-UseScenarioGates` activates a scenario-owned host drift threshold, and records missing host rows or ratios above the active limit. `suite-optimization-targets.csv` ranks each managed row by the largest measured phase and preserves request, redirect, cache, package, downloaded-byte, output-size, and output-throughput context so the next optimization pass starts from evidence instead of treating every slow-looking save row the same. It also reports `LastBottleneck`, `LastBottleneckMs`, `LastDependencyMs`, `LastSlowestDependencyPackage`, `LastCoalescedWaitMs`, `LastInstallLockWaitMs`, `LastExtractionCacheLockWaitMs`, `LastPromotionMoveMs`, `LastSlowestMaterializedPackage`, and `LastNextQuestion`, which are the better starting point for warm-cache diagnostics where the first iteration intentionally includes cold downloads. `LastWarmOptimizationLane`, `LastWarmOptimizationLaneMs`, `LastMaterializationMs`, and `LastMaterializationDominantPhase` split broad warm-cache `RootDependency` rows into actionable dependency-versus-materialization work so the next optimization pass can pick the correct engine lane before changing concurrency or filesystem primitives. A warm row with elapsed package-delivery time but zero package requests, zero downloaded bytes, and package-cache hits is classified as `PackageCache` rather than `Download`, so cache read/copy/hash work is not mistaken for network throughput. Raw managed detail package rows include `CoalescedWaitMilliseconds` from the managed install result when available, so dependency-wait edges can be sorted directly without treating ordinary already-installed no-op rows as scheduler pressure. They also include `InstallLockWaitMilliseconds`, which measures waiting for the per-module filesystem lock separately from coalescing; `ExtractionCacheLockWaitMilliseconds`, which measures waiting for the expanded-package cache lock separately from extraction or cache-copy work; and `PromotionLockWaitMilliseconds`/`PromotionMoveMilliseconds`, which split final promotion wait from the filesystem move into the target module root. Use those fields to distinguish duplicate dependency scheduling, final disk-safety serialization, extracted-cache serialization, and promotion move cost before changing concurrency or materialization behavior. Dependency branch fields such as `TotalDependencyMilliseconds`, `SlowestDependencyPackageName`, and `SlowestDependencyPackageParent` split the broad root dependency phase into the package branch that kept the graph open longest. `BottleneckShareRaw` keeps the mathematical phase-to-wall ratio; `BottleneckShare` is shown as `>100%` with a timing note when summed package/dependency phase timings overlap because parallel work exceeds wall-clock time. Prefer the unique package columns for optimization planning; use the tree package columns when investigating repeated dependency traversal.

Use `Export-ManagedModuleBenchmarkReadmeTable.ps1` to turn one or more retained `suite-scoreboard.csv` artifacts into the normalized public README table. The exporter recalculates every comparable provider against Managed as the `1.00x` baseline, preserves failed and skipped cells, and renders known capability gaps such as ModuleFast save/find/update rows as `Not equivalent`. Pass explicit scoreboards for release-candidate evidence, or use `-BenchmarkRoot` plus `-LatestPerScenario` for local exploratory refreshes. Add `-PublicComparison -SplitByOperation` when updating the public README so only the curated find, install, save, and update rows land in the marker block. Keep repair, security, publish, and warm-cache diagnostics in suite notes unless the README explicitly needs that separate proof. The optional `-ReadmePath` mode updates the marker block in the root README.

Use `Measure-ManagedModuleMaterializationStrategies.ps1` when a warm-cache save row points at a slow materialized package and you need to distinguish an engine primitive issue from full-family concurrency. The probe is external to the module: it imports the locally built cmdlets, seeds the managed package cache for one exact package, then times the same `Save-ManagedModule -SkipDependencyCheck` package twice. `ExtractedPayload` keeps the expanded `.x` package cache, while `PackageArchive` deletes only that package's expanded-cache folder before the timed run so the engine reuses the cached `.nupkg` and repopulates the expanded cache. This compares the real managed paths without adding benchmark-only switches to shipped cmdlets.

```powershell
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\Benchmarks\ManagedModules\Measure-ManagedModuleMaterializationStrategies.ps1 -ModuleName Az.Cdn -Version 6.0.2 -AcceptLicense -RepeatCount 3 -SkipBuild
```
...
Performance gates are optional. `-ManagedMaxRank` fails when a successful managed row ranks worse than the requested rank; `-ManagedMaxVsFastest` fails when managed is slower than the fastest successful engine by more than the requested ratio; `-ManagedMinAuthenticodeCheckedFiles` fails when managed reports fewer checked signable files than required; `-ManagedMinAuthenticodeCatalogFiles` fails when managed reports fewer checked catalog files than required; `-ManagedMaxWindowsPowerShellVsPowerShell7` fails when a matching Windows PowerShell 5.1 managed row is missing or exceeds the requested ratio over the PowerShell 7 baseline. Named gate scenarios can also carry their own provider and host thresholds; use `-UseScenarioGates` to apply those scenario-owned defaults. Explicit command-line gate parameters override scenario defaults for experiments. Gate failures are reported after CSV/JSON artifacts are written, so failed runs still leave evidence for diagnosis. Skipped competitor rows do not count as faster engines.
...
- `SaveGate` now selects ModuleFast and PowerShellGet alongside managed and PSResourceGet so the wide scoreboards always show the full provider columns. A PowerShell 7 smoke after that change measured managed first at 1152.37 ms, PSResourceGet at 1501.48 ms, PowerShellGet at 3922.09 ms, and ModuleFast as an explicit `Skipped` cell because it has no save command.
...
- After preserving package dependency order and seeding the first direct dependency before broad fan-out, repeated full-family warm-cache save diagnostics were rerun on PowerShell 7 and Windows PowerShell 5.1. Full `Microsoft.Graph` 2.38.0 measured warm medians of 1335.40 ms on PowerShell 7 and 2120.16 ms on Windows PowerShell 5.1, with zero warm repository/package requests, zero downloaded bytes, 40 package-cache hits, and 40 extraction-cache hits. Full `Az` 16.0.0 measured warm medians of 2192.08 ms on PowerShell 7 and 4622.14 ms on Windows PowerShell 5.1, with zero warm package requests/downloaded bytes, 103 package-cache hits, and 103 extraction-cache hits. This is retained managed-engine optimization evidence for cache-correct warm save, not a provider scoreboard; the save-provider race remains `HeavySaveGate`, and ModuleFast remains visible in install-capable lanes only.
- Optimization-target classification now separates warm package-cache delivery from network download. Reprocessing the retained full Graph/Az warm-cache summaries with the updated classifier leaves Graph PowerShell 7 on `Materialization`, moves Graph Windows PowerShell 5.1 to `PackageCache` with zero package requests and zero downloaded MB, keeps Az PowerShell 7 on `Materialization`, and keeps Az Windows PowerShell 5.1 on `Dependency`.
- Isolated materialization-strategy probes on 2026-06-29 showed the extracted payload cache is still the right steady-state primitive. `Az.Cdn` 6.0.2 measured extracted/archive medians of 97.67/136.26 ms on PowerShell 7 and 199.17/734.04 ms on Windows PowerShell 5.1. `Microsoft.Graph.Files` 2.38.0 measured 149.89/351.79 ms on PowerShell 7 and 164.29/731.71 ms on Windows PowerShell 5.1. Read this beside the full-family Az warm-cache row: the slow `Az.Cdn` package inside the broad save is a contention/coordination symptom, not evidence that archive extraction should replace expanded-cache materialization.
- `SecurityGate` promotes Authenticode install/save compatibility into a named suite scenario. The PowerShell 7 and Windows PowerShell 5.1 proof for `ThreadJob` 2.1.0 with `-AuthenticodeCheck` now passes strict managed-rank gates plus `GateManagedMinAuthenticodeCheckedFiles=1`. PowerShell 7 measured managed install/save at 1386.63/815.64 ms versus PSResourceGet at 3208.73/3065.33 ms. Windows PowerShell 5.1 measured managed install/save at 1417.27/799.97 ms versus PSResourceGet at 3228.07/2468.69 ms. Each managed suite row reported `ManagedAuthenticodeCheckedFiles=4`, covering the signable `.psd1`, `.psm1`, and dependency `.dll`/manifest files before promotion. Catalog-file evidence now has its own `ManagedAuthenticodeCatalogFiles` column and minimum gate. This is initial file-signature and catalog-evidence plumbing, not full catalog-member, timestamp, or short-lived-certificate parity.
- A follow-up PowerShell 7 `SecurityGate` smoke after the catalog-evidence plumbing measured managed install/save at 1359.76/683.8 ms and reported `ManagedAuthenticodeCheckedFiles=4` with `ManagedAuthenticodeCatalogFiles=0` for both rows. That zero is expected for `ThreadJob`; catalog-specific fixtures should use `-ManagedMinAuthenticodeCatalogFiles` when they are added.
...
- `HeavySaveCacheGate` suite aggregation now preserves first/last managed phase timings into `suite-summary.csv`, `suite-optimization-targets.csv`, and `suite-notes.md`. A full Graph/Az rerun on PowerShell 7 and Windows PowerShell 5.1 showed every warm second-iteration row using zero package requests, zero downloaded bytes, full package-cache hits, and full extraction-cache hits. Use `LastBottleneck` for broad wall-clock context and `LastWarmOptimizationLane` for the next actionable lane, because warm-cache root dependency time can still hide dependency branch, extraction copy, promotion move, and install-lock work.
- Warm-cache detail summaries now split `RootDependency` into coalesced dependency wait pressure and the slowest materialized package. This keeps install reuse wins separate from save output-layout work and makes it clearer whether the next pass should tune scheduling or package materialization.
- A fresh full Graph/Az `HeavySaveCacheGate` rerun with the dependency-branch fields measured PowerShell 7 warm second saves at 2548.57 ms for `Microsoft.Graph` and 4225.29 ms for `Az`; Windows PowerShell 5.1 measured 2391.72 ms and 4991.83 ms. All four warm rows used zero package requests, zero downloaded bytes, and full package/extraction cache hits. `Microsoft.Graph` pointed at `Microsoft.Graph.Applications` as the slowest branch, while `Az` pointed at `Az.Advisor` and showed repeated shared `Az.Accounts` waits. The next useful optimization should reduce shared dependency pressure or final materialization cost; simply raising the dependency fan-out cap from 32 to 64 was tested and not kept because it worsened Az median and Windows PowerShell 5.1 warm rows even though one Graph PowerShell 7 warm row improved.
- Extracted-cache materialization now avoids a redundant `Directory.CreateDirectory` call for every copied file after the destination directory tree has already been pre-created. A PowerShell 7 `Graph.Full.Save.ManagedWarmCache` diagnostic after that cleanup measured iteration 1 at 4204.16 ms and the warm second iteration at 2091.29 ms with zero package requests, zero downloaded bytes, 40 package-cache hits, and 40 extraction-cache hits. The optimization artifact now points the next Graph warm-cache pass at materialization/promotion move rather than extraction-cache locking.
- Valid extracted-cache payloads can now be validated before install and written directly to brand-new final module paths, while force/reinstall still uses the staged rollback path. A PowerShell 7 `Graph.Full.Save.ManagedWarmCache` diagnostic measured the warm second save at 1071.01 ms, with 39 direct materializations, zero package requests, zero downloaded bytes, 40 cache hits, and `ManagedLastPromotionFinalMoveMs=0.69`. Windows PowerShell 5.1 measured the same full Graph warm second save at 3141.57 ms, with 39 direct materializations and `ManagedLastPromotionFinalMoveMs=1.00`. A single-module Windows PowerShell 5.1 `Graph.Authentication.Save` run stayed on the staged path because the benchmark save operation exercises root force semantics; it still measured the warm second save at 606.96 ms.
