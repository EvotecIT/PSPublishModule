...
- `managed-install-details-<n>.json`: managed install package tree details, written under the benchmark run folder for install runs. It includes package/dependency tree counts, unique package/status counts, per-package elapsed/download/extraction/promotion timings, request counts, cache hits, and byte counts.
