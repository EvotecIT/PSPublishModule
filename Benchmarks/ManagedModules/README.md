...
Run the Authenticode compatibility gate when validating certificate/signature behavior for install and save. The current lane compares managed delivery with PSResourceGet because both expose an equivalent signature-check switch for these operations:
...
- `SecurityGate` promotes Authenticode install/save compatibility into a named suite scenario. The first PowerShell 7 and Windows PowerShell 5.1 proof for `ThreadJob` 2.1.0 with `-AuthenticodeCheck` passed with strict managed-rank gates. PowerShell 7 measured managed install/save at 1344.88/1009.92 ms versus PSResourceGet at 3037.22/2392.46 ms. Windows PowerShell 5.1 measured managed install/save at 1641.74/830.40 ms versus PSResourceGet at 3610.44/2346.62 ms. Managed checked the signable `.psd1`, `.psm1`, and dependency `.dll`/manifest files before promotion. This is initial file-signature evidence, not full catalog/timestamp/short-lived-certificate parity.
