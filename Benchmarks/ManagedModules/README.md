...
- Suite scenarios can now carry owned gate thresholds and `-UseScenarioGates` applies them. `SaveGate` owns its 1.05 ratio, `SpeedGate` owns strict managed rank 1, and `LifecycleGate` owns its 1.25 ratio. A `SaveGate` run using `-UseScenarioGates` passed on PowerShell 7 and Windows PowerShell 5.1 with managed ranking first at 1250.96 ms and 1542.30 ms.
