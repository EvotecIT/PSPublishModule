...
`VersionResolutionMilliseconds` includes dependency-range resolution that happens before the child install request starts. `DependencyBranchOverheadMilliseconds` captures any remaining explicit branch time not explained by version resolution, queue wait, download, extraction, dependency, promotion, coalescing, or install-lock phases.
