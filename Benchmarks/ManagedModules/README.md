...
Use `Export-ManagedModuleBenchmarkReadmeTable.ps1` to turn one or more retained `suite-scoreboard.csv` artifacts into the normalized public README table. The exporter recalculates every comparable provider against Managed as the `1.00x` baseline, preserves failed and skipped cells, and renders known capability gaps such as ModuleFast save rows as `Not equivalent`. Pass explicit scoreboards for release-candidate evidence, or use `-BenchmarkRoot` plus `-LatestPerScenario` for local exploratory refreshes. Add `-SplitByOperation` when updating the public README so install, save, update, publish, and repair evidence land in separate tables. The optional `-ReadmePath` mode updates the marker block in the root README.
