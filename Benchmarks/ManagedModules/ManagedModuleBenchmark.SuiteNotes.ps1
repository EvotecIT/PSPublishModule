...
function Write-ManagedBenchmarkSuiteNotes {
    ...
    if ($OptimizationRows -and $OptimizationRows.Count -gt 0) {
        ...
        $lines.Add('')
        Add-ManagedBenchmarkMarkdownTable -Lines $lines -Rows $OptimizationRows -Columns @('BenchmarkRole', 'Suite', 'Scenario', 'Host', 'Operation', 'ManagedMs', 'Bottleneck', 'LastMs', 'LastBottleneck', 'LastBottleneckMs', 'LastDependencyMs', 'LastSlowestDependencyPackage', 'LastSlowestDependencyPackageParent', 'LastSlowestDependencyPackageMs', 'LastCoalescedWaitMs', 'LastInstallLockWaitMs', 'ExtractionCacheLockWaitMs', 'LastExtractionCacheLockWaitMs', 'LastPromotionMoveMs', 'LastSlowestMaterializedPackage', 'LastSlowestMaterializedPackageMs', 'LastSlowestMaterializedPackageExtractionCacheLockWaitMs', 'LastSlowestMaterializedPackagePromotionMoveMs', 'LastNextQuestion')
        $lines.Add('')
    }
