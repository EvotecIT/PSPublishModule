...
function Write-ManagedBenchmarkSuiteNotes {
    ...
    if ($EngineRows -and $EngineRows.Count -gt 0) {
        ...
        $lines.Add('')
        Add-ManagedBenchmarkMarkdownTable -Lines $lines -Rows $EngineRows -Columns @('BenchmarkRole', 'Suite', 'Scenario', 'Host', 'Operation', 'Engine', 'MedianMs', 'WarmRuns', 'WarmMedianMs', 'WarmMinMs', 'WarmMaxMs', 'MedianOutputMB', 'MedianOutputMBPerSecond', 'MedianOutputFilesPerSecond', 'Runs', 'Succeeded', 'Failed', 'Skipped', 'FirstMs', 'LastMs')
        $lines.Add('')
    }
