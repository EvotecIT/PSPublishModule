...
function Write-ManagedBenchmarkSuiteNotes {
    ...
    if ($OptimizationRows -and $OptimizationRows.Count -gt 0) {
        ...
        $lines.Add('')
        Add-ManagedBenchmarkMarkdownTable -Lines $lines -Rows $OptimizationRows -Columns @('BenchmarkRole', 'Suite', 'Scenario', 'Host', 'Operation', 'ManagedMs', 'Bottleneck', 'LastMs', 'LastBottleneck', 'LastBottleneckMs', 'LastCoalescedWaitMs', 'LastInstallLockWaitMs', 'LastSlowestMaterializedPackage', 'LastSlowestMaterializedPackageMs', 'LastNextQuestion')
        $lines.Add('')
    }
