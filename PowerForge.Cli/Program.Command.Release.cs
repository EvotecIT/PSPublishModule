...
internal static partial class Program
{
    ...
    private static void WriteAppleReceiptSummary(ILogger logger, PowerForgeAppleReleaseReceipt receipt)
    {
        ...
        {
            logger.Info(
                $" -> {target.Name} {target.Platform}/{target.DistributionRoute} {target.Version} ({target.Build}): " +
        ...
        foreach (var diagnostic in receipt.Diagnostics.Take(5))
            logger.Warn($"{diagnostic.Code}: {diagnostic.Summary} Next: {diagnostic.Action}");
    }
        ...
        PowerForgeReleaseRequest? defaults = null)
    {
        ...
        request.ReleaseVersion = ChooseString(request.ReleaseVersion, TryGetOptionValue(argv, "--release-version"));
