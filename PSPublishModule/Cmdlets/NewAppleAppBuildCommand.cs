...
/// </summary>
[Cmdlet(VerbsCommon.New, "AppleAppBuild", SupportsShouldProcess = true)]
[OutputType(typeof(AppleAppBuildResult))]
public sealed class NewAppleAppBuildCommand : PSCmdlet
{
    ...
    /// <summary>Physical device identifier used for destination resolution.</summary>
    [Parameter]
    ...
    /// <summary>xcrun executable name or path, used when resolving Device by name.</summary>
    [Parameter]
    ...
    /// <summary>Maximum build runtime in minutes.</summary>
    [Parameter]
    ...
    protected override void ProcessRecord()
    {
        ...
        if (!ShouldProcess(resolvedProjectPath, $"Build Apple app for scheme '{Scheme}'"))
            return;

        var result = new AppleDeviceDeploymentService()
            .BuildAsync(CreateRequest(resolvedProjectPath))
            .GetAwaiter()
            .GetResult();

        if (!result.Succeeded)
            ThrowTerminatingError(AppleDeviceCommandSupport.CreateProcessError(result.ProcessResult, "AppleAppBuildFailed", "xcodebuild build failed."));

        WriteObject(result);
    }

    private AppleAppBuildRequest CreateRequest(string resolvedProjectPath)
        => new()
        {
