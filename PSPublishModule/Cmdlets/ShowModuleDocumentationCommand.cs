...
public sealed partial class ShowModuleDocumentationCommand : PSCmdlet
{
    ...
    protected override void ProcessRecord()
    {
        ...
        if (!string.IsNullOrEmpty(manifestPathMeta))
        {
            ...
            if (psoFull != null)
            {
                ...
                foreach (var item in EnumerateItems(reqVal))
                {
                    ...
                    {
                        var dep = new DocumentationModuleDependency { Kind = DocumentationDocumentationModuleDependencyKind.Required };
                    ...
                    {
                        var name = item?.ToString()?.Trim(); if (!string.IsNullOrEmpty(name)) meta.Dependencies.Add(new DocumentationModuleDependency { Kind = DocumentationDocumentationModuleDependencyKind.Required, Name = name! });
                    }
                }
                ...
                {
                    var name = e?.ToString()?.Trim(); if (!string.IsNullOrEmpty(name)) meta.Dependencies.Add(new DocumentationModuleDependency { Kind = DocumentationDocumentationModuleDependencyKind.External, Name = name! });
                }
                ...
                foreach (var d in meta.Dependencies.ToList())
                {
                    ...
                    foreach (var item in EnumerateItems(req2Val))
                    {
                        ...
                        {
                            var dep2 = new DocumentationModuleDependency { Kind = DocumentationDocumentationModuleDependencyKind.Required };
                        ...
                        {
                            var name2 = item?.ToString()?.Trim(); if (!string.IsNullOrEmpty(name2)) d.Children.Add(new DocumentationModuleDependency { Kind = DocumentationDocumentationModuleDependencyKind.Required, Name = name2! });
                        }
                    }
                }
            }
        ...
        {
            case "raw":  meta.ExamplesMode = PowerForge.ExamplesMode.Raw; break;
            case "maml": meta.ExamplesMode = PowerForge.ExamplesMode.Maml; break;
            default:      meta.ExamplesMode = PowerForge.ExamplesMode.Auto; break;
        }
