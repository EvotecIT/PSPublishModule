...
public sealed class ModulePipelineRunner
{
    ...
    public ModulePipelineResult Run(ModulePipelineSpec spec, ModulePipelinePlan plan, IModulePipelineProgressReporter? progress = null)
    {
        ...
        var appleAppSteps = steps
            .Where(s => s.AppleAppSegment is not null)
            .ToDictionary(s => s.AppleAppSegment!, s => s);
        var xcodeProjectVersionSteps = steps
            .Where(s => s.XcodeProjectVersionSegment is not null)
            .ToDictionary(s => s.XcodeProjectVersionSegment!, s => s);
        ...
        try
        {
            ...
            var appleAppResults = new List<AppleAppReleasePreparationResult>();
            var xcodeProjectVersionResults = new List<XcodeProjectVersionUpdateResult>();
            if (plan.AppleApps is { Length: > 0 })
            {
                var editor = new XcodeProjectVersionEditor();
                foreach (var segment in plan.AppleApps)
                {
                    appleAppSteps.TryGetValue(segment, out var step);
                    SafeStart(step);
                    try
                    {
                        var result = PrepareAppleApp(plan, segment, editor);
                        appleAppResults.Add(result);
                        xcodeProjectVersionResults.Add(result.XcodeProjectVersionResult);
                        SafeDone(step);
                    }
                    catch (Exception ex)
                    {
            ...
            if (plan.XcodeProjectVersions is { Length: > 0 })
            {
                ...
                {
                    xcodeProjectVersionSteps.TryGetValue(segment, out var step);
                    SafeStart(step);
                    try
                    {
                        xcodeProjectVersionResults.Add(UpdateXcodeProjectVersion(plan, segment, editor));
                        SafeDone(step);
                    }
                    catch (Exception ex)
                    {
        ...
        ModuleInstallerResult? installResult = null;
            ...
            return new ModulePipelineResult(
                ...
                signingResult,
                xcodeProjectVersionResults.ToArray(),
                appleAppResults.ToArray());
        }
        catch (Exception ex)
        {
            pipelineFailure = ex;
