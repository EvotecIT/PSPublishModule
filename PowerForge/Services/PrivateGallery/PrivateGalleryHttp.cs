...
internal static class PrivateGalleryHttp
{
    ...
    internal static HttpClient CreateClient(int timeoutSeconds, HttpMessageHandler? handler)
        => handler is null
            ? new HttpClient { Timeout = TimeSpan.FromSeconds(NormalizeTimeoutSeconds(timeoutSeconds)) }
            : new HttpClient(handler, disposeHandler: false) { Timeout = TimeSpan.FromSeconds(NormalizeTimeoutSeconds(timeoutSeconds)) };
