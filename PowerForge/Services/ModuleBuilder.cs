...
{
    private const string GeneratedBootstrapperMarker = "# Auto-generated by PowerForge. Do not edit.";
    private static readonly string[] BootstrapperScriptFolders = { "Enums", "Classes", "Private", "Public" };
    ...
    private readonly Func<string, IEnumerable<string>> _enumerateScriptFiles;
    ...
    public ModuleBuilder(
        ...
        : this(
            logger,
            manifestMutator,
            scriptFunctionExportDetector,
            static directory => Directory.EnumerateFiles(directory, "*.ps1", SearchOption.AllDirectories))
    {
    }
    ...
    internal ModuleBuilder(
        ...
        IModuleManifestMutator manifestMutator,
        IScriptFunctionExportDetector scriptFunctionExportDetector,
        Func<string, IEnumerable<string>> enumerateScriptFiles)
    {
        ...
        _enumerateScriptFiles = enumerateScriptFiles ?? throw new ArgumentNullException(nameof(enumerateScriptFiles));
    }
    ...
    public ModuleOwnerNote[] BuildInPlace(Options opts)
    {
        ...
        var supportsCompleteAliasAnalysis = _scriptFunctionExportDetector is IScriptAliasExportAnalysisDetector;
        if (opts.RootModuleScriptWillBeReplaced &&
        ...
        var rootOnlyAliases = opts.RootModuleScriptWillBeReplaced &&
                              ...
                              rootAliasAnalysis.IsComplete &&
                              aliasAnalysis.IsComplete
            ? rootAliasAnalysis.Aliases.Except(aliasAnalysis.Aliases, StringComparer.OrdinalIgnoreCase)
            : Array.Empty<string>();
        ...
        else
        {
            ...
            else
            {
                ...
                if (!supportsCompleteAliasAnalysis || !aliasAnalysis.IsComplete)
                {
                    _logger.Warn(
    ...
    private static ScriptFileDiscovery DiscoverAuthoredRootModuleScript(string path)
    {
        try
        {
            var isGenerated = File.ReadLines(path)
                .Take(5)
                .Any(line => string.Equals(line.Trim(), GeneratedBootstrapperMarker, StringComparison.Ordinal));
            return new ScriptFileDiscovery(
                isGenerated ? Array.Empty<string>() : new[] { path },
                isComplete: true);
        }
        catch (FileNotFoundException)
        {
            return new ScriptFileDiscovery(Array.Empty<string>(), isComplete: true);
        }
        ...
        {
            return new ScriptFileDiscovery(Array.Empty<string>(), isComplete: false);
        }
    }

    private sealed class ScriptFileDiscovery
    {
        internal ScriptFileDiscovery(IEnumerable<string> files, bool isComplete)
        {
