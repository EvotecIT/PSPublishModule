...
internal static class AppleReleaseFailureClassifier
{
    private static readonly (Regex Pattern, string Category, string Code, string Action, bool Retryable)[] Rules =
    {
        ...
        Rule("timed out|timeout|temporarily unavailable|rate[ _-]?limit(?:[ _-]?(?:ed|exceeded))?|too many requests|(?:HTTP(?: status)?|status(?: code)?|response)\\s*[:=]?\\s*(?:429|5[0-9]{2})\\b|\\b429\\s+Too Many Requests|\\b5[0-9]{2}\\s+(?:Internal Server Error|Bad Gateway|Service Unavailable|Gateway Timeout)", "transient", "APPLE_TRANSIENT", "Keep the receipt and retry the smallest failed action after confirming Apple service health.", true),
