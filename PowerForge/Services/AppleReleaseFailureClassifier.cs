...
internal static class AppleReleaseFailureClassifier
{
    ...
    {
        Rule("Apple source trust validation failed:", "source-trust", "APPLE_SOURCE_TRUST", "Remove untracked, host-dependent, or mutable build inputs; bind supported provenance to PowerForge-owned settings, then rerun Doctor from the exact source commit.", false),
        ...
        Rule("authentication|unauthorized|forbidden|API key|(?:HTTP(?: status)?|status(?: code)?|response)\\s*[:=]?\\s*(?:401|403)\\b", "authentication", "APPLE_AUTH", "Verify the App Store Connect key id, issuer id, private key, role, and repository secret wiring.", false),
        Rule("timed out|timeout|temporarily unavailable|rate[ _-]?limit(?:[ _-]?(?:ed|exceeded))?|too many requests|(?:HTTP(?: status)?|status(?: code)?|response)\\s*[:=]?\\s*(?:429|5[0-9]{2})\\b|failed\\s*\\((?:429|5[0-9]{2})\\b|\\b429\\s+Too Many Requests|\\b5[0-9]{2}\\s+(?:Internal Server Error|Bad Gateway|Service Unavailable|Gateway Timeout)", "transient", "APPLE_TRANSIENT", "Keep the receipt and retry the smallest failed action after confirming Apple service health.", true),
