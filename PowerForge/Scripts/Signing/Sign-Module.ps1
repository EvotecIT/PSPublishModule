...
try {
  ...
  if ($IsWindows -or $PSVersionTable.PSVersion.Major -le 5) {
    ...
    $targets = if ($overwrite) {
      $all
    } else {
      $all | Where-Object { (Get-AuthenticodeSignature -FilePath $_).Status -eq 'NotSigned' }
    }
    ...
    foreach ($f in $targets) {
      ...
      if ($r -and $r.Status -ne 'Valid' -and $r.Status -ne 'UnknownError') {
        throw "Signing failed for '$f' (status: $($r.Status))."
      }
      $signed++
    }
  } else {
    ...
    $targets = if ($overwrite) {
      ...
      $all | Where-Object { (Get-OpenAuthenticodeSignature -FilePath $_).Status -eq 'NotSigned' }
    }
  ...
  Write-Output ('PFSIGN::COUNT::' + [string]$signed)
  exit 0
} catch {
