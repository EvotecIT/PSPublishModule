...
public sealed class ModulePipelineRegressionParityTests
{
    ...
    public void Plan_TracksExternalModulesSeparately_AndInboxModulesAreIgnored()
    {
        ...
        try
        {
            ...
            Assert.DoesNotContain(plan.RequiredModules, m => string.Equals(m.ModuleName, "Az.Accounts", StringComparison.OrdinalIgnoreCase));
    ...
    [Fact]
    public void Run_WritesNonInboxExternalModulesOnlyToPsData()
    {
        var root = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), "PowerForge.Tests", Guid.NewGuid().ToString("N")));
        try
        {
    ...
    private static MissingFunctionAnalysisResult CreateMissingFunctionAnalysisResult(params string[] commandNames)
    {
        return new MissingFunctionAnalysisResult(
            summary: (commandNames ?? Array.Empty<string>())
                ...
                .Select(name => new MissingCommandReference(name, string.Empty, string.Empty, isAlias: false, isPrivate: false, error: string.Empty))
                .ToArray(),
