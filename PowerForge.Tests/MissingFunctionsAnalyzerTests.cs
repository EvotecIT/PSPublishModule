...
public sealed class MissingFunctionsAnalyzerTests
{
    ...
    [Fact]
    public void Analyze_ReportsDeclarationPassedAsArgumentToDotInvokedCommand()
    {
        var analyzer = new MissingFunctionsAnalyzer();
        const string code = """
            function Outer {
                function Save-Callback { param([scriptblock] $Callback) }
                . Save-Callback { function Invoke-PowerForgeDotArgumentLocal { 'local' } }
                Invoke-PowerForgeDotArgumentLocal
            }
            """;
        ...
        Assert.Contains(report.Summary, item =>
            string.Equals(item.Name, "Invoke-PowerForgeDotArgumentLocal", StringComparison.OrdinalIgnoreCase));
    }
