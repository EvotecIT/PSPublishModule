...
public sealed class ModulePublisherRepositoryVersionTests
{
    ...
    [Fact]
    public void Publish_ManagedModule_RejectsRequiredModuleMirroringUntilManagedPathExists()
    {
        var stagingRoot = Path.Combine(Path.GetTempPath(), "PowerForgeTests", Guid.NewGuid().ToString("N"));
        using var feed = new TemporaryDirectory();
        try
        {
            Directory.CreateDirectory(stagingRoot);
            var manifestPath = Path.Combine(stagingRoot, "PSPublishModule.psd1");
            File.WriteAllText(manifestPath, "@{ ModuleVersion = '3.0.13'; GUID = 'eb76426a-1992-40a5-82cd-6480f883ef4d'; RootModule = 'PSPublishModule.psm1'; Author = 'Evotec'; Description = 'PowerForge publish test module.' }");
            File.WriteAllText(Path.Combine(stagingRoot, "PSPublishModule.psm1"), string.Empty);
            ...
            Assert.Contains("not yet available", exception.Message, StringComparison.OrdinalIgnoreCase);
            Assert.False(File.Exists(Path.Combine(feed.Path, "PSPublishModule.3.0.13.nupkg")));
        }
        finally
        {
            if (Directory.Exists(stagingRoot))
