...
public sealed partial class PowerShellCompilationArtifactHardeningTests
{
    ...
    [InlineData("Get-Date -Format FileDate")]
    [InlineData("Get-Date -Format $format")]
    [InlineData("Get-Date -Format 'bad%'")]
    ...
    public void Transpile_GetDateLiteralFormatProducesStringWithoutPowerShellHost(string format)
    {
        var document = PowerShellSourceParser.Parse(
            $"function Get-FormattedDate {{ [string] $value = Microsoft.PowerShell.Utility\\Get-Date -Format '{format}'; return $value }}",
            Path.Combine(Path.GetTempPath(), "PowerForge.Tests", "runtime-state-get-date-format.ps1"));
        var result = new PowerShellSemanticCompilationPipeline().Compile(
            new[] { document }, "net10.0", PowerShellCompilationCapabilities.TypedExecutable);

        Assert.Empty(result.Emitted.Diagnostics.Select(static diagnostic => diagnostic.Code + ": " + diagnostic.Message));
        var lowered = Assert.IsType<PowerShellLoweredRuntimeStateExpression>(
            Assert.IsType<PowerShellLoweredAssignmentStatement>(Assert.Single(result.Lowered.Functions).Statements[0]).Value);
        Assert.Equal(PowerShellRuntimeStateIntrinsicKind.FormattedCurrentLocalDateTime, lowered.Kind);
        Assert.Equal(typeof(string), Assert.Single(result.Analyzed.Functions).Locals.Single().Type.ClrType);
        var method = Assert.Single(result.Emitted.Methods);
        Assert.Contains("global::System.DateTime.Now.ToString", method.Source, StringComparison.Ordinal);
        Assert.DoesNotContain("System.Management.Automation", method.Source, StringComparison.Ordinal);
        Assert.False(method.RequiresPowerShellCommandRegions);
    }

    [Fact]
    public void Transpile_GetDateColonFormatUsesTheSameLiteralContract()
    {
        var document = PowerShellSourceParser.Parse(
            "function Get-FormattedDate { return Microsoft.PowerShell.Utility\\Get-Date -Format:yyyy }",
            Path.Combine(Path.GetTempPath(), "PowerForge.Tests", "runtime-state-get-date-colon-format.ps1"));
        var result = new PowerShellSemanticCompilationPipeline().Compile(
            new[] { document }, "net10.0", PowerShellCompilationCapabilities.TypedExecutable);

        Assert.Empty(result.Emitted.Diagnostics.Select(static diagnostic => diagnostic.Code + ": " + diagnostic.Message));
        Assert.Contains("global::System.DateTime.Now.ToString", Assert.Single(result.Emitted.Methods).Source, StringComparison.Ordinal);
    }
    ...
    [InlineData("net472")]
    [InlineData("net10.0")]
    public void Build_StrictLibraryExecutesLiteralDateFormatWithoutPowerShellRuntime(string targetFramework)
    {
        using var fixture = ArtifactFixture.Create("function Get-FormattedYear { return Get-Date -Format yyyy }");
        var result = new PowerShellCompilationArtifactBuilder().Build(new PowerShellCompilationBuildSpec(
            fixture.ScriptPath,
            Path.Combine(fixture.OutputPath, targetFramework),
            "PowerForge.FormattedYear" + targetFramework.Replace(".", string.Empty, StringComparison.Ordinal),
            PowerShellCompilationArtifactKind.Library,
            PowerShellCompilationMode.Strict,
            allowUnreviewedDependencyResolution: true)
        {
        ...
        var method = Assert.Single(assembly.GetTypes().SelectMany(static type => type.GetMethods()),
            static method => method.Name == "Get_FormattedYear");
        var before = DateTime.Now.Year;
        var value = Assert.IsType<string>(method.Invoke(null, null));
        var after = DateTime.Now.Year;

        Assert.InRange(int.Parse(value, System.Globalization.CultureInfo.InvariantCulture), before, after);
    }
    ...
    [InlineData("net10.0", "pwsh")]
    [InlineData("net472", "powershell.exe")]
    public void Build_StrictBinaryModulePreservesQualifiedLiteralDateFormatAndConversion(string targetFramework, string host)
    {
        if (targetFramework == "net472" && !RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) return;
        using var fixture = ArtifactFixture.Create(
            "function Get-FormattedYear { return Microsoft.PowerShell.Utility\\Get-Date -Format yyyy }; " +
            "function Get-FormattedYearNumber { return [int](Microsoft.PowerShell.Utility\\Get-Date -Format yyyy) }",
            ".psm1");
        var result = new PowerShellCompilationArtifactBuilder().Build(new PowerShellCompilationBuildSpec(
            fixture.ScriptPath,
            fixture.OutputPath,
            "PowerForge.FormattedDateModule" + targetFramework.Replace(".", string.Empty, StringComparison.Ordinal),
            PowerShellCompilationArtifactKind.BinaryModule,
            PowerShellCompilationMode.Strict,
            allowUnreviewedDependencyResolution: true)
        {
        ...
        Assert.True(result.Succeeded, result.Error + Environment.NewLine + result.BuildOutput);
        Assert.Equal("powerforge.command.runtime-state.get-date", Assert.Single(result.Manifest!.CommandProviders).ProviderId);
        const string proof = "$before=[DateTime]::Now.Year; $text=Get-FormattedYear; $number=Get-FormattedYearNumber; " +
                             "$after=[DateTime]::Now.Year; " +
                             "\"$($text.GetType().FullName)|$($number.GetType().FullName)|$([bool]([int]$text -ge $before -and [int]$text -le $after))|$([bool]($number -ge $before -and $number -le $after))\"";
        var original = Run(host, "-NoProfile", "-NonInteractive", "-Command",
            $"Import-Module -Name '{fixture.ScriptPath.Replace("'", "''", StringComparison.Ordinal)}' -Force; {proof}");
        var compiled = Run(host, "-NoProfile", "-NonInteractive", "-Command",
            $"Import-Module -Name '{result.ArtifactPath!.Replace("'", "''", StringComparison.Ordinal)}' -Force; {proof}");

        Assert.Equal((0, "System.String|System.Int32|True|True"), (original.ExitCode, original.StandardOutput.Trim()));
        Assert.Equal((0, "System.String|System.Int32|True|True"), (compiled.ExitCode, compiled.StandardOutput.Trim()));
        Assert.True(string.IsNullOrWhiteSpace(original.StandardError), original.StandardError);
        Assert.True(string.IsNullOrWhiteSpace(compiled.StandardError), compiled.StandardError);
    }
