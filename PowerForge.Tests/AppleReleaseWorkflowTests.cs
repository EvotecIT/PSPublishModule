...
public sealed partial class AppleReleaseWorkflowTests
{
    ...
    public void AppleActionKeepsPlansAndConfirmedMutationExplicit()
    {
        ...
        Assert.Contains("'contents.xcworkspacedata'", trackedInputs, StringComparison.Ordinal);
        Assert.Contains("'project.pbxproj'", trackedInputs, StringComparison.Ordinal);
    ...
    public void MonitorWorkflowRunsDoctorAndMaintainsOneProactiveIncident()
    {
        ...
        Assert.Contains("gh issue edit", workflow, StringComparison.Ordinal);
        Assert.DoesNotContain("gh issue comment $issue.number --body $body", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("powerforge-apple-monitor-state:", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("$monitoredErrors = @($errorDiagnostics | Where-Object { -not (Test-DeferredReadinessDiagnostic $_) })", workflow, StringComparison.Ordinal);
        Assert.Contains("$actionableErrors = @($monitoredErrors | Where-Object { -not [bool] $_.retryable })", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("state=all&per_page=100", workflow, StringComparison.Ordinal);
        Assert.Contains("gh issue reopen", workflow, StringComparison.Ordinal);
        Assert.Contains("Unable to enumerate all repository issues", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("Unable to refresh proactive Apple incident", workflow, StringComparison.Ordinal);
