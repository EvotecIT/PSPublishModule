...
public sealed partial class PowerShellCompilationSemanticOracleTests
{
    ...
    {
        const string source = "function Select-Value { [CmdletBinding()] [OutputType([int])] " +
                              "param([Parameter(ValueFromPipeline)][int] $Value) " +
                              "begin { [int] $Final = 10 } process { $Value; $Value } end { $Final } } " +
                              "function Invoke-Select { param([int[]] $Values) $Values | Select-Value } " +
                              "Invoke-Select -Values $null";
        using var fixture = OracleFixture.Create(source);
        var build = new PowerShellCompilationArtifactBuilder().Build(new PowerShellCompilationBuildSpec(
            fixture.ScriptPath,
            Path.Combine(fixture.RootPath, targetFramework),
            "PipelineLifecycleOutput" + targetFramework.Replace(".", string.Empty, StringComparison.Ordinal),
            PowerShellCompilationArtifactKind.Executable,
            PowerShellCompilationMode.Strict,
            allowUnreviewedDependencyResolution: true)
        {
        ...
        Assert.Equal(new[] { 1, 1, 2, 2, 10 }, Assert.IsType<int[]>(method.Invoke(null, new object?[] { new[] { 1, 2 } })));
        Assert.Equal(new[] { 0, 0, 10 }, Assert.IsType<int[]>(method.Invoke(null, new object?[] { null })));
        Assert.Equal(new[] { 10 }, Assert.IsType<int[]>(method.Invoke(null, new object?[] { Array.Empty<int>() })));
    }

    [PinnedSemanticHostFact]
    public void RuntimeFreePipelineLifecycleProcessOutputMatchesConfiguredExactPowerShellProfiles()
    {
        const string source = "function Select-Value { [CmdletBinding()] [OutputType([int])] " +
                              "param([Parameter(ValueFromPipeline)][int] $Value) " +
                              "begin { [int] $Final = 10 } process { $Value; $Value } end { $Final } } " +
        ...
        var strict = new PowerShellCompilationSemanticRuntimeFreeArtifactObserver().Observe(
            PowerShellCompilationSemanticOracleCatalog.PowerShell76ProfileId,
            build);
        Assert.Equal(new[] { "1", "1", "2", "2", "10" }, strict.Success.Select(static item => item.Value));
