...
public sealed class ExternalAssetPreparationServiceTests
{
    ...
    public void Prepare_RejectsProjectRootOutputPathBeforeMaterializingFiles()
    {
        ...
        try
        {
            ...
            Assert.Contains("child path", ex.Message, StringComparison.OrdinalIgnoreCase);
            Assert.False(File.Exists(Path.Combine(projectRoot, "tool.zip")));
            Assert.False(File.Exists(Path.Combine(projectRoot, "manifest.json")));
        }
        finally
        {
            TryDelete(workspace);
    ...
    [Fact]
    public void Run_RejectsExternalAssetManifestInsideAnotherBundleOutputBeforeMaterializingFiles()
    {
        var workspace = CreateTempDirectory();
        try
        {
                ...
                Segments = new IConfigurationSegment[]
                {
                    new ConfigurationExternalAssetSegment
                    {
                        Configuration = new ExternalAssetConfiguration
                        {
                            Name = "VendorTool",
                            OutputPath = "Resources/VendorTool",
                            ManifestPath = "Resources/OtherTool/vendor-manifest.json",
                            Files = new[]
                            {
                                new ExternalAssetFileConfiguration
                                {
