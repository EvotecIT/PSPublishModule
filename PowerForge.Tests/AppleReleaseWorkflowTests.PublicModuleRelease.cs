...
public sealed partial class AppleReleaseWorkflowTests
{
    ...
    public void PublicModuleReleaseEnablesOnlyExactVerifiedGitHubRecovery()
    {
        ...
        var command = $". '{escapedHelper}'; " +
                      ...
                      "if (-not $config.GitHub.ReuseExistingRelease -or -not $config.GitHub.ReplaceExistingAssets -or -not $config.GitHub.RequireExpectedExistingRelease -or $config.GitHub.ExpectedExistingReleaseId -ne 42 -or -not $config.GitHub.RequirePublishedStableRelease -or -not $config.GitHub.RequirePublishedNuGetAssets -or -not $config.GitHub.RequirePublishedModuleAssets -or $config.GitHub.PublishedModuleSource -ne 'https://www.powershellgallery.com/api/v3/index.json') { throw 'Effective recovery binding is incomplete.' }; " +
