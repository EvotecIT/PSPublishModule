...
The September 20 checkpoint recorded 1,063 compiler-gate tests and six Strict Windows programs passing. Its pinned PSSharedGoods census recorded 188/282 complete emitted functions and 36 promoted regions. The September 22 same-input census reached **196/282**, leaving 86 functions retained or unsupported; the eight additions are not all execution-qualified. These are packet-specific results, not a language-coverage percentage or speedup. An emitted Hybrid method may still delegate most of its work to PowerShell. The older heterogeneous 6/196-unit packet has different inputs and must not be compared as if it were the same census.

## Delivery order

| Priority | Milestone | Current state | Required outcome |
| --- | --- | --- | --- |
| 1 | M29 script and module coverage | Current | 30 more executed complete functions on the fixed packet, plus two module workflows and one script/application workflow with original/generated parity |
...
**Outcome:** remove recurring semantic blockers so substantially more real PowerShell functions, scripts, and modules compile and run with preserved behavior. This is a multi-wave coverage milestone, not a three-workflow task.

- [ ] Freeze the September 22 **196/282** PSSharedGoods packet and its 86-function retained/unsupported frontier with source and toolchain hashes. Re-run the baseline before counting a gain, classify every co-blocker, and separately execute the eight functions already added by the `-as` slice; none of those eight counts as new M29 coverage. Add at least two unrelated, redistributable script/module packets so one module cannot stand in for the ecosystem.
- [ ] **First checkpoint, not exit:** unlock three previously blocked complete workflows across two semantic families and exercise one ordinary module consumer. Use those cases to validate the selection, differential harness, and same-input accounting before expanding the next waves.
- [ ] Work through multiple recurring blocker families, grouped by semantic shape, ownership, effects, and lifetime rather than source name or one CLR type at a time. Target **at least 30 additional complete emitted functions** on the unchanged 282-function packet—more than one third of its current 86-function frontier—across at least four independently recurring blocker families. Keep a ledger of functions still retained, rejected, partially promoted, or blocked by another cause. A census increase without reachable execution proof cannot satisfy the exit gate.
- [ ] Qualify at least **two complete module workflows and one standalone script/application workflow** from the pinned and unrelated packets. Invoke exported commands through ordinary imports or launch the built executable, including state and cross-function calls where present. Each function counted toward the 30 must execute safely against unchanged original and generated source, either directly or in a qualified workflow; cover consequential failure paths for each changed semantic family. Do not execute untrusted administration actions merely to count coverage.
- [ ] Close reusable contracts in the canonical parser → binder → bound IR → analysis → lowering → backend owners. Differentially compare values and CLR types, identity, order/cardinality, relevant streams, scope/module state, errors, continuation, stopping, cleanup, and repeated invocation where reachable. Preserve safe Hybrid fallback and fail-closed Strict admission; claim runtime-free growth only for a separately qualified complete Strict artifact. No source-name policy or emitter-side eligibility rule may create a gain.
- [ ] After each wave, re-run the fixed packets, relevant focused tests, and the canonical compiler gate. Show no loss of previously qualified complete functions or workflows, reconcile explain/diagnose and the final ledger with executed artifacts, and record unsupported shapes and remaining co-blockers. Review hard-to-observe semantic changes before declaring the milestone closed.

**Exit gate:** the unchanged PSSharedGoods packet reaches at least **226/282 complete emitted functions** (30 new beyond the September 22 baseline), with gains from four recurring blocker families and no previously qualified losses. At least two module workflows and one standalone script/application workflow from the pinned and unrelated packets pass original/generated differential execution on their claimed supported hosts, including relevant failure and lifecycle paths. Function counts measure this packet only; they do not imply general PowerShell compatibility, runtime-free execution, or speedup. If the pinned packet changes or the frontier cannot safely support the numeric target, present the evidence and revise the gate explicitly; never silently change the denominator or weaken fallback/Strict admission to make the count.
