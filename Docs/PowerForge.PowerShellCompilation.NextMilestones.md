...
- [x] Close the first Hybrid indexed-target wave without losing prior admissions: pinned PowerInfoblox moved **41/66 → 44/66** and PSSharedGoods **196/282 → 199/282** complete emitted functions. One new PowerInfoblox function has both-host original/generated execution; five other new methods remain unexecuted. The optional AD-typed function stays hosted and its safe region survives. The corrected candidate passed 1,139/1,139 compiler tests and 6/6 Strict programs. This is one of four required blocker families and **1/30** newly executed functions, not M29 completion.
- [ ] Pin a reproducible portfolio with source revision, archive/tree hash, license and redistribution disposition, loader/dependency closure, artifact kind, target framework, semantic profile, and per-function disposition. Include PSSharedGoods, PowerInfoblox, another unrelated Evotec module, at least two independent third-party modules from different scenario families, and one standalone script/application. PowerInfoblox and independent MIT-licensed PSScriptTools are now pinned for discovery; PSScriptTools' dynamic dot-source loader is a source-closure blocker, not a 1/1-function success. Existing external assessment packets remain candidates. Never import or execute third-party source during discovery.
...
- [ ] Prove at least **three complete module workflows**: one in PowerInfoblox, one in another Evotec module, and one in an independent third-party module, plus one standalone script/application workflow. Use unchanged source, ordinary original/generated imports or launches, and deterministic offline providers for network, directory, WMI/CIM, or destructive actions. Compare values and CLR types, order/cardinality, relevant streams, state, errors, continuation, stopping, cleanup, and repeated calls on supported hosts. Do not run an untrusted or mutating administration path merely to increase a count.
