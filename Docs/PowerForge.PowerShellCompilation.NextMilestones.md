...
The first complete-workflow qualification set is fixed below. The unchanged binary-to-string function now passes its command-artifact differential gate in Strict and Hybrid modes on Windows PowerShell 5.1, PowerShell 7.4.18, and PowerShell 7.6.5. The other two workflows and the required runtime-free artifact remain open. Keep the authored functions intact; compiler improvements must apply to their general language contracts. Analyze both command and runtime-free targets because their blockers differ.
...
| PSSharedGoods `Public/Converts/Convert-BinaryToString.ps1`, commit `2a807a4f11ba458b7bc405ce3674d93838639af1` | By-value Byte-array parameter binding and alias; conditional output; non-null framework Encoding receiver; exact Unicode decoding and error continuation. Library admission also lacks pipeline binding; command admission currently rejects the conditional output expression. | Null/empty/singleton/even/odd byte arrays, malformed UTF-16, non-ASCII text, nested and invalid caller input, zero/one/many input records, alias binding, later commands after failure. |
