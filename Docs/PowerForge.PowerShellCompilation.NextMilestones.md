...
- [ ] Prove at least **three complete module workflows**: one in PowerInfoblox, one in another Evotec module, and one in an independent third-party module, plus one standalone script/application workflow. PowerInfoblox's offline query workflow, PSSharedGoods' offline firewall/time workflow, and PSScriptTools' public compiled calls have both-host original/generated full-module evidence. A platyPS public import/metadata workflow also passed, but its tested public command is hosted and the compiled helper was invoked separately. Qualify the standalone workflow before closing this item. The [assessment](PowerForge.PowerShellCompilation.Assessment.md) records a read-only search of 215 public-domain candidate scripts that produced no typed executable unit; this is a concrete gap, not workflow qualification. Use unchanged source, ordinary original/generated imports or launches, and deterministic offline providers for network, directory, WMI/CIM, or destructive actions. Compare values and CLR types, order/cardinality, relevant streams, state, errors, continuation, stopping, cleanup, and repeated calls on supported hosts. Do not run an untrusted or mutating administration path merely to increase a count.
