...
Captured loop output now has a bound statement and a lowered temporary success sink. Qualified loops collect records once, preserve explicit null and nested arrays, collapse zero records to AutomationNull, and restore the enclosing sink before later output. Initial comparisons cover zero/one/many records, nested captures, local calls, and loop flow control on PowerShell 7; scalar-result capture also compiles on Windows PowerShell 5.1. Collection-valued streamed records retain PowerShell execution on 5.1 because native cmdlet wrapping changes observable serialization. Captures with return transfers, constrained or runtime-owned targets, hosted command regions, or statement-error paths remain explicit boundaries. The complete hexadecimal workflow still needs numeric-union CLR arguments, partial-error continuation, and NoEnumerate qualification.
