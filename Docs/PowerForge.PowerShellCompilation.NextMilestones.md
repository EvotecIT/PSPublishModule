...
The statement-error host foundation is qualified separately from compiler admission. Linked C# command fixtures match source functions on Windows PowerShell 5.1, PowerShell 7.4.18, and PowerShell 7.6.5 for CLR parse failures, inherited and explicit continuation preferences, caller catches, typed catch records, explicit throws, finally output, error redirection, read-only caller preferences, and error-variable cleanup after normal completion or downstream stop. Native conversion also preserves authored method exceptions, reflection wrappers, and call-depth exceptions. All nine differential tests pass. Independent review exposed an exception-wrapping defect; the fix and related preference correction passed targeted confirmation. The bridge invokes the loaded host's error machinery without executing source expressions. Its private host contract is checked before invocation work. Binder/IR/lowering integration, generated-artifact qualification, nested local-command boundaries, and runtime-free error semantics remain open; this foundation does not increase emitted-workflow counts.
