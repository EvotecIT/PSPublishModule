...
Updated: 2026-09-23. Active continuation: `feature/powershell-compiler`. The readiness assessment records the qualified revisions and remaining integration boundaries.
...
- [ ] Record same-input Hybrid census and explain results for every packet, with complete emitted, retained, region-only, rejected, and input-failure counts. Keep a per-function ledger and classify co-blockers. Check each packet's previous disposition baseline, even when its aggregate count improves. The September 23 third-party assessment still has five regression records from one previously emitted Locksmith function; two other records were a checker error that counted a complete-method promotion as loss of its earlier partial region. Resolve or explicitly requalify the real regression before accepting a new baseline.
- [x] Qualify recurring blocker families by the portfolio's shared semantic shapes, effects, ownership, and lifetime. The accepted local waves cover **30/30 distinct newly emitted and differentially executed complete functions** across four families: native indexed targets, dynamic format templates, bounded source closure, and native statement-array capture. The packet totals are PowerInfoblox 7, PSSharedGoods 7, platyPS 1, and PSScriptTools 15, so no module supplies more than half. The fixed same-input five-packet, two-target ledger has no lost previously emitted method. Partial regions and unexecuted emitted functions receive no count. Standalone workflow and remaining portfolio records are separate open items below.
- [ ] Prove at least **three complete module workflows**: one in PowerInfoblox, one in another Evotec module, and one in an independent third-party module, plus one standalone script/application workflow. PowerInfoblox's offline query workflow, PSSharedGoods' offline firewall/time workflow, and PSScriptTools' public compiled calls have both-host original/generated full-module evidence. A platyPS public import/metadata workflow also passed, but its tested public command is hosted and the compiled helper was invoked separately. Qualify the standalone workflow before closing this item. The [assessment](PowerForge.PowerShellCompilation.Assessment.md) records a 215-script public-domain screen that produced no typed executable unit and an offline IPv4 application candidate whose helper emits as a Hybrid module native function but not as a Hybrid executable function. Add a guarded native-function registration path in the executable host before admitting that helper: capability, explain, composer, generated runtime, and actual invocation must agree. Merely enabling `NativeFunctionBinding` would delete its source body without installing a callable replacement. Preserve the existing fallthrough/no-output distinction exposed by the Braille candidate. Use unchanged source, ordinary original/generated imports or launches, and deterministic offline providers for network, directory, WMI/CIM, or destructive actions. Compare values and CLR types, order/cardinality, relevant streams, state, errors, continuation, stopping, cleanup, and repeated calls on supported hosts. Do not run an untrusted or mutating administration path merely to increase a count.
- [ ] Implement reusable contracts in the canonical parser → binder → bound IR → analysis → lowering → backend owners. Preserve safe Hybrid fallback and fail-closed Strict admission. Claim runtime-free growth only after a separately qualified complete Strict artifact; do not add source-name policy or emitter-side eligibility rules.
- [ ] After each wave, rerun the fixed portfolio, focused differential tests, and canonical compiler gate. Show no loss of any previously qualified function or workflow, reconcile explain/diagnose with executed artifacts, and record unresolved co-blockers. Review hard-to-observe semantic changes before closure.
