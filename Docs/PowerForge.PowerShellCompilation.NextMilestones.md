...
- [x] Qualify a pinned second Evotec full-module workflow through offline providers. Generated `Get-ComputerFirewall` and `Get-ComputerTime` matched the original PSSharedGoods module on both hosts across positive, repeated, empty and missing-data cases; `Get-WinADDSAGuid` remains unqualified after a zero-output exploratory probe. Focused tests passed 2/2 and the full gate passed 1,143/1,143 compiler tests plus 6/6 Strict programs. Current progress is **6/30** newly executed functions across three module packets, still only two blocker families; this test-only wave adds no language admission.
- [ ] Pin a reproducible portfolio with source revision, archive/tree hash, license and redistribution disposition, loader/dependency closure, artifact kind, target framework, semantic profile, and per-function disposition. Include PSSharedGoods, PowerInfoblox, another unrelated Evotec module, at least two independent third-party modules from different scenario families, and one standalone script/application. PowerInfoblox, independent MIT-licensed PSScriptTools, and platyPS 0.14.2 are pinned; the latter has a built and executed module packet. Apache-2.0 powershell-yaml 0.4.12 is pinned for read-only discovery, but its top-level enum currently blocks all **14/14** functions on both target frameworks and binary-loader closure is outstanding. Pinned InvokeBuild 5.14.23 exports script aliases rather than module functions and may serve a standalone packet. PSScriptTools' dynamic dot-source loader remains a source-closure blocker, not a 1/1-function success. Existing external assessment packets remain candidates. Never import or execute third-party source during discovery.
- [ ] Establish bounded source closure for the pinned PSScriptTools loader (`Get-ChildItem -Exclude | ForEach-Object { . $_.FullName }`) or select a different safely runnable independent module with a reviewed, re-pinned baseline. Prove discovered-file identity and host-conditional imports before counting its functions or executing a deterministic workflow.
