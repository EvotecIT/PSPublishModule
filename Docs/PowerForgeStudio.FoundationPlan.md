...
`PowerForgeStudio` is the working title for a Windows desktop release cockpit that coordinates
...
- `PR 7` foundation in progress: retry/resume affordances, an opt-in smoke harness for a live repository build contract, guarded real-signing smoke mode, richer custom-feed/private-repository verification, mixed-repo wrapper hardening, explicit PSPublishModule engine visibility in the shell, repository-detail drill-in for queue/checkpoint evidence and adapter summaries, adapter output/error tail visibility plus checkpoint payload inspection, a lightweight GitHub inbox for PR/workflow/release attention signals, persisted release-drift snapshots for "what changed since the last known release" visibility, portfolio focus filters for attention/ready/queue-active triage, saved portfolio triage state so the shell restores the last used focus/search view, one-click presets for Ready Today, Attention, Queue Active, Reset, and USB Waiting, clickable queue-aware dashboard cards for Ready Today, USB Waiting, Publish Ready, Verify Ready, and Failed, a ranked release inbox that merges queue blockers, USB pauses, publish/verify-ready work, GitHub pressure, ready-today candidates, and failed git quick actions into one action list, repository-family grouping so primary repos, worktrees, and review clones can be triaged as one operational unit, family-level queue actions for scoped prepare/retry flows, a clickable family-lane board that maps each family member into Ready/USB Waiting/Publish Ready/Verify Ready/Failed/Completed lanes and now surfaces failed git quick actions at family scope, a reusable workspace snapshot service that composes scan/plan/GitHub/drift/queue state for both UI and non-UI hosts, reusable inbox, dashboard, family summary, and family-lane projections inside that shared snapshot so CLI and future hosts can answer both "what needs attention now?" and "show me this repo family as a board" without WPF-only logic, reusable snapshot query helpers for selecting inbox slices, resolving a family board by key/name/path, and resolving a repository by name/path, a reusable workspace command service that can prepare queues, run the next queue step, approve USB signing, and retry failed queue items with optional family scoping from non-WPF hosts, a reusable workspace git command service that can list safe git quick actions for a resolved repository, execute one action, and persist the resulting receipt for any host, a reusable queue command service that owns prepare/run-next/USB-approve/retry orchestration against persisted state, reusable station projection snapshots for signing/publish/verification manifests plus receipt summaries, a small CLI host that proves the same orchestrator can run outside WPF and now supports direct `inbox`, `dashboard`, `families`, `family-lane`, `git-actions`, `git-run-action`, `queue-prepare`, `queue-run-next`, `queue-approve-usb`, and `queue-retry-failed` commands, persisted publish receipt source paths so verification can keep following real artifacts after recovery or restart, a git preflight layer that flags detached HEAD, missing upstream, dirty/behind state, and protected `main`/`master` push risk before the release flow reaches a terminal git error, remote GitHub governance confirmation for default branch and branch protection state, copyable git remediation commands in the repository detail pane, executable git quick actions for safe local commands plus GitHub PR/branch/compare links, and persisted last-action receipts so the shell can show output/error tails and promote failed git actions back into the inbox after refresh or restart
