...
- [x] 2026-06-29: `SecurityGate` now promotes Authenticode install/save compatibility into a named benchmark suite. The first `ThreadJob` 2.1.0 proof passed on PowerShell 7 and Windows PowerShell 5.1 with strict managed-rank gates and `-AuthenticodeCheck` enabled by the scenario. PowerShell 7 measured managed install/save at 1345/1010 ms versus PSResourceGet at 3037/2392 ms. Windows PowerShell 5.1 measured managed install/save at 1642/830 ms versus PSResourceGet at 3610/2347 ms. Managed checked the signable `.psd1`, `.psm1`, and dependency `.dll`/manifest files before promotion. Full catalog, timestamp, and short-lived certificate parity remains tracked separately.
