...
- [x] 2026-06-28: Native update preparation failures are now captured as typed failed benchmark runs when `ContinueOnError` is enabled, so provider seed/install failures still produce reportable transition-gate evidence.
- [x] 2026-06-28: Native compatibility disk evidence is now measured from the validated installed module directory instead of the broader sandbox root; a PowerShell 7.6.3 `ThreadJob` 1.0.0 to 2.1.0 update smoke validated nonzero managed and PSResourceGet disk evidence from the resolved module paths.
- [x] 2026-06-28: Transition-gate results now expose provider-specific compatibility limitations as typed JSON data and Markdown report columns, and fallback reasons prefer the concrete provider failure over a generic missing-baseline message.
...
- [x] 2026-06-28: PowerShell 7.6.3 imported the current local `net8.0` build after PSGallery v2-read selection and ran `Measure-ManagedModule -Operation Install,Save,Update,Publish -Engine Managed,PSResourceGet,PowerShellGet -EnableNativeInstallUpdateBenchmark -RequireManagedEvidenceReady -RequireCompatibilityRetirementReady` against managed-published local packages. All 12 runs completed, all four transition gates were `Ready`, managed evidence was `Ready`, and compatibility retirement was `Ready`; report root: `C:\Users\przemyslaw.klys.EVOTEC\AppData\Local\Temp\pf-local-retire-ps7-20260628-051238`.
- [x] 2026-06-28: Windows PowerShell 5.1.26100.8655 imported the current local `net472` build after PSGallery v2-read selection and ran the same all-operation local-feed proof. All 12 runs completed, all four transition gates were `Ready`, managed evidence was `Ready`, and compatibility retirement was `Ready`; report root: `C:\Users\przemyslaw.klys.EVOTEC\AppData\Local\Temp\pf-local-retire-winps-20260628-051407`.
