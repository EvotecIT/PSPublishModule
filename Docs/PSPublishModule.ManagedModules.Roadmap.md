...
- [x] Keep ModuleFast selected in save scoreboard scenarios as an explicit skipped capability marker, so README tables do not confuse unsupported save operations with missing comparison evidence.
...
- [x] 2026-06-29: `SaveGate` now selects ModuleFast and PowerShellGet alongside managed and PSResourceGet so wide save scoreboards keep all provider columns visible. A PowerShell 7 smoke measured managed first at 1152.37 ms, PSResourceGet at 1501.48 ms, PowerShellGet at 3922.09 ms, and ModuleFast as an explicit `Skipped` cell because it has no save command.
...
- [x] 2026-06-29: Host comparison output now keeps median gate timing separate from first-iteration and warm-last timing. `suite-host-comparison.csv` and `suite-notes.md` expose `BaselineFirstMs`/`ComparisonFirstMs` and `BaselineLastMs`/`ComparisonLastMs` so full Graph/Az save cache diagnostics explain cold materialization versus warm-cache materialization without reading detail JSON by hand.
- [x] 2026-06-29: Full `Az` 16.0.0 install now has fresh provider-matrix evidence on PowerShell 7 after the dependency-selection prewarm pass. Managed completed in 5577.84 ms with 204 package rows, 103 unique packages, 104 whole-operation repository requests, and 137.5 MB downloaded; PSResourceGet completed in 419585.41 ms, PowerShellGet completed in 183095.28 ms, and ModuleFast failed resolving `Az.DataTransfer(1.0.0)` from its provider-default source.
- [x] 2026-06-29: Full `Microsoft.Graph` 2.38.0 save now has fresh cross-host heavy-save evidence. Managed ranked first on PowerShell 7 at 4103.53 ms versus PSResourceGet at 127675.43 ms and PowerShellGet at 91328.80 ms; managed ranked first on Windows PowerShell 5.1 at 5813.84 ms versus PSResourceGet at 55415.47 ms, with PowerShellGet skipped by the provider lane and ModuleFast explicitly skipped because it has no save command.
- [x] 2026-06-29: Managed repository clients now expose and apply `MaxConnectionsPerServer`, defaulting to 32 so the HTTP connection policy matches bounded dependency delivery instead of relying on framework defaults. Focused repository/install tests passed, and a repeated Windows PowerShell 5.1 managed-only full Graph save measured a 4408.20 ms median with summed download time around 32-41 seconds per iteration, improving over the immediately preceding 5813.84 ms heavy-save row while keeping the same 40 package-delivery requests.
- [x] 2026-06-29: Managed detail artifacts now include `CoalescedWaitMilliseconds` on each package row. The summary still reports total and slowest coalesced waits, but the raw detail rows can now be sorted directly to separate dependency wait pressure from ordinary already-installed no-op rows before changing the scheduler.
- [x] 2026-06-29: Managed install results now carry explicit `CoalescedWaitElapsed` timing for waits behind another in-flight install target. Benchmark detail rows prefer that measured value and only fall back to inference for synthetic objects, so scheduler optimization can distinguish true dependency coalescing from normal already-installed skips.
...
- [x] 2026-06-29: Suite scenario lists, summaries, host comparisons, metadata, optimization-target artifacts, and gate failures now also emit `BenchmarkInterpretation`. This puts the human-readable meaning directly beside the numbers, for example that `HeavySaveCacheGate` is a managed-only package-cache/materialization diagnostic while `HeavySaveGate` is the real save-provider scoreboard.
- [x] 2026-06-29: Warm-cache save detail summaries now report coalesced dependency wait pressure and the slowest materialized package, and suite optimization notes promote the last-iteration values. This makes the install-vs-save distinction explicit: install lanes can win through reuse/no-op behavior, while save diagnostics show whether the remaining work is shared dependency scheduling or output tree materialization.
- [x] 2026-06-29: Expanded-cache materialization now uses the platform file-copy path on modern .NET too, matching the existing Windows PowerShell 5.1 path. A PowerShell 7 full `Microsoft.Graph` warm-cache save smoke improved the second iteration from 1678 ms to 1266 ms, with `Microsoft.Graph.Files` materialization dropping from 289 ms to 108 ms while keeping zero package requests, zero downloaded bytes, 40 package-cache hits, and 40 extraction-cache hits.
...
- [x] 2026-06-29: A repeated `SaveGate` rerun exposed a current Windows PowerShell 5.1 gap: managed saved `Microsoft.Graph.Authentication` 2.38.0 at 1667 ms while PSResourceGet measured 1505 ms, with the managed detail row showing the package path using two package requests and one redirect. The `net472` PowerShell Gallery download path now uses the same direct CDN package URL as modern PowerShell and keeps the v2 package endpoint only as a CDN-miss fallback. The repeated gate then passed on both hosts: PowerShell 7 managed median 1173 ms, Windows PowerShell 5.1 managed median 984 ms, both rank 1 with one package request and zero redirects.
- [x] 2026-06-29: The repeated exact-version install lifecycle gate passed on both PowerShell 7 and Windows PowerShell 5.1 for `Microsoft.Graph.Authentication` 2.38.0 and `Az.Accounts` 5.5.0 with Managed, ModuleFast, and PSResourceGet in the install-capable comparison set. Managed ranked first for all eight timed rows: PS7 Graph no-op/force 556/721 ms, PS7 Az no-op/force 582/724 ms, PS5.1 Graph no-op/force 565/868 ms, and PS5.1 Az no-op/force 555/888 ms. Timed managed rows used zero repository requests; no-op rows spent about 3-5 ms in the managed root operation, and force rows reused package and extraction caches. The suite optimization artifact correctly points at child-host harness overhead rather than install engine work for this small exact-version lane.
- [x] 2026-06-29: The full `Microsoft.Graph` 2.38.0 `HeavySaveGate` scoreboard was rerun on both hosts after the `net472` CDN fix. Managed ranked first on PowerShell 7 at 4816 ms versus PSResourceGet at 53278 ms and PowerShellGet at 68946 ms. Managed also ranked first on Windows PowerShell 5.1 at 5471 ms versus PSResourceGet at 55364 ms and PowerShellGet at 89473 ms. Both managed rows saved about 1.05 GB, used 40 package requests, followed zero redirects, downloaded 186.5 MB of package data, and retained only the compact CSV/JSON evidence after output-root cleanup.
- [x] 2026-06-29: The full `Az` 16.0.0 `HeavySaveGate` scoreboard was rerun on both hosts after the `net472` CDN fix. Managed ranked first on PowerShell 7 at 5726 ms versus PowerShellGet at 128869 ms and PSResourceGet at 146363 ms. Managed also ranked first on Windows PowerShell 5.1 at 8561 ms versus PowerShellGet at 151052 ms, while PSResourceGet still failed after 84264 ms with zero output. Both managed rows saved about 623 MB across 2789 files, used 103 package requests, followed zero redirects, and downloaded 137.5 MB of package data. ModuleFast is retained in save scoreboards as an explicit skip because it has no save operation. This replaces the older Windows PowerShell 5.1 full-Az save concern with current evidence: PS5.1 is still about 1.5x PS7 for managed full-Az save, but no longer shows the old 31.9-second managed path.
- [x] 2026-06-29: The post connection-policy full `Az` 16.0.0 `HeavySaveGate` save scoreboard was rerun on both hosts. Managed ranked first on PowerShell 7 at 5499 ms versus PSResourceGet at 167241 ms and PowerShellGet at 132082 ms. Managed also ranked first on Windows PowerShell 5.1 at 5802 ms; PSResourceGet failed during long-path package extraction, PowerShellGet was not available in the disposable host, and ModuleFast remained an explicit skip because it has no equivalent save command. The managed Windows PowerShell row was 1.06x the PowerShell 7 baseline while saving about 623 MB across 2789 files with 103 package requests, zero redirects, and 137.5 MB downloaded.
- [x] 2026-06-29: The current-head exact install/save `LifecycleGate` was rerun with `-RepeatCount 3`, rotated engine order, warm cache, scenario gates, and output-root cleanup for `Microsoft.Graph.Authentication` 2.38.0 and `Az.Accounts` 5.5.0 on PowerShell 7 and Windows PowerShell 5.1. Managed ranked first in all 16 install/save no-op/force rows. PowerShell 7 medians ranged from 28-82 ms for save rows and 621-763 ms for install rows; Windows PowerShell 5.1 medians ranged from 33-146 ms for save rows and 604-878 ms for install rows. Timed managed rows used zero repository requests, and the Windows PowerShell rows were 0.94x-1.77x the matching PowerShell 7 baselines.
- [x] 2026-06-29: `SecurityGate` now promotes Authenticode install/save compatibility into a named benchmark suite. The latest `ThreadJob` 2.1.0 proof passed on PowerShell 7 and Windows PowerShell 5.1 with strict managed-rank gates, `-AuthenticodeCheck` enabled by the scenario, and `GateManagedMinAuthenticodeCheckedFiles=1`. PowerShell 7 measured managed install/save at 1387/816 ms versus PSResourceGet at 3209/3065 ms. Windows PowerShell 5.1 measured managed install/save at 1417/800 ms versus PSResourceGet at 3228/2469 ms. Suite summaries expose `ManagedAuthenticodeCheckedFiles=4`, covering the signable `.psd1`, `.psm1`, and dependency `.dll`/manifest files before promotion, and the scenario-owned minimum means a fast but unchecked run fails. Full catalog, timestamp, and short-lived certificate parity remains tracked separately.
- [x] 2026-06-29: The expanded package-cache materialization path now uses the Windows platform copy primitive on `net472` while keeping the existing buffered stream path for modern PowerShell. This is a targeted Windows PowerShell 5.1 optimization from `HeavySaveCacheGate` evidence, not a broad PS7 claim. Full `Az` 16.0.0 warm-cache save on Windows PowerShell 5.1 improved the second iteration from the prior 6744 ms record to 3900 ms with 103 package-cache hits, 103 extraction-cache hits, zero package requests, and zero downloaded bytes. Current PowerShell 7 diagnostic reruns stayed in the 3526-3755 ms second-iteration range while using the unchanged modern stream path, so further PS7 work should be measured separately before changing that code path.
- [x] 2026-06-29: Suite host comparisons now have an optional `-ManagedMaxWindowsPowerShellVsPowerShell7` gate that writes `suite-host-gate.csv` and fails when a Windows PowerShell 5.1 managed row is missing or slower than the matching PowerShell 7 baseline by more than the requested ratio. The first `SecurityGate` smoke with a 2.0 ratio passed: Windows PowerShell 5.1 measured 1.01x PowerShell 7 for managed install and 1.06x for managed save while still reporting four checked Authenticode files.
- [x] 2026-06-29: Scenario-owned host drift gates are now part of the named lifecycle and security rows, so `-UseScenarioGates` enforces the PowerShell 5.1 versus PowerShell 7 budget without repeating `-ManagedMaxWindowsPowerShellVsPowerShell7` on every command. Host comparison rows carry `GateManagedMaxWindowsPowerShellVsPowerShell7`, `suite-host-gate.csv` is written when either an explicit host gate or scenario-owned host gate is active, and tests cover both enforced and observation-only host comparison rows.
- [x] 2026-06-29: The scenario-owned host drift path was proven end to end with a fresh `SecurityGate` run on PowerShell 7 and Windows PowerShell 5.1 using `-UseScenarioGates`. Managed passed both the provider rank/signature gate and the host drift gate, with `suite-host-gate.csv` emitted and empty. PowerShell 7 measured managed install/save at 1990/904 ms versus PSResourceGet at 3155/2416 ms; Windows PowerShell 5.1 measured managed install/save at 1552/1058 ms while PSResourceGet failed its own timed signature lane. All managed rows reported four checked Authenticode files.
- [x] 2026-06-29: Authenticode benchmark artifacts now separate catalog-file evidence from general signable-file evidence. Managed verification results, detail JSON, comparison rows, suite summaries, metadata, and gate failures can expose `ManagedAuthenticodeCatalogFiles`; `-ManagedMinAuthenticodeCatalogFiles` and scenario-owned `GateManagedMinAuthenticodeCatalogFiles` can fail a security lane that expects catalog evidence. A PowerShell 7 `SecurityGate` smoke passed with managed install/save at 1359.76/683.8 ms, `ManagedAuthenticodeCheckedFiles=4`, and `ManagedAuthenticodeCatalogFiles=0` for both rows. This is evidence plumbing for the next catalog fixture, not yet catalog-member, timestamp, or short-lived-certificate parity.
- [x] 2026-06-29: Warm-cache suite aggregation now preserves first/last managed phase timings into suite summaries, optimization targets, and notes. A full Graph/Az `HeavySaveCacheGate` rerun on PowerShell 7 and Windows PowerShell 5.1 used zero warm package requests, zero warm downloaded bytes, full package-cache hits, and full extraction-cache hits in every second iteration. The regenerated `suite-notes.md` now shows `LastBottleneck=RootDependency` for Graph and Az on both hosts: Graph PS7/PS5.1 warm second saves measured 2092/3227 ms, and Az PS7/PS5.1 measured 4291/4112 ms. This replaces the earlier vague materialization suspicion with a concrete next target: dependency fan-out/materialization after cache hits.
...
- [ ] Investigate warm-cache full-family save dependency fan-out/materialization, because current `HeavySaveCacheGate` `LastBottleneck` evidence points at root dependency time after package and extraction caches are already hot.
  - [x] 2026-06-29: Added an operation-scoped installed-version cache to the managed install context so parallel dependency branches can reuse known installed module versions instead of rediscovering the same destination root state. The first full Graph/Az `HeavySaveCacheGate` rerun after this change kept the warm package/extraction cache proof intact (`LastDownloadBytes=0`, full cache hits) and produced richer throughput evidence: Graph warm saves wrote about 1005 MB at 3702 ms on PowerShell 7 and 2149 ms on Windows PowerShell 5.1; Az warm saves wrote about 594 MB at 4055 ms on PowerShell 7 and 6176 ms on Windows PowerShell 5.1. The result is still mixed, so this item remains open and the next pass should isolate dependency-branch time from extraction copy and promotion move cost instead of treating all root dependency time as lookup overhead.
  - [x] 2026-06-29: `suite-optimization-targets.csv` now adds `LastWarmOptimizationLane`, `LastWarmOptimizationLaneMs`, `LastMaterializationMs`, materialization throughput, and the dominant materialization phase. This keeps broad `LastBottleneck` evidence while explicitly separating dependency branch cost from extraction/promotion move cost on warm-cache save rows.
  - [x] 2026-06-29: Expanded-cache materialization now uses bounded parallel file copy for larger payloads, with a higher Windows PowerShell 5.1 threshold so small Graph-style package sets stay on the lower-overhead serial path. The follow-up `HeavySaveCacheGate` rerun kept every warm second iteration at zero package requests, zero downloaded bytes, and full package/extraction cache hits. Warm-last saves improved on PowerShell 7 from Graph 3702 ms / Az 4055 ms to Graph 2665 ms / Az 2155 ms, and on Windows PowerShell 5.1 for Az from 6176 ms to 4314 ms; Windows PowerShell 5.1 Graph remained noisy at 3026 ms versus the prior 2149 ms. Because package work runs concurrently, summed dependency/extraction/promotion phase milliseconds can exceed wall-clock time; keep this item open until the next pass separates wall-clock critical path from summed child-package work.
  - [x] 2026-06-29: Extracted-cache materialization no longer calls `Directory.CreateDirectory` for every copied file after the destination directory tree has already been pre-created. A PowerShell 7 `Graph.Full.Save.ManagedWarmCache` diagnostic after that cleanup measured the warm second iteration at 2091 ms with zero package requests, zero downloaded bytes, 40 package-cache hits, and 40 extraction-cache hits. The optimization artifact now points the next Graph warm-cache pass at materialization/promotion move rather than extraction-cache locking.
  - [x] 2026-06-29: Modern managed installs now stage under the target module folder so final promotion can rename the version directory from a close sibling path, while Windows PowerShell 5.1 keeps the temp-root staging path for long-path safety. Installed-version discovery now ignores `.pfmm-stage-*` folders so interrupted staging cannot look like an installed version. A PowerShell 7 `Graph.Full.Save.ManagedWarmCache` diagnostic kept zero warm package requests/downloads and 40 package/extraction cache hits; the slowest materialized package promotion move dropped from the prior 703 ms evidence to 0.7 ms, shifting the dominant materialization phase back to extraction. The full warm second iteration was noisy at 3241 ms, so this closes the promotion-move hotspot but leaves root dependency coordination and extraction/materialization as the next targets.
  - [x] 2026-06-29: Warm-cache benchmark details now expose dependency scheduler queue wait and root dependency unattributed time. A candidate extracted-cache materialization throttle was tested but not retained because reruns were volatile and could worsen the lane. The latest PowerShell 7 `Graph.Full.Save.ManagedWarmCache` diagnostic still kept zero warm package requests/downloads and 40 package/extraction cache hits; it measured the warm second iteration at 4152 ms, with `ManagedLastDependencyQueueWaitMs=0`, `ManagedLastRootDependencyUnattributedMs=2549`, and a promotion-move spike on `Microsoft.Graph.Identity.SignIns` (`ManagedLastSlowestMaterializedPackagePromotionMoveMs=1227.73`). The next optimization should explain or reduce the unattributed root dependency gap and promotion-move variance before adding another concurrency throttle.
  - [x] 2026-06-29: Optimization-target reports now carry first/last dependency branch elapsed time and last branch overhead beside the critical-lane fields, so warm-cache save evidence no longer hides overlapped dependency branch work. A focused PowerShell 7 `Az.Full.Save.ManagedWarmCache` rerun (`Ignore/Benchmarks/MM/S20260629170910-22544`) measured warm iteration 2 at 2519 ms with 103 package-cache hits, 103 extraction-cache hits, zero package requests, zero downloaded bytes, `LastDependencyBranchElapsedMs=50874`, and `LastDependencyBranchOverheadMs=5`. That points away from scheduler overhead and toward shortening the root critical branch and the slowest direct materialization package path.
  - [x] 2026-06-29: Slowest-package materialization reporting now includes file count, extracted MB, MB/sec, and files/sec. A focused PowerShell 7 `Az.Full.Save.ManagedWarmCache` rerun (`Ignore/Benchmarks/MM/S20260629171529-165608`) measured warm iteration 2 at 2129 ms with `Az.Cdn` as the slowest materialized package: 101 files, 14.04 MB, 1122 ms package time, 12.51 MB/sec, and 89.98 files/sec. The next engine optimization should be a measured materialization-throughput change, not another dependency branch scheduler change.
  - [x] 2026-06-29: Tested and rejected raising the modern-runtime per-package parallel copy threshold from 8 files to 128 files. The focused PowerShell 7 `Az.Full.Save.ManagedWarmCache` rerun (`Ignore/Benchmarks/MM/S20260629171850-162496`) improved the local `Az.Cdn` slowest-package rate to 18.09 MB/sec, but the warm iteration regressed to 4305 ms and root dependency critical-path gap rose to 2562 ms. Keep the current threshold until a broader materialization strategy improves the whole operation, not just one package.
  - [x] 2026-06-29: Tested and rejected trusting known extracted-cache byte totals to avoid per-file length reads during cache materialization. The focused PowerShell 7 `Az.Full.Save.ManagedWarmCache` rerun (`Ignore/Benchmarks/MM/S20260629172100-130832`) regressed warm iteration 2 to 4651 ms and raised the root dependency critical-path gap to 3143 ms. The remaining optimization target is broader critical-path coordination/materialization behavior, not per-file byte accounting.
  - [x] 2026-06-29: Dependency detail rows now carry full branch elapsed time so the benchmark can separate scheduler queue wait from dependency version resolution, child install, and synchronous branch execution. The first retained PowerShell 7 `Graph.Full.Save.ManagedWarmCache` diagnostic measured the warm second iteration at 1285 ms with zero package requests/downloads and 40 package/extraction cache hits; `ManagedLastDependencyQueueWaitMs=0`, `ManagedLastDependencyBranchElapsedMs=1102`, `ManagedLastRootDependencyMs=1055`, and the slowest individual branch was only 109 ms. A final retained rerun stayed cache-correct but showed the remaining variance clearly: warm second iteration 3776 ms, `ManagedLastRootDependencyMs=3543`, `ManagedLastDependencyBranchElapsedMs=5184`, `ManagedLastRootDependencyUnattributedMs=1868`, and `ManagedLastPromotionMoveMs=2460`. A worker-pool branch scheduling experiment was measured and rejected: it reduced attribution gaps but made the suite median worse while inflating lock/coalescing/promotion contention (`ManagedLastInstallLockWaitMs=14106`, `ManagedLastPromotionMoveMs=20402`). The next retained optimization should reduce promotion/install-lock contention or introduce more selective scheduling instead of blindly parallelizing every dependency branch.
  - [x] 2026-06-29: Package reader and install planning now preserve package/manifest dependency order instead of sorting dependencies alphabetically. This keeps shared direct dependencies in the publisher-supplied order; for the current full Graph package, `Microsoft.Graph.Authentication` is installed before modules that depend on it, so nested Authentication rows become cheap already-installed checks rather than a critical child install under `Microsoft.Graph.Applications`. PowerShell 7 `Graph.Full.Save.ManagedWarmCache` repeat-3 evidence stayed cache-correct (`ManagedLastRepositoryRequests=0`, `ManagedLastDownloadBytes=0`, 40 package/extraction cache hits) and measured a warm last iteration at 2549 ms with zero coalesced wait and `ManagedLastDependencyMs=0.88`; a repeat-2 run on Windows PowerShell 5.1 measured a warm last iteration at 2446 ms with the same zero-download/full-cache proof. Promotion-move variance remains visible (`ManagedLastPromotionMoveMs=1309` on the PS7 repeat-3 run), so the next optimization should target materialization/promotion stability rather than dependency order.
  - [x] 2026-06-29: Suite summaries, engine summaries, optimization targets, and generated notes now report steady-state warm metrics (`WarmRuns`, `WarmMedianMs`, `WarmMinMs`, and `WarmMaxMs`) by excluding the first successful iteration. A focused PowerShell 7 `Az.Full.Save.ManagedWarmCache` repeat-3 rerun (`Ignore/Benchmarks/MM/S20260629172852-150264`) kept cache correctness (`ManagedLastRepositoryRequests=1`, `ManagedLastDownloadBytes=0`, 103 package-cache hits, and 103 extraction-cache hits), measured a full median of 7527 ms, and exposed a warmer post-first median of 5748 ms with a 3969-7527 ms warm range. This confirms that full-family save optimization must be judged by repeated warm distributions, not a single second-iteration or last-iteration value.
  - [x] 2026-06-29: Broad dependency fan-out now seeds the publisher's first dependency before launching the remaining parallel branches once dependency count exceeds the engine concurrency cap. This keeps small dependency sets unchanged while giving full-family meta packages a chance to install their core dependency before nested branches pile up behind it. A PowerShell 7 `Az.Full.Save.ManagedWarmCache` repeat-3 rerun (`Ignore/Benchmarks/MM/S20260629173622-170960`) stayed cache-correct and improved the warm median from the previous 5748 ms to 2192 ms, with warm runs at 1957-2427 ms and last root dependency time down to 1431 ms. A matching Windows PowerShell 5.1 rerun (`Ignore/Benchmarks/MM/S20260629173735-29292`) stayed cache-correct with warm median 4622 ms and last warm 3741 ms; PS5.1 still shows root dependency attribution gaps, so this is a retained scheduling improvement rather than a completed heavy-save optimization.
  - [x] 2026-06-29: The same first-dependency fan-out build was rerun against full `Microsoft.Graph` 2.38.0 warm-cache save on both hosts. PowerShell 7 (`Ignore/Benchmarks/MM/S20260629174046-180108`) kept zero warm repository/package requests, zero downloaded bytes, 40 package-cache hits, and 40 extraction-cache hits; warm median was 1335 ms with a 1187 ms last iteration. Windows PowerShell 5.1 (`Ignore/Benchmarks/MM/S20260629174106-68816`) kept the same cache correctness and measured a 2120 ms warm median with a 2314 ms last iteration. Graph no longer shows coalesced or dependency-queue wait in these retained rows; the remaining measured gap is root dependency attribution/materialization stability, especially on Windows PowerShell 5.1 and the broader Az family.
  - [x] 2026-06-29: Warm optimization-target classification now labels elapsed package delivery with zero package requests, zero downloaded bytes, and package-cache hits as `PackageCache` instead of `Download`. Reprocessing the retained full Graph/Az warm-cache summaries points Graph PS7 at materialization, Graph PS5.1 at package-cache delivery, Az PS7 at materialization, and Az PS5.1 at dependency fan-out, so the next engine pass can target the correct lane per host/family.
  - [x] 2026-06-29: Tested and rejected changing cached-package SHA256 reads from `File.OpenRead` to the repository client's asynchronous sequential-scan package stream. The goal was to reduce Graph Windows PowerShell 5.1 package-cache delivery without trusting sidecar hashes, but retained reruns were not better enough to keep: PS5.1 produced one noisy warm median at 2958 ms (`Ignore/Benchmarks/MM/S20260629175619-192052`) and one near-baseline warm median at 2100 ms (`Ignore/Benchmarks/MM/S20260629175645-167600`), while PowerShell 7 regressed to a 3731 ms warm median (`Ignore/Benchmarks/MM/S20260629175708-175792`) versus the previous retained 1335 ms. Keep the current hash path until a cache optimization improves both hosts or is safely host-specific.
  - [x] 2026-06-29: Added `Measure-ManagedModuleMaterializationStrategies.ps1` as an external benchmark probe for the real managed save engine. It seeds the package cache once, then compares the current extracted-payload path against package-archive extraction by deleting only the selected package's expanded-cache folder before the timed archive run. Isolated retained probes show extracted payload remains the right steady-state materialization path: `Az.Cdn` 6.0.2 measured extracted/archive medians of 97.67/136.26 ms on PowerShell 7 and 199.17/734.04 ms on Windows PowerShell 5.1; `Microsoft.Graph.Files` 2.38.0 measured 149.89/351.79 ms on PowerShell 7 and 164.29/731.71 ms on Windows PowerShell 5.1. Do not add a package-archive fallback heuristic from the full-Az slow package row; the remaining full-family issue is contention/coordination under broad concurrent materialization, not the isolated extracted-cache primitive.
  - [x] 2026-06-29: Added an explicit managed dependency fanout control (`DependencyConcurrency` in the reusable install/update requests and `-DependencyConcurrency` on `Install-ManagedModule`, `Save-ManagedModule`, and `Update-ManagedModule`) while preserving the existing default when omitted. The external `Measure-ManagedModuleDependencyConcurrency.ps1` probe now reruns normal comparison rows across fanout values and can include ModuleFast for install-capable scoreboards while keeping save as a managed/provider-save diagnostic. Use this to measure Graph/Az on both PowerShell 7 and Windows PowerShell 5.1 before changing the default fanout.
...
- [x] Add an explicit suite-level PS5.1-versus-PS7 host gate so host regressions are caught separately from provider scoreboard failures.
- [x] Keep focused single-host scenario reruns compatible with scenario-owned host gates by preserving `MissingComparison` host evidence without failing when the comparison host was not requested.
...
- [x] Decide whether public `-Proxy` and `-ProxyCredential` parameters are required on managed cmdlets or remain repository/profile policy.
  - [x] 2026-06-29: `Find-ManagedModule`, `Install-ManagedModule`, `Save-ManagedModule`, `Update-ManagedModule`, and `Publish-ManagedModule` expose public `-Proxy` and `-ProxyCredential` parameters. The cmdlets map them into the reusable `ManagedModuleRepositoryClientOptions` path; the default still uses the system proxy behavior, and `-ProxyCredential` requires an explicit `-Proxy` so credentials are not silently ignored.
- [x] Add PSResourceGet-style `-RequiredResource` and `-RequiredResourceFile` support for module batch installs.
  - [x] 2026-07-04: `Install-ManagedModule` accepts nested required-resource hashtables and PowerShell data files for module resources, including per-resource `Version`, `Repository`, `Prerelease`, `Scope`, `Reinstall`, `AcceptLicense`, `NoClobber`, and dependency-skip settings. `TrustRepository` and `Quiet` are accepted for resource-map compatibility while repository trust and output behavior remain governed by managed command policy.
- [x] Add initial Windows managed `-AuthenticodeCheck` support for install/save/update signable files before promotion.
- [x] Add separate benchmark evidence and gate fields for catalog files so catalog-specific fixtures cannot pass by only checking ordinary signable files.
...
- [x] 2026-06-29: Benchmark child process handling now reads stdout/stderr concurrently and has a configurable child timeout, so suite runs fail with a bounded diagnostic instead of wedging when a child host blocks. The previously stuck Windows PowerShell 5.1 `Graph.Authentication.SaveExact.NoOpForce` managed lane completed with `SaveNoOp` at `190.65 ms` and `SaveForce` at `127.52 ms`; the force row used zero repository requests, one package/extraction cache hit, and split `14.14 ms` total promotion move into `1.61 ms` final move, `2.08 ms` backup move, and `10.46 ms` backup cleanup. A PowerShell 7 full `Microsoft.Graph` warm-cache retry then completed under the safer harness. Its warm second save used zero repository requests/download bytes and 40 package/extraction cache hits, but measured `7596.11 ms`; `Microsoft.Graph.Teams` dominated with `5548.70 ms` spent in final directory promotion, no overwrite, no backup move, and no cleanup. The next engine optimization target is therefore brand-new target materialization/promotion, not repository, dependency, or overwrite cleanup.
- [x] 2026-06-29: The exact-version `LifecycleGate` Graph.Authentication/Az.Accounts no-op and force suite passed on PowerShell 7 and Windows PowerShell 5.1 with `-RepeatCount 3`, rotated engine order, warm cache mode, scenario gates, and output-root cleanup (`Ignore\Benchmarks\MM\S20260629164515-96728`). Managed ranked first in all 16 scoreboard rows. PS7 medians were Graph install force/no-op `713.74/604.05 ms`, Graph save force/no-op `70.61/27.83 ms`, Az install force/no-op `750.26/649.12 ms`, and Az save force/no-op `76.24/28.80 ms`. Windows PowerShell medians were Graph install force/no-op `841.42/581.59 ms`, Graph save force/no-op `100.99/30.35 ms`, Az install force/no-op `835.18/577.18 ms`, and Az save force/no-op `138.93/35.27 ms`. Host-drift gates had zero violations; the largest PS5.1-vs-PS7 ratio was Az save force at `1.82x` against the `2.0x` gate. Install scoreboards included the install-equivalent speed provider, while save scoreboards kept install-only providers as non-equivalent skipped capability markers.
