...
- [x] 2026-06-29: The full `Az` 16.0.0 `HeavySaveGate` scoreboard was rerun on both hosts after the `net472` CDN fix. Managed ranked first on PowerShell 7 at 5726 ms versus PowerShellGet at 128869 ms and PSResourceGet at 146363 ms. Managed also ranked first on Windows PowerShell 5.1 at 8561 ms versus PowerShellGet at 151052 ms, while PSResourceGet still failed after 84264 ms with zero output. Both managed rows saved about 623 MB across 2789 files, used 103 package requests, followed zero redirects, and downloaded 137.5 MB of package data. This replaces the older Windows PowerShell 5.1 full-Az save concern with current evidence: PS5.1 is still about 1.5x PS7 for managed full-Az save, but no longer shows the old 31.9-second managed path.
