...
- [x] 2026-06-29: `SecurityGate` now promotes Authenticode install/save compatibility into a named benchmark suite. The first `ThreadJob` 2.1.0 proof passed on PowerShell 7 and Windows PowerShell 5.1 with strict managed-rank gates and `-AuthenticodeCheck` enabled by the scenario. PowerShell 7 measured managed install/save at 1364/997 ms versus PSResourceGet at 3041/2476 ms. Windows PowerShell 5.1 measured managed install/save at 1400/1286 ms versus PSResourceGet at 4190/2662 ms. Suite summaries now expose `ManagedAuthenticodeCheckedFiles=4`, covering the signable `.psd1`, `.psm1`, and dependency `.dll`/manifest files before promotion. Full catalog, timestamp, and short-lived certificate parity remains tracked separately.
