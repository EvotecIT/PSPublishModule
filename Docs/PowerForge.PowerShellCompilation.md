...
- `if`/`elseif`/`else`, conservative scalar `switch`, `for`, `while`, `foreach` over typed arrays or an explicitly typed scalar string, and catch-all `try`/`catch` with bounded `finally` blocks;
...
- lookup-only, case-insensitive string dictionaries created from homogeneous ordinary or `[ordered]` string hashtable literals;
...
- direct local function calls in Strict executables, including positional, named, alias, unique-abbreviation, switch, mandatory, and omitted-default binding for the supported parameter subset;
...
The current five-product frontier candidate reports 1,132 authored files, 1,222 units, 15 typed, 1,207 fallback, and zero parse errors. PSSharedGoods rises from seven to eight typed units; the other product totals remain unchanged. An earlier exploratory count reached 18, but inspection of the emitted C# showed that four command regions contained nested closures over locals or module state that the generated cmdlet did not bind. Those functions correctly remain fallback. This is the intended gate behavior: semantic safety wins over a larger eligibility number.
...
| --- | ---: | ---: | ---: | ---: | --- | ---: | ---: |
| PowerInfoBlox `9de3730` | 58 | 58 | 1 / 57 | 1.72% | 51 / 51 | 202,133 bytes | 2 files, 35 lines, 1 mapped method |
| PSSharedGoods `12e9c25` | 283 | 285 | 8 / 277 | 2.81% | 226 / 226 | 1,327,623 bytes | 2 files, 171 lines, 8 mapped methods |
| PSWriteHTML `fa88b1b` | 242 | 323 | 4 / 319 | 1.24% | 291 / 291 | 1,429,685 bytes | 2 files, 106 lines, 4 mapped methods |
| O365Essentials `fad8288` | 284 | 286 | 2 / 284 | 0.70% | 214 / 214 | 745,364 bytes | 2 files, 54 lines, 2 mapped methods |
| ADEssentials `b2b1f76` | 265 | 270 | 0 / 270 | 0% | 157 / 157 | 1,878,963 bytes | 2 files, 13 lines, 0 mapped methods |
