...
The executable-provider matrix now uses the same locked package route to prove scalar and collection string success values, verbose/debug/warning/information/host/error sinks, terminating adapter errors, escaped CLR entry-point identifiers, fail-closed null results, and an exact two-assembly managed dependency closure in Strict and Hybrid. The compiler reader invokes the same canonical contract validator as the SDK packer, and it reconciles exact dependency IDs and versions with the package's NuGet metadata. Reviewed dependency content identities are independently checked against NuGet's resolved lock and acquired bytes during isolated project restore. These are runtime observations of the packaged implementation, not metadata credit. Cooperative cancellation, deterministic resource cleanup, non-string values, NativeAOT, and representative provider families remain required before Milestone 16 is complete.
