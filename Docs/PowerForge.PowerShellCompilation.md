...
Canonical command and pipeline semantics are complete within their bounded provider contract. Milestones 14–18 are complete for the explicitly supported target profiles, measured optimization contract, external provider SDK/trust boundary, and reproducible semantic/diagnostic contracts. New command families still add one deterministic binder/registry owner, typed stage/cardinality contracts, and lowering support rather than coordinated special cases in analysis, emission, shaping, and census.
