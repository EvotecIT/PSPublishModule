...
The manifest maps source/resource policy, one named semantic profile, provider packages and trust, an exact artifact matrix, dependency/provider lock paths, an optional ABI baseline, and diagnostic/IR policy onto the existing compiler contracts. Each target must have a unique kind/mode/TFM/RID/architecture/deployment identity. No source or module identity changes compiler behavior. In the current candidate, `SemanticProfileId` is validated and carried in project/package evidence but is not yet an effective compiler-target input: it does not change target hashing, binding, lowering, provider selection, or cache identity. Until that roadmap gate closes, a profile name is evidence metadata and must not be used to claim different compiled semantics.
...
Canonical command and pipeline semantics have one implementation route within the built-in bounded provider contract. Milestones 14, 15, and 17 are complete within their documented profiles; Milestones 16 and 18 are partial. The external provider SDK currently validates metadata packages but cannot yet supply independently implemented executable lowering/runtime-free adapters, and provider publisher/license claims are not yet reconciled with canonical package metadata and signer policy. The semantic-oracle harness exists, but semantic profiles do not yet govern compilation and not every promoted feature has exact-host structured differential evidence. New command families must still add one deterministic binder/registry owner, typed stage/cardinality contracts, and lowering support rather than coordinated special cases in analysis, emission, shaping, and census.
...
The compiler is ready for targeted semantic expansion through its canonical parser → binder → bound IR → lowering → backend route. It is not yet ready for a broad “compile arbitrary PowerShell natively” promise or a public stable-channel push. The fixed public Hybrid packet currently builds, imports, and invokes 10/10 modules while safely retaining or runtime-routing all 735 authored units, but it emits no CLR units. The separate six-workload assessment emits 3/185 units (1.62%) and 3/173 functions (1.73%) and performs no complete-workload execution. Hybrid is therefore useful today as a fail-safe migration/delivery architecture; broad typed-emission benefit remains the next product proof.

Before widening syntax and command coverage, the roadmap requires five closure items: make semantic profiles effective compiler inputs; finish structured exact-host semantic oracles; turn the provider SDK from metadata-only registration into an executable provider ABI with authoritative package/signer trust; enforce both corpus baselines through one bounded acquisition owner; and split the 799-line lowered backend plus the 1,501-line module bootstrapper by responsibility. This ordering protects one semantic brain and keeps coverage work from embedding profile, trust, or acquisition behavior in local workarounds.

WMI, CIM, CDXML, Active Directory-style binary modules, native/process calls, and COM are present with deliberately different guarantees. Hybrid fixtures prove that PowerShell-hosted routes can be retained safely. A direct C# local CIM query proves part of the typed management-adapter foundation. They do not yet prove that arbitrary PowerShell WMI/CIM/CDXML commands are emitted as runtime-free CLR. Strict management support still needs the executable provider ABI, generated-artifact execution for the complete operation family, authentication and transport contracts, and local/remote target-host failure and cleanup tests. Certificate authentication is not currently a usable CIM request input and must be implemented or removed from the advertised authentication enum.
