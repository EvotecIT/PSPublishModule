...
The reusable workflow serializes releases per repository and invokes the matching tagged action. The action installs an exact `PowerForge.Build` package version, so the workflow, action, and engine do not drift between retries.
...
2. runs the plugin npm validation/build or produces the configured integration zip;
3. commits only the version metadata with source PR and merge trailers;
4. pushes the version commit to the default branch;
5. creates or resumes `v<version>` at that exact commit and uploads the HACS asset;
6. reads the release and tag back from GitHub and verifies the source marker, tag target, and required asset.
...
- repository metadata ahead of the latest release is published without incrementing again;
- existing releases and same-named assets are reused or replaced;
- mismatched tag targets and missing HACS assets fail closed.
