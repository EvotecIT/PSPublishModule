...
- [x] Pass the complete compiler gate with 975/975 tests and all six Strict `net10.0` `win-x64` programs. Build `PowerForge.PowerShell` for both `net10.0` and `net472` with zero warnings and errors.
...
- [ ] Choose the next family from grouped remaining consumer contracts—object/member/index access, collection mutation, or complete pipeline flow—only when several unchanged workflows close the same value, error, continuation, and lifetime boundary. Do not widen the factory intrinsic by type count alone.
...
Exit gate: **Partial.** The continuation source implements guarded prefixes, later statement-aligned regions, structured scalar/map/vector/list transfers, retained definite assignment, fresh-local and earlier-region ownership, return-or-fallthrough envelopes, distinct no-value/explicit-null return cardinality, one closed stable-scalar `foreach` array capture, one exact local `ArrayList` factory call, post-bind opportunity discovery, complete-body/terminal-region promotion, bounded numeric representation, and integer-range proofs through the existing semantic pipeline. Region admission has no minimum statement count: one-statement and later-region artifacts preserve the same semantic contracts. The unchanged PSSharedGoods `Get-ObjectEnumValues`, `Split-Array`, `Get-ADEncryptionTypes`, `Get-ADTrustAttributes`, `ConvertTo-ImmutableID`, `Find-DatesPastWeek`, and four local-factory consumers plus CleanupMonster `Get-CloudCacheComputer` supply real structural evidence while the retained parent module remains the state and downstream-enumeration owner. General enumeration, general local-call collection closure, dynamic state, compiled collection mutation, wider collection/member/index behavior, and Strict stateful-module ownership remain open and are sequenced in M24–26. Further promotion depends on correct reusable support and complete-workload evidence; speedup is not an acceptance gate.
...
8. [ ] Select the next grouped consumer contract—object/member/index access, retained collection mutation, or complete pipeline flow—from multiple unchanged workflows before widening the local-call intrinsic or adding another type.
9. [ ] Productize CLR consumption and the source development loop through M27/M28 using the existing ABI, project, diagnostics, and source-map owners.
10. [ ] Run M29's provider/platform/public-package qualification as an independent authorized lane. Include additional detection-script states on a disposable target; do not mutate an ambient machine to complete a checkbox.
11. [ ] Optimize already qualified workloads under M30 with fresh measurements and explicit per-workload budgets.
