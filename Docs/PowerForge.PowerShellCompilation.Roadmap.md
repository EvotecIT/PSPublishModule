...
| Extensibility/profile architecture gate | Complete within promoted profiles and provider ABI 4 | Effective semantic profiles, exact-host oracles, authoritative package/signer policy, full bounded provider conformance, Strict-executable host ABI, NativeAOT adapter execution, and one representative external filesystem operation are implemented |
...
| 16. Productize the provider SDK and trust model | Complete within provider ABI 4 | Versioned providers are authoritatively identified, locked, and executable through one bounded route; typed results, streams, errors, cooperative cancellation, deterministic cleanup, dependency closure, NativeAOT, and one representative external filesystem operation are observed |
...
The branch's former 36-commit gap was resolved by integrating `origin/main`, and the post-integration focused gates were rerun. These findings did not reopen the canonical parser → binder → bound IR → lowering → backend design. Exact-host oracle coverage and the bounded provider conformance/family gate are now closed; three-family Hybrid benefit, useful external ecosystems, additional physical targets, and authorized release proof remain explicit product gates.
...
- [x] Define one versioned executable provider ABI through which an independently built package supplies typed binding/lowering metadata and a runtime-free adapter implementation without compiler-source edits, command-name branches, assembly loading in the compiler, or a second eligibility authority. ABI 4 adds the separately named `ExternalOperation` family and Strict-executable stream/cancellation host contract; ABI 3 and earlier packages fail exact negotiation instead of inheriting the new meaning.
...
3. [ ] **Run value-ranked semantic waves:** compatible `#requires`, bounded read-only process/user/culture state, nullable CLR value-member reads, stable scalar stream-message interpolation, and terminal `try`/`catch` output are complete; broad dynamic scope and untyped object members remain intentionally runtime-backed. Continue with typed invoke-member expressions, broader pipeline syntax/lifecycle, parameter typing, and common provider shapes. Every change must use parser → binder → bound IR → lowering → backend and the exact profile/oracle path. The current external packet is 7/185 emitted units and 7/173 emitted functions, so semantic breadth and measured workload benefit remain separate claims.
