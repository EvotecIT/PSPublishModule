...
- at review time, the semantic-oracle harness did not preserve every claimed observation, pin every exact host artifact, or automate immutable-profile monitoring. The current branch now carries reviewed exact pins for all three promoted hosts, native cases for every promoted family, and a read-only scheduled upstream monitor; per-case runtime-free observations and the remaining null/cardinality/encoding/process-effect envelope stay open under Milestone 18.
...
Exit gate: **Partial / exact-host pin and minimized-case foundation complete.** Named profiles govern target identity, binding/lowering, providers, caches, artifacts, diagnostics, and compatible `#requires`; schema-2 observations integrity-bind exact executable/runtime identity, preserve ordered structured streams/errors and original property order, and promotion fails closed on unpinned or unexplained results. Every promoted family now has a native minimized case executed across the three immutable pinned hosts, and upstream patch changes produce review-only evidence without changing a pin. The gate closes when corresponding runtime-free observations and the remaining null/cardinality/encoding/process-effect envelope are complete.
