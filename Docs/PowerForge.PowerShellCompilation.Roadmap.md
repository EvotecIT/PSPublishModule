...
| 23. Compile module-scale state and regions without a second runtime | Partial | Bounded live state, guarded prefixes, terminal/complete-body regions, exact scalar transfer, and a real same-function two-region Hashtable workflow are implemented; wider transfer, general additional regions, and Strict stateful-module closure continue through M24–26 |
...
| [26. Stateful modules and multiple Hybrid regions](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-26--compile-stateful-modules-and-multiple-hybrid-regions) | In progress / closes M23 | A bounded two-region scalar/Hashtable shape now runs in unchanged real module source through the existing Hybrid state owner; wider/general regions and remaining Strict lifetime closure stay open |
...
- [ ] Preserve enough canonical bound/lowered structure for an otherwise-hosted function to select and emit safe typed regions around its hosted remainder. Selection must occur in semantic/bound IR before the final ledger records the shaped result; it may not use the ledger as a planning input or add an AST-to-C# island compiler. Each promoted region must retain the graph's value, stream, error, ordering, source-map, and boundary identity and must fall back as a whole when those contracts cannot be closed.
...
Exit gate: **Partial for M26; complete for M24/M25.** The continuation source implements guarded prefixes, later statement-aligned regions, schema-6 transfer ownership including fresh stable-vector construction, return/cardinality contracts, local collection factories, Hybrid-native computed members, sequential output, and complete hosted multi-stage pipelines. Dual-host matrices cover values, identity, streams, partial failure, stopping, cancellation, cleanup, and continued execution; the same-input pinned census remains 188/282 emitted functions and 36 promoted regions. Wider/general additional regions, borrowed compiled reference mutation, runtime-free dynamic object/ETS access, computed method names, null-conditional access, and Strict stateful-module ownership remain M26-or-later work. Performance is not an admission gate.
