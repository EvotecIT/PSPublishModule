...
| 18. Establish versioned semantic oracles | Partial / minimized host-case foundation | Named profiles govern target identity and compilation; observations carry integrity-bound executable/runtime identity, promotion rejects missing pins or unexplained differences, and 19 native cases execute across all three host profiles; 7/19 cases have matching runtime-free results while 12 remain open |
...
- at review time, the semantic-oracle harness did not preserve every claimed observation, pin every exact host artifact, or automate immutable-profile monitoring. The current branch now carries reviewed exact pins for all three promoted hosts, native cases for every promoted family, a bounded schema-3 structured envelope, and a read-only scheduled upstream monitor; compatible `#requires`, profile-fixed version-major state, bounded compiler-owned dictionary flow, compile-time-safe literal conversion, local function graphs, comparison operators, and logical operators have runtime-free differential evidence, while explicit `AutomationNull` identity, framed nullable/string/culture-sensitive observation, direct child-process sequencing, and 12 per-case runtime-free observations stay open under Milestone 18.
...
- [ ] Add a minimized executable oracle case and exact observed-host matrix for every promoted semantic feature. Nineteen native `.ps1` resources cover all currently promoted feature families, provenance resolves to real applicable case IDs, and the complete 19-case matrix executes on the pinned Windows PowerShell 5.1, PowerShell 7.4.19, and PowerShell 7.6.5 hosts. The certified Strict-executable observer validates ABI type/cardinality, executes artifacts without PowerShell, and promotes compatible `#requires`, profile-fixed `$PSVersionTable.PSVersion.Major`, bounded compiler-owned dictionary flow, compile-time-safe literal conversion, local function graphs, comparison operators, and logical operators against the exact PowerShell 7.6.5 pin. Seven of 19 cases (36.84%) now have runtime-free observations; 12 still require work through their value-ranked semantic owners. This percentage measures the minimized oracle catalog, not the PowerShell language.
...
Exit gate: **Partial / bounded schema-3 and seven runtime-free cases complete.** Named profiles govern target identity, binding/lowering, providers, caches, artifacts, diagnostics, compatible `#requires`, the profile-fixed PowerShell major version, bounded compiler-owned dictionary flow, compile-time-safe literal conversion, local function graphs, comparison operators, and logical operators; schema-3 observations integrity-bind exact executable/runtime identity, reject contradictory/unbounded evidence, preserve ordered values/cardinality/streams/errors/state and original property order, and promotion fails closed on unpinned or unexplained results. Every promoted family has a native minimized case executed across the three immutable pinned hosts; those seven features now also have artifact-hash-bound Strict-executable evidence, and upstream patch changes produce review-only evidence without changing a pin. The gate closes when explicit `AutomationNull` identity, the framed nullable/string/culture-sensitive protocol, directly observed process effects, and the remaining 12 runtime-free case observations are complete.
