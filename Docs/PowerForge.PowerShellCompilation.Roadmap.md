...
Exit gate: **Partial / executable foundation expanded.** A separately built provider package is reconciled with canonical NuGet metadata, governed by package/publisher/license/signer policy, locked by exact archive and assembly identity, metadata-validated without loading it into the compiler, and invoked from generated Strict and Hybrid artifacts through the canonical provider route. The route now proves closed scalar/collection string and primitive-value types, all seven stream sinks, terminating errors, and an exact transitive managed assembly at runtime. The gate closes when executable conformance covers cooperative cancellation, deterministic cleanup, NativeAOT, and representative filesystem/HTTP or equivalent provider families. Public feed publication remains a separate release decision.
