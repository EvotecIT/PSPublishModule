...
11. [ ] Qualify the grouped range-and-match tranche through two explicit operator contracts. The target set is three range-only, three match-only, and one co-blocked unchanged workflow; preserve endpoint, slicing, regex, `$Matches`, error, and stopping behavior rather than treating either operator as direct syntax sugar.
12. [ ] Productize CLR consumption and the source development loop through M27/M28 using the existing ABI, project, diagnostics, and source-map owners.
13. [ ] Run M29's provider/platform/public-package qualification as an independent authorized lane. Include additional detection-script states on a disposable target; do not mutate an ambient machine to complete a checkbox.
14. [ ] Optimize already qualified workloads under M30 with fresh measurements and explicit per-workload budgets.
