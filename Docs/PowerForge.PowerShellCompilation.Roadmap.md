...
- [ ] Preserve enough canonical bound/lowered structure for an otherwise-hosted function to select and emit safe typed regions around its hosted remainder. Selection must occur in semantic/bound IR before the final ledger records the shaped result; it may not use the ledger as a planning input or add an AST-to-C# island compiler. Each promoted region must retain the graph's value, stream, error, ordering, source-map, and boundary identity and must fall back as a whole when those contracts cannot be closed.
  ...
  - [ ] Extend additional bounded array/foreach transformations, wider local-call closure, and compiled-owned collection mutation only with explicit canonical contracts. The complete native-consumer factory projection is still limited to the exact source-proved zero-parameter fresh-`ArrayList` shape. Direct `System.Object`/ETS values, arbitrary enumerators, statically concrete multidimensional or object-element arrays, explicit output-shaping commands, terminating control flow inside captures, and compiled reference mutation remain outside the transfer policy. Exact `System.Array` and `List<object>` references stay opaque to compiled code; their elements are observed only by the retained PowerShell boundary. Do not infer these contracts from generated C# or split a statement.
...
Exit gate: **Partial.** The continuation source implements guarded prefixes, later statement-aligned regions, structured scalar/map/vector/list transfers, retained definite assignment, fresh-local and earlier-region ownership, return-or-fallthrough envelopes, distinct no-value/explicit-null return cardinality, one closed stable-scalar `foreach` array capture, one exact local `ArrayList` factory projection across region and complete native-consumer paths, Hybrid-native computed member access, and complete hosted multi-stage pipelines with retained collection mutation. Region admission has no minimum statement count: one-statement and later-region artifacts preserve the same semantic contracts. The unchanged PSSharedGoods `Get-ObjectEnumValues`, `Get-MyIpAddress`, `Remove-DuplicateObjects`, `Split-Array`, `Get-ADEncryptionTypes`, `Get-ADTrustAttributes`, `ConvertTo-ImmutableID`, `Find-DatesPastWeek`, `New-SqlTableMapping`, `Set-XML`, and four local-factory consumers plus CleanupMonster `Get-CloudCacheComputer` supply real structural evidence. Wider enumeration, wider local-call closure, runtime-free dynamic object/ETS access, compiled-owned collection mutation, computed method names, null-conditional access, and Strict stateful-module ownership remain open and are sequenced in M24–26. Further promotion depends on correct reusable support and complete-workload evidence; speedup is not an acceptance gate.
...
19. [ ] Finish that M24 family with condition-side failure, prior-destination, injected enumeration/cleanup, wider non-success streams, and stopping proof. Keep the pinned census and unchanged complete-workflow gates current as admission expands. The first observed-catch follow-up admits only a directly bound `System.Type` local with closed linear type-literal/alias writes under catch-all handling; dual-host synthetic success/failure proof did not change the 188/282 complete-method census. A following `[ref]` slice admits one trailing direct local only for static CLR calls whose matching overloads take a by-reference final parameter; source/generated dual-host tests prove writeback on success and invalid `TryParse` input, while unsupported storage and argument-alias shapes stay hosted. Its full gate passed 1,050/1,050 tests and six Strict programs; the same-input census remains 188/282 complete methods and 36 promoted regions. Mixed type values, general reference aliasing, and the live UDP `Test-ComputerPort` workflow remain separate boundaries.
20. [ ] Advance M25 complete-command coverage through the existing output, command-region, statement-error, and local-call owners. Select complete workflows with multiple output-producing calls and retained pipeline stages; compare streams, partial failure, stopping, cancellation, and cleanup on both supported hosts.
