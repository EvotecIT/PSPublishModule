...
| 19. Qualify real dependency and interop ecosystems | Partial / two bounded gates | Signed binary wrappers, the complete typed CIM/MI and LDAP families, locked native delivery, remote execution, session reuse, bounded results, authentication failure, mutation, and safe failure/cleanup cases are proven; cancellable or isolated LDAP initialization and the controlled target-reboot/reconnect exercise remain open |
...
- [ ] Replace the Windows Negotiate provider's synchronous initial bind with a cancellable native owner or process-isolated initialization boundary. Until then the executable contracts declare `PostInitializationCooperative`, pass the token after initialization, and do not claim that `StopProcessing()` can interrupt the platform bind-response wait. Microsoft's [`ldap_bind_s` contract](https://learn.microsoft.com/en-us/windows/win32/api/winldap/nf-winldap-ldap_bind_s) documents that synchronous boundary and its platform-owned timeout.
...
Exit gate: **Partial at two explicit boundaries.** The complete typed CIM/MI and LDAP operation families execute through compiler-generated Strict artifacts without a PowerShell runtime. The release-wired management package carries three exact managed assemblies and one parsed, architecture-checked native bridge; the release-wired directory package carries its exact adapter and LDAP protocol assembly plus the reviewed Windows LDAP ABI. Both packages flow through lock, provenance, SBOM, publication disposition, native-import closure, and delivered-closure verification. Management proves local/remote operation, reusable caller-session ownership, bounded results, failure, cancellation, disconnect, and cleanup. Directory qualification proves the explicitly advertised `win-x64` LDAP/Negotiate profile, read-only integrated access, bounded paging-state release, reusable provider-created sessions, every mutation operation, true/false compare, rename, deletion, and verified cleanup in a caller-declared test location. Unqualified LDAP transport/authentication modes and arbitrary caller-created connections fail closed instead of inheriting credit from the proven profile. Directory cancellation is cooperative only after the synchronous Windows Negotiate bind; a cancellable native owner or process isolation remains required. WMI v1, CDXML, and arbitrary directory cmdlets remain explicit hosted/profile contracts rather than automatic translation. Completion also requires an authorized reboot/reconnect exercise on a disposable Windows management target; neither gate is inferred from metadata or replaced by rebooting an ambient runner.
...
5. [ ] **Qualify ecosystems:** the signed external-module/wrapper plus executable CIM and directory operation families are complete through the same provider and clean-target contracts; finish cancellable or isolated directory initialization and the controlled management-target reboot/reconnect acceptance case.
...
- [ ] Milestone 19's signed external binary-module/managed-wrapper, executable CIM and LDAP providers, safe remote operation/failure matrices, bounded Hybrid WMI/CIM/CDXML, native/process, and Windows COM routes pass; cancellable or isolated LDAP initialization and authorized disposable-target reboot/reconnect evidence remain;
