...
The canonical compiler pipeline is **Complete**, but the extensibility/profile architecture gate is **Partial**. The separately built provider SDK now supplies canonical NuGet/signer-aware package identity, reviewed lock/provenance binding, metadata-only compiler discovery, and a bounded executable adapter ABI that runs through Strict and Hybrid artifacts without loading provider assemblies into the compiler. Semantic profiles now affect target identity, compilation, providers, caches, artifacts, diagnostics, and compatible requirements. The oracle now integrity-binds observations to exact executable/runtime identities and fails promotion on unpinned hosts or unexplained differences. Milestone 18 remains **Partial** until every promoted feature has a minimized structured evidence matrix against recorded host pins, and Milestone 16 remains **Partial** until provider conformance and representative provider families cover their full runtime-free contracts. Milestones 19–22 retain the corresponding ecosystem, useful-coverage, public-release, and platform gates.
...
| Extensibility/profile architecture gate | Partial | Effective semantic profiles plus a locked executable provider ABI and authoritative package/signer policy are implemented; complete provider conformance and per-feature exact-host oracles remain open |
...
| 16. Productize the provider SDK and trust model | Partial / executable foundation | Versioned providers are authoritatively identified, locked, and executable through a bounded ABI; complete stream/error/cancellation/AOT conformance and real provider families remain open |
...
| 18. Establish versioned semantic oracles | Partial / complete minimized host-case catalog | Named profiles govern target identity and compilation; observations carry integrity-bound executable/runtime identity, promotion rejects missing pins or unexplained differences, and all 19 native cases execute across all three host profiles with matching runtime-free results; explicit `AutomationNull`, framed nullable/string observation, and sequenced child-process effects remain open |
| 19. Qualify real dependency and interop ecosystems | Partial / external qualification | Hosted interop, one direct typed CIM query, and a locked external adapter through Strict/Hybrid are proven; signed binary wrappers and the remote management matrix remain open |
| 20. Establish generic measurement and expand useful coverage | Partial / enforced measurement foundation | Public and external baselines are enforced, two unrelated emitted commands are invoked, and a realistic Strict application runs on Windows/Linux; three-family Hybrid benefit and richer application evidence remain open |
...
- `SemanticProfileId` now participates in the normalized target/hash, binder/lowering, provider selection, cache/artifact identity, diagnostics, and compatible `#requires` decisions. Exact-host per-feature oracle completeness remains open.
- provider packages are reconciled with canonical `.nuspec` identity and reviewed signer policy, and one separately built adapter executes through Strict and Hybrid without compiler loading or source-specific branches. Full streams/errors/cancellation/cleanup/AOT conformance and real provider families remain open.
- at review time, the semantic-oracle harness did not preserve every claimed observation, pin every exact host artifact, or automate immutable-profile monitoring. The current branch now carries reviewed exact pins for all three promoted hosts, native cases for every promoted family, a bounded schema-3 structured envelope, and a read-only scheduled upstream monitor; compatible `#requires`, profile-fixed version-major state, bounded compiler-owned dictionary flow, compile-time-safe literal conversion, stable-string interpolation, local function graphs, typed/defaulted parameters, bounded parameter-validation metadata, exact/alias/abbreviated parameter binding, index/member assignment targets, ordered typed catch filters, bounded scalar regex-switch matching, typed integral compound arithmetic, comparison operators, and logical operators have runtime-free differential evidence, while explicit `AutomationNull` identity, framed nullable/string/culture-sensitive observation, direct child-process sequencing, and 3 per-case runtime-free observations stay open under Milestone 18.
- the fixed public Hybrid packet still proves safe compatibility rather than typed benefit because it emits 0/735 units, but its baseline is now enforced. A separate opt-in external qualification proves one emitted command, and the fixed three-family Hybrid benefit packet remains open.

The branch's former 36-commit gap was resolved by integrating `origin/main`, and the post-integration focused gates were rerun. These findings did not reopen the canonical parser → binder → bound IR → lowering → backend design. They left exact-host oracle completeness, full provider conformance/families, three-family Hybrid benefit, ecosystem qualification, and authorized release proof as explicit product gates.
...
- [x] Reconcile provider identity, publisher, license, dependencies, and package contents against canonical NuGet `.nuspec` metadata rather than trusting only `provider.json`; require an explicit reviewed signer fingerprint when signer policy is configured rather than treating signature integrity alone as publisher authorization.
- [x] Define one versioned executable provider ABI through which an independently built package supplies typed binding/lowering metadata and a runtime-free adapter implementation without compiler-source edits, command-name branches, assembly loading in the compiler, or a second eligibility authority.
- [ ] Expand isolated executable-provider conformance beyond the current deterministic string adapter to prove values, types, cardinality, every stream, errors, cancellation, cleanup, dependency closure, and AOT/runtime-free claims against each package's actual implementation.
...
- [ ] Reuse the executable route for filesystem/HTTP, directory-service, WMI/CIM/CDXML, native/process, and future COM providers where those operations can honestly be runtime-free.
...
Exit gate: **Partial / executable foundation complete.** A separately built provider package is reconciled with canonical NuGet metadata, governed by package/publisher/license/signer policy, locked by exact archive and assembly identity, metadata-validated without loading it into the compiler, and invoked from generated Strict and Hybrid artifacts through the canonical provider route. The gate closes when the executable conformance matrix covers full values/cardinality/streams/errors/cancellation/cleanup/AOT behavior and representative filesystem/HTTP or equivalent provider families. Public feed publication remains a separate release decision.
...
- [ ] Preserve an explicit `AutomationNull` identity before PowerShell's public hosting boundary collapses it to null/no-output. Do not infer the sentinel from formatted output or label an ordinary null as `AutomationNull`.
- [ ] Add a versioned framed Strict-observation protocol for nullable, string/multiline, and culture-sensitive values, and directly instrument sequenced child-process launches/exits. Schema 3 deliberately rejects ambiguous line-based ABI shapes and does not mislabel final mutable `LASTEXITCODE` state as a child-process history.
- [x] Add a minimized executable oracle case and exact observed-host matrix for every promoted semantic feature. Nineteen native `.ps1` resources cover all currently promoted feature families, provenance resolves to real applicable case IDs, and the complete 19-case matrix executes on the pinned Windows PowerShell 5.1, PowerShell 7.4.19, and PowerShell 7.6.5 hosts. The certified Strict-executable observer validates ABI type/cardinality, executes artifacts without PowerShell, and promotes compatible `#requires`, profile-fixed `$PSVersionTable.PSVersion.Major`, bounded compiler-owned dictionary flow, compile-time-safe literal conversion, stable-string interpolation, local function graphs, typed/defaulted parameters, bounded parameter-validation metadata, exact/alias/abbreviated parameter binding, index/member assignment targets, ordered typed catch filters, bounded scalar regex-switch matching, bounded allocated typed-array `ForEach-Object` enumeration with compiler-owned `$_`/`$PSItem`, bounded typed-executable begin/process/end lifecycle invocation, bounded local `Get-Help` Name/Synopsis metadata, typed integral compound arithmetic, comparison operators, and logical operators against the exact PowerShell 7.6.5 pin. The lifecycle slice accepts one explicitly typed stable-scalar `ValueFromPipeline` parameter, a compiler-allocated typed input array, and explicit begin/process/end blocks; other lifecycle shapes and binary-module lifecycle commands remain hosted. The runtime-free help slice accepts one statically named compiled local function and exposes immutable Name/Synopsis strings from the canonical comment-help binder; help discovery, formatting, options, and other properties remain hosted. All 19 cases (100%) now have runtime-free observations. This percentage measures the minimized oracle catalog, not the PowerShell language.
...
Exit gate: **Partial / bounded schema-3 and sixteen runtime-free cases complete.** Named profiles govern target identity, binding/lowering, providers, caches, artifacts, diagnostics, compatible `#requires`, the profile-fixed PowerShell major version, bounded compiler-owned dictionary flow, compile-time-safe literal conversion, stable-string interpolation, local function graphs, typed/defaulted parameters, bounded parameter-validation metadata, exact/alias/abbreviated parameter binding, index/member assignment targets, ordered typed catch filters, bounded scalar regex-switch matching, typed integral compound arithmetic, comparison operators, and logical operators; schema-3 observations integrity-bind exact executable/runtime identity, reject contradictory/unbounded evidence, preserve ordered values/cardinality/streams/errors/state and original property order, and promotion fails closed on unpinned or unexplained results. Every promoted family has a native minimized case executed across the three immutable pinned hosts; those sixteen features now also have artifact-hash-bound Strict-executable evidence, and upstream patch changes produce review-only evidence without changing a pin. The gate closes when explicit `AutomationNull` identity, the framed nullable/string/culture-sensitive protocol, directly observed process effects, and the remaining 3 runtime-free case observations are complete.
...
Exit gate: **Partial.** Bounded WMI/CIM/CDXML, administration-style, native/process, and COM examples are preserved through declared Hybrid routes; one direct typed local CIM query proves an adapter foundation, and the generic executable-provider trust boundary is now available. Generated Strict management-provider execution, signed external binary-module/transitive-wrapper proof, and the remote management operation/failure matrix remain required. Deterministic planning and hosted execution are not presented as native compilation.
...
- [ ] Extend the Strict application packet with dependency, error, stream, resource, startup, and performance evidence on every advertised target.

Exit gate: **Partial / enforced measurement and initial benefit proof complete.** Ten unrelated public modules pass baseline-gated clean-target Hybrid build/import/invocation with all 735 units safely retained or runtime-routed; four Strict programs totaling 18 emitted units, including one four-file/11-method application, execute on `win-x64` and `linux-x64`. The external packet baseline is also enforced, and its opt-in qualification invokes two emitted commands from two unrelated workloads with original/generated parity; both are currently in one scenario family, and one deliberately uses a direct-root surface because full-manifest dependencies are unavailable on the qualification host. The gate closes when a fixed Hybrid benefit packet emits and invokes commands across at least three unrelated scenario families and the Strict application gains the remaining dependency/error/stream/resource/startup/performance matrix.

Frontier snapshot on 2026-08-30: the separate six-workload packet acquired and analyzed 154 source files with zero parser-error files. Profile-compatible `#requires` support raised post-emission coverage to 4/185 units (2.16%) and 4/173 functions (2.31%). Complete-workload execution remains 0/6 because the assessment lane is census-only; a separate opt-in qualification builds two exact acquired modules and proves 2/2 selected emitted commands with original/generated invocation parity without claiming either full workload. The strongest cross-workload gaps remain runtime scope, bound pipeline syntax, member/invoke-member expressions, pipeline lifecycle, missing lowering, parameter types, and common command-provider shapes. Coverage work must address those semantic families generically and then pass the oracle, fallback, artifact, dependency, and target-profile gates before this baseline is refreshed.
...
2. [ ] **Close one-brain correctness:** effective profiles, canonical NuGet/signer trust, the executable provider ABI, shared corpus acquisition, and responsibility splits are complete; finish structured exact-host per-feature oracles and full executable-provider conformance.
...
- [ ] the fixed public benefit lane invokes emitted commands across three unrelated scenario families rather than proving hosted compatibility alone.

The semantic compiler does not have a competing AST/emitter eligibility brain. Profiles are effective, the external-provider route is executable and trust-bound, corpus policy has one owner, and active production files remain below the hard ceiling. The remaining risks are incomplete exact-host per-feature oracle evidence, incomplete provider conformance/families, and too little emitted Hybrid benefit across unrelated workloads. Those gaps must close through the existing owners rather than creating parallel behavior as coverage expands.
...
- [x] the fixed public-corpus and Strict-program packet enforces its baseline, and opt-in external qualification invokes 2/2 emitted commands from unrelated workloads without product-specific compiler paths;
- [ ] the fixed three-family Hybrid benefit packet and the Strict application's dependency/error/stream/resource/performance matrix remain open under Milestone 20;
