...
3. [ ] **Run value-ranked semantic waves:** compatible `#requires`, bounded read-only process/user/culture state, nullable CLR value-member reads, stable scalar stream-message interpolation, terminal `try`/`catch` output, and the first exact typed invoke-member expansion are complete; broad dynamic scope and untyped object members remain intentionally runtime-backed. Constructed `List<T>` now uses normalized open-generic target-reference identities for exact construction, member invocation, and reads, including null `Count` behavior; unrelated generic definitions, generic methods, dynamic receivers, and non-exact overloads still fail closed. Continue with broader pipeline syntax/lifecycle, parameter typing, and common provider shapes. Every change must use parser → binder → bound IR → lowering → backend and the exact profile/oracle path. The external packet remains 8/196 emitted units and 8/183 emitted functions, while invoke-member diagnostics fell from 9 to 7 occurrences and from 6 to 4 affected units; no sole blocker was removed, so semantic breadth and measured workload benefit remain separate claims.
