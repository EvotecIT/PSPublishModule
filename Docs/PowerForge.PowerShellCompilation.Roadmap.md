...
- [ ] Inventory the next grouped batch across bounded `foreach`/enumeration, range-and-match control flow, type/conversion expressions, and local-function closure. These families must be separated by value, enumeration, ETS, stream, and lifetime contracts before implementation; the four remaining `syntax.foreachstatement` sole blockers are not one safe shape.
...
Exit gate: **Partial.** The continuation source implements guarded prefixes, later statement-aligned regions, structured scalar/map/vector/list transfers, retained definite assignment, fresh-local and earlier-region ownership, return-or-fallthrough envelopes, distinct no-value/explicit-null return cardinality, one closed stable-scalar `foreach` array capture, one exact local `ArrayList` factory call, Hybrid-native computed member access, and complete hosted multi-stage pipelines with retained collection mutation. Region admission has no minimum statement count: one-statement and later-region artifacts preserve the same semantic contracts. The unchanged PSSharedGoods `Get-ObjectEnumValues`, `Get-MyIpAddress`, `Remove-DuplicateObjects`, `Split-Array`, `Get-ADEncryptionTypes`, `Get-ADTrustAttributes`, `ConvertTo-ImmutableID`, `Find-DatesPastWeek`, `New-SqlTableMapping`, `Set-XML`, and four local-factory consumers plus CleanupMonster `Get-CloudCacheComputer` supply real structural evidence. General enumeration, general local-call collection closure, runtime-free dynamic object/ETS access, compiled-owned collection mutation, computed method names, null-conditional access, and Strict stateful-module ownership remain open and are sequenced in M24–26. Further promotion depends on correct reusable support and complete-workload evidence; speedup is not an acceptance gate.
...
10. [ ] Inventory the next grouped batch across bounded `foreach`/enumeration, range-and-match control flow, type/conversion expressions, and local-function closure. Select several consumers that share one complete ownership and failure contract before implementing it.
11. [ ] Productize CLR consumption and the source development loop through M27/M28 using the existing ABI, project, diagnostics, and source-map owners.
12. [ ] Run M29's provider/platform/public-package qualification as an independent authorized lane. Include additional detection-script states on a disposable target; do not mutate an ambient machine to complete a checkbox.
13. [ ] Optimize already qualified workloads under M30 with fresh measurements and explicit per-workload budgets.
