...
- [ ] Preserve enough canonical bound/lowered structure for an otherwise-hosted function to select and emit safe typed regions around its hosted remainder. Selection must occur in semantic/bound IR before the final ledger records the shaped result; it may not use the ledger as a planning input or add an AST-to-C# island compiler. Each promoted region must retain the graph's value, stream, error, ordering, source-map, and boundary identity and must fall back as a whole when those contracts cannot be closed.
  ...
  - [x] Add target-compatible type literals as one bounded expression family. `TypeExpressionAst` binds to one `System.Type` value only when the selected target reference surface contains the type, and the backend emits `typeof(...)`. Missing types and .NET 10-only types under net472 fail closed. A direct or transitively hoisted type literal consumed by an invocation inside an observed native `try/catch` remains hosted after the unchanged SID conversions exposed a method-invocation wrapper mismatch; this is a semantic boundary, not a type whitelist exception. This checkpoint reports 168 emitted functions, 40 promoted regions, 70 candidates, and 14 opportunities.
...
15. [ ] Productize CLR consumption and the source development loop through M27/M28 using the existing ABI, project, diagnostics, and source-map owners.
16. [ ] Run M29's provider/platform/public-package qualification as an independent authorized lane. Include additional detection-script states on a disposable target; do not mutate an ambient machine to complete a checkbox.
17. [ ] Optimize already qualified workloads under M30 with fresh measurements and explicit per-workload budgets.
