...
- `dotnet test .\PowerForge.Tests\PowerForge.Tests.csproj -c Release --filter "Category=PowerShellCompilation"` selects 931 compiler tests. On the current integrated candidate the no-build Windows run passed 930 in 6m49s with zero failures and one explicit certificate-gated skip; an exact-SHA Linux/WSL build-and-test run passed the same 930 in 6m28s with zero failures and the Windows-only signing skip. Certificate-backed signing remains a separate live acceptance gate when a suitable Windows identity is configured;
