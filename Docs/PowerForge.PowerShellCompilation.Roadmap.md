...
- [x] package-local canonical compilation evidence re-finalized after packed signing, expressed with portable relative paths, and authenticated by a detached CMS signature in signed archives
- [x] explicit analyzed, emitted, semantic-fallback, shaping-fallback, runtime-routed, omitted, and coverage fields; manifest hooks remain runtime requirements without being miscounted as analyzed source units
...
| 17. Add compiler explainability and reproducible diagnostics | Partial / trace and reproduction foundation | Human and versioned JSON explain output now reports stable per-unit type, capability, dependency, lowering, and artifact decisions; integrity-bound redacted reproduction evidence is emitted, while generated-failure mapping, optional IR snapshots, audit details, compatibility, and retention policy remain open |
...
5. [x] Run external-module, managed-wrapper/provider, native/process, and Windows COM clean-target disposition fixtures.
...
- `dotnet test .\PowerForge.Tests\PowerForge.Tests.csproj -c Release --no-build --no-restore --filter "Category=PowerShellCompilation"` selected the complete compiler trait with the live certificate-backed case enabled and passed all 926 tests with zero failures or skips in 10m09s on the final candidate; the signed ZIP case also passed independently, and the previously omitted 39-test reviewer selection passed 39/39;
...
- the certificate-backed public DSL acceptance signed the exact finalized payload, recomputed canonical manifest and per-file hashes after signing, verified the recorded certificate/signature counts, extracted the ZIP, revalidated Authenticode status, and invoked the delivered command;
...
- finalized payload hashes are captured after checkpoint finalization and checked after lifecycle hooks and before every delivery boundary; checkpoint authority stays machine-local, while signed archives receive refreshed portable post-sign compiler evidence plus their own detached CMS signature;
- canonical compiler diagnostics, decision trace, reproduction evidence, and manifest survive staging and delivery; explicit unit counters distinguish semantic fallback, artifact-shaping fallback, runtime routing, omission, and typed coverage without counting manifest hooks as authored source units;
...
- [x] Preserve the canonical compiler manifest and diagnostics in the delivered payload, then recompute artifact and per-file hashes plus Authenticode status after signing so release evidence describes the bytes actually delivered.
...
- [ ] Document the route for reusable filesystem, HTTP, JSON/CSV, managed-wrapper, directory-service, native, process, and future COM providers without turning command names into compiler intrinsics.
...
- [x] Produce integrity-bound redacted reproduction evidence containing normalized source identities and hashes, compiler/semantic-profile and toolchain identities, target contract, provider contracts, dependency lock, generated source hash, ABI hash, source-map hash, trace hash, and diagnostics hash without copying authored source, secrets, absolute paths, or machine-owned state.
...
- [x] touched non-generated compiler production/test files stay below the 1,000-line hard ceiling; semantic owners remain below the preferred 800-line split point, while the 995-line plan aggregator delegates named responsibilities to focused planning partials;
