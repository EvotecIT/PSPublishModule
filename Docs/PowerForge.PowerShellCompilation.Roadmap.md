...
- [ ] Expand isolated executable-provider conformance beyond the original deterministic information-stream adapter to prove values, types, cardinality, every stream, errors, cancellation, cleanup, dependency closure, and AOT/runtime-free claims against each package's actual implementation. The exact locked executable route now proves string scalar and collection success values, all seven PowerShell stream sinks, terminating adapter errors, and a two-assembly managed dependency closure in both Strict and Hybrid where applicable. Cooperative cancellation, deterministic resource cleanup, non-string value types, and NativeAOT execution of an external adapter remain open; metadata declarations alone do not close them.
...
Exit gate: **Partial / executable foundation expanded.** A separately built provider package is reconciled with canonical NuGet metadata, governed by package/publisher/license/signer policy, locked by exact archive and assembly identity, metadata-validated without loading it into the compiler, and invoked from generated Strict and Hybrid artifacts through the canonical provider route. The route now proves scalar/collection string values, all seven stream sinks, terminating errors, and an exact transitive managed assembly at runtime. The gate closes when executable conformance covers cooperative cancellation, deterministic cleanup, non-string value types, NativeAOT, and representative filesystem/HTTP or equivalent provider families. Public feed publication remains a separate release decision.
