...
- [x] Add a bounded compiler PR gate with then-pinned Windows PowerShell 7.6.5 acquisition, .NET 8 artifact references, contract tests, six Strict programs, and uploaded evidence.
...
- [x] Pin the exact executable host artifact for every promoted profile—patch/build, edition, OS, architecture, culture, feature switches, executable hash, and upstream commit/release identity—instead of a broad version range or executable name. The embedded schema-1 pin set records reviewed Windows x64 evidence for Windows PowerShell 5.1.26100.9168, PowerShell 7.4.20, and PowerShell 7.6.6; the standalone releases also bind the official archive URI/hash and peeled upstream tag commit. The schema-2 runner can require those canonical identities, and the promotion gate uses the immutable catalog by default.
...
- [ ] Preserve enough canonical bound/lowered structure for an otherwise-hosted function to select and emit safe typed regions around its hosted remainder. Selection must occur in semantic/bound IR before the final ledger records the shaped result; it may not use the ledger as a planning input or add an AST-to-C# island compiler. Each promoted region must retain the graph's value, stream, error, ordering, source-map, and boundary identity and must fall back as a whole when those contracts cannot be closed.
  ...
  - [ ] Unlock additional real commands or complete workflows through correct reusable semantic contracts. Existing report and formatting parity, numeric computation, and bounded-counter evidence inform this work; their measured speed does not determine coverage acceptance.
