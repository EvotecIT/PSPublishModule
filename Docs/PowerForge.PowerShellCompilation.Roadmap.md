...
| 19. Qualify real dependency and interop ecosystems | Partial / controlled external qualification | Signed binary wrappers, the complete typed local CIM/MI family, locked native delivery, remote WS-Man/DCOM execution, session reuse/disconnect, bounded enumeration/subscription, authentication failure, and safe failure/cleanup cases are proven; a separately versioned directory provider and controlled target-reboot/reconnect exercise remain open |
...
Exit gate: **Partial only at controlled external boundaries.** The complete typed CIM/MI operation family executes through compiler-generated Strict artifacts without a PowerShell runtime. The release-wired `PowerForge.PowerShell.Provider.Management.Runtime` project emits the same nine-contract package used by acceptance tests, carrying three exact managed assemblies and one parsed, architecture-checked native bridge through reviewed lock, provenance, SBOM, publication disposition, native-import closure, and delivered-closure verification. Local mutation/method/association/subscription, remote WS-Man/DCOM, reusable caller-session ownership, bounded results, invalid credentials, unreachable and missing targets, cancellation, disconnect, and disposable result/session cleanup are observed. WMI v1 and CDXML remain explicit hosted/profile contracts rather than automatic cmdlet translation. Completion still requires a separately versioned executable directory-service provider and an authorized reboot/reconnect exercise on a disposable Windows target; those external gates are not inferred from metadata or replaced by rebooting an ambient runner.
...
5. [ ] **Qualify ecosystems:** the signed external-module/wrapper and executable CIM provider are complete through the same provider and clean-target contracts; finish the executable directory provider and the controlled management-target reboot/reconnect acceptance case.
...
- [ ] Milestone 19's signed external binary-module/managed-wrapper, executable CIM provider, safe remote-management operation/failure matrix, bounded Hybrid WMI/CIM/CDXML, native/process, and Windows COM routes pass; only the executable directory provider and authorized disposable-target reboot/reconnect evidence remain;
