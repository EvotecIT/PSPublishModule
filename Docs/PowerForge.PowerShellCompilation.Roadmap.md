...
17. [ ] Advance M25 complete-command coverage through the existing output, command-region, statement-error, and local-call owners. Select complete workflows with multiple output-producing calls and retained pipeline stages; compare streams, partial failure, stopping, cancellation, and cleanup on both supported hosts.
18. [ ] Productize CLR consumption and the source development loop through M27/M28 using the existing ABI, project, diagnostics, and source-map owners.
19. [ ] Run M29's provider/platform/public-package qualification as an independent authorized lane. Include additional detection-script states on a disposable target; do not mutate an ambient machine to complete a checkbox.
20. [ ] Optimize already qualified workloads under M30 with fresh measurements and explicit per-workload budgets.
