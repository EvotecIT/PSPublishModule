...
| 18. Establish versioned semantic oracles | Partial / exact-host enforcement foundation | Named profiles govern target identity and compilation; observations carry integrity-bound executable/runtime identity and promotion rejects missing pins or unexplained differences; the immutable per-profile pin set and minimized per-feature structured matrix remain open |
...
Exit gate: **Partial / exact-host enforcement foundation complete.** Named profiles govern target identity, binding/lowering, providers, caches, artifacts, diagnostics, and compatible `#requires`; schema-2 observations integrity-bind exact executable/runtime identity, preserve ordered structured streams/errors and original property order, and promotion fails closed on unpinned or unexplained results. The gate closes when the immutable host-pin set, complete null/cardinality/encoding/process-effect envelope, minimized per-feature observed-host matrix, and automated upstream-change lane are complete.
