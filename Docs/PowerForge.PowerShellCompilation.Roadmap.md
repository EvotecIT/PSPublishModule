...
| Overall architecture completion gate | Partial / Milestone 16 | The compiler core, bounded delivery/optimization, and explainability gates are complete; external-provider SDK trust remains open |
...
- canonical compiler diagnostics, decision trace, reproduction evidence, and manifest survive staging and delivery; manifest schema 10 embeds one immutable unit-disposition ledger whose metrics are consumed by coverage, explain, census, reproduction, and boundary evidence without counting manifest hooks or hosted lifecycle wrappers as typed authored emission;
