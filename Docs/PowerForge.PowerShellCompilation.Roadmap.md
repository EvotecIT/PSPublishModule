...
- [x] Pass the complete compiler category: 881/881 tests on .NET 10, including generated `net10.0`/PowerShell 7.6 and `net472`/Windows PowerShell 5.1 artifact cases.
- [x] Pass all six fixed Strict corpus programs as `net10.0` `win-x64` artifacts.
- [x] Promote one second statement-aligned scalar Hybrid region around retained hosted work when canonical graph inputs and outputs exactly match its established-local transfer contracts. Preserve output/error order, stopping, and reuse on both supported hosts.
...
| 23. Compile module-scale state and regions without a second runtime | Partial | Bounded live state, scalar prefixes, terminal/complete-body regions, and one second detached scalar region are implemented; wider transfer, general multiple regions, and Strict stateful-module closure continue through M24–26 |
...
| [26. Stateful modules and multiple Hybrid regions](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-26--compile-stateful-modules-and-multiple-hybrid-regions) | In progress / closes M23 | One bounded two-region scalar shape shares the real Hybrid state owner; real-workflow application, wider/general regions, and remaining Strict lifetime closure stay open |
...
- [ ] Preserve enough canonical bound/lowered structure for an otherwise-hosted function to select and emit safe typed regions around its hosted remainder. Selection must occur in semantic/bound IR before the final ledger records the shaped result; it may not use the ledger as a planning input or add an AST-to-C# island compiler. Each promoted region must retain the graph's value, stream, error, ordering, source-map, and boundary identity and must fall back as a whole when those contracts cannot be closed.
  ...
  - [x] Unlock additional real commands or complete workflows through correct reusable semantic contracts. The unchanged pinned PSSharedGoods IP conversion command executes a newly admitted one-statement scalar prefix on both supported hosts, and the full pinned-module census exposes 14 promoted regions instead of two. Existing report and formatting parity, numeric computation, and bounded-counter evidence inform further work; measured speed does not determine coverage acceptance.
  - [ ] Extend multiple-region selection, error envelopes, local-call closure, and wider transfer only with explicit canonical contracts. Do not infer these contracts from generated C# or split a statement.
...
Exit gate: **Partial.** The continuation source implements bounded scalar prefixes with a retained continuation, one later detached established-scalar region, post-bind opportunity discovery, complete-body/terminal-region promotion, bounded numeric representation, and integer-range proofs through the existing semantic pipeline. Region admission has no minimum statement count: one-statement prefix and later-region artifacts preserve the same semantic contracts. Retained functions remain runtime-routed in coverage. Original/generated parity passed for the recorded command and artifact cases. Real-workflow same-function multiple-region use, general additional regions, error envelopes, local-call closure, dynamic state, wider collection/member/index behavior, and Strict stateful-module ownership remain open and are sequenced in M24–26. Further promotion depends on correct reusable support and complete-workload evidence; speedup is not an acceptance gate.
