...
1. [ ] Freeze M29's 196/282 same-input baseline, classify its 86 retained/unsupported functions, and qualify the eight recently emitted functions without counting them as new M29 gains. Use three previously blocked complete workflows as the first checkpoint, then continue across four recurring blocker families until at least 30 additional functions execute correctly and two modules plus one script/application workflow pass end to end. Keep Hybrid fallback, Strict rejection, and co-blockers explicit.
