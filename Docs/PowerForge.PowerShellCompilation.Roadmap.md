...
- [ ] Preserve enough canonical bound/lowered structure for an otherwise-hosted function to select and emit safe typed regions around its hosted remainder. Selection must occur in semantic/bound IR before the final ledger records the shaped result; it may not use the ledger as a planning input or add an AST-to-C# island compiler. Each promoted region must retain the graph's value, stream, error, ordering, source-map, and boundary identity and must fall back as a whole when those contracts cannot be closed.
  ...
  - [x] Unlock additional real commands or complete workflows through correct reusable semantic contracts. The unchanged pinned PSSharedGoods IP conversion command executes a one-statement scalar prefix, and `Get-ObjectEnumValues` executes a guarded Hashtable initializer plus an exact prefix-owned terminal passthrough on both supported hosts. The full pinned-module census exposes 16 promoted regions instead of two. Existing report and formatting parity, numeric computation, and bounded-counter evidence inform further work; measured speed does not determine coverage acceptance.
  - [ ] Extend additional-region selection, error envelopes, local-call closure, and wider transfer only with explicit canonical contracts. Arrays/lists and compiled collection mutation remain outside the current transfer policy. Do not infer these contracts from generated C# or split a statement.
...
Exit gate: **Partial.** The continuation source implements guarded scalar/Hashtable prefixes with a retained continuation, one later detached established-scalar region, an exact read-only terminal Hashtable passthrough, post-bind opportunity discovery, complete-body/terminal-region promotion, bounded numeric representation, and integer-range proofs through the existing semantic pipeline. Region admission has no minimum statement count: one-statement prefix and later-region artifacts preserve the same semantic contracts. The unchanged PSSharedGoods `Get-ObjectEnumValues` function now supplies real same-function two-region evidence on both supported hosts while the retained parent module remains the state owner. General additional regions, error envelopes, local-call closure, dynamic state, arrays/lists, compiled collection mutation, wider collection/member/index behavior, and Strict stateful-module ownership remain open and are sequenced in M24–26. Further promotion depends on correct reusable support and complete-workload evidence; speedup is not an acceptance gate.
...
2. [ ] Continue [M24](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-24--preserve-values-collections-and-error-continuation) with a complete collection/enumeration-failure and wider-transfer contract. The current real-module scan exposes arrays, lists, dictionaries, and fresh typed locals as the co-blockers; rank complete-workflow unlocks rather than diagnostic frequency.
3. [ ] Use M25 and M26 to apply the bounded second-region ABI to an unchanged real workflow, then extend complete pipelines and module-scale state/regions. Qualify real DLL/EXE behavior, including failures and continuation; speed is not an admission gate.
