...
| 18. Establish versioned semantic oracles | Partial / effective-profile foundation | Named profiles govern target identity and compilation, and compatible requirements have cross-host oracle proof; exact-host artifact identity and per-feature structured coverage remain open |
...
Exit gate: **Partial / effective-profile and harness foundation complete.** Named profiles govern target identity, binding/lowering, providers, caches, artifacts, diagnostics, and compatible `#requires`; provenance schemas, black-box cross-host cases, and the prohibition on PowerShell internals are present. The gate closes when exact host artifacts are pinned, every promoted family has structured per-feature oracle evidence, and upstream change review is automated without silently moving behavior.
