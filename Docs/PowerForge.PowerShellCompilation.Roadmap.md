...
This is the current coverage milestone. Hybrid must preserve one live PowerShell module and accelerate proven code around it; Strict must not inherit credit from that hosted state.
...
- [ ] Preserve enough canonical bound/lowered structure for an otherwise-hosted function to select and emit safe typed regions around its hosted remainder. Selection must occur in semantic/bound IR before the final ledger records the shaped result; it may not use the ledger as a planning input or add an AST-to-C# island compiler. Each promoted region must retain the graph's value, stream, error, ordering, source-map, and boundary identity and must fall back as a whole when those contracts cannot be closed.
  ...
  - [x] Extend canonical candidate discovery beyond an always-returning terminal suffix before extending execution. For every binder-rejected Hybrid function that still contains successfully bound top-level statements, the binder now records maximal statement-aligned runs and a dedicated analysis path reuses the ordinary optimizer, semantic passes, lowerer, and canonical region-graph builder without invoking the backend. The public evidence records exact source identity, statement/span size, continuation shape, separate predecessor/input and continuation/output completeness, typed live inputs, conservatively widened live outputs when an unbound continuation could observe any mutation, streams, errors, ordering, direct local calls with transitive bound-call closure, and state-boundary facts. Evidence is replaced per function on every fixed-point binding attempt, sequence reachability stops at the first terminating statement, and optimizer-flattened regions are mapped back only when the complete overlapping authored statement remains typed; an authored statement containing a mixed boundary is omitted until a substatement identity contract exists. Strict receives no opportunity evidence. Final compilation, promoted-region counts, generated C#, and retained PowerShell are unchanged, and legacy census JSON without the additive field still reads as an empty opportunity set.
    ...
    The benchmark plan is fixed before execution work: select a compiler-described shape with complete stable-scalar transfers in at least two unrelated families; preserve one source/semantic checksum and exact stream/error transcript; compare retained Hybrid against the candidate region path in counterbalanced warm/cold samples; record wall time, allocations, crossings, transferred values, artifact/build cost, and p50/p95; and require the measured gain to exceed the isolated boundary overhead in both real commands. No source or workload identity may influence selection.
  - [ ] Broaden the opportunity census beyond binder-rejected functions to functions retained only by later semantic, graph, cmdlet-shaping, or artifact decisions. Preserve the canonical bound/lowered owner and reconcile final disposition without allowing census or ledger output to become a planning input. Until this closes, zero opportunities means only that this first binder-rejection lane found none.
  - [ ] Extend selection to useful non-terminal and multiple regions only after explicit continuation, error-envelope, stream-order, local-call-closure, live-value transfer, and exception/trap semantics are represented in canonical IR. Do not infer these contracts from generated C# or split a statement.
...
- [ ] Execute at least one newly unlocked command from unrelated module families and show semantic parity plus a measured benefit that amortizes its state/region crossings before promoting this milestone beyond preview.

Exit gate: **Partial / Current.** Direct Hybrid reads/writes, authored-cast member/index reads, the canonical coarse graph for already-emitted methods, the first fail-closed terminal-region promotion, machine-readable candidate decisions, and binder-rejection opportunity discovery are implemented through one state and semantic/lowered-IR owner. A retained function can delegate only the proven scalar terminal suffix to generated CLR and remains honestly counted as runtime-routed rather than as an emitted function. The new public/external frontiers finally expose larger and repeated non-terminal work, but they also show that the tempting multi-statement shapes cross incomplete unbound continuations, multiple locals, `System.Object`, or modeled CLR errors. General object/local transfer would therefore add semantic and ABI risk before a complete, measurable target exists. Broader post-bind opportunity discovery, non-terminal/multiple execution, exact continuation/output consumption, error envelopes, stream order, sound live-local transfer, executable local-call closure, untyped/dynamic state, member/index mutation, Strict stateful-module ownership, external-workload execution, and measured multi-family benefit remain open and should precede another large command-provider expansion.

## Next implementation order

The canonical semantic pipeline, profile, provider, oracle, and bounded benefit gates are established. Continue in this order so semantic breadth, ecosystem qualification, platform promotion, and release evidence do not hide one another:
...
- [ ] Milestone 23 must progress from its live Hybrid state and first terminal-region slice to general continuation/error/transfer-safe region compilation, Strict stateful-module closure, and measured multi-family benefit without adding a second state or eligibility owner.
