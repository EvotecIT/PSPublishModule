...
- [x] Pass the complete compiler gate with 935/935 tests and all six Strict `net10.0` `win-x64` programs. Build `PowerForge.PowerShell` for both `net10.0` and `net472` with zero warnings and errors.
...
- [ ] Select the next bounded optional-scalar or typed-array family across multiple real workflows; do not create a second collection runtime or infer eligibility from generated C#.
...
6. [ ] Select the next bounded optional-scalar or typed-array M24 family, then extend M25/M26 local-call closure and module-scale regions; speed is not an admission gate.
7. [ ] Productize CLR consumption and the source development loop through M27/M28 using the existing ABI, project, diagnostics, and source-map owners.
8. [ ] Run M29's provider/platform/public-package qualification as an independent authorized lane. Include additional detection-script states on a disposable target; do not mutate an ambient machine to complete a checkbox.
9. [ ] Optimize already qualified workloads under M30 with fresh measurements and explicit per-workload budgets.
