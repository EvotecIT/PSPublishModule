...
- [ ] Add a minimized executable oracle case and exact observed-host matrix for every promoted semantic feature. Nineteen native `.ps1` resources cover all currently promoted feature families, provenance resolves to real applicable case IDs, and the complete 19-case matrix executes on the pinned Windows PowerShell 5.1, PowerShell 7.4.19, and PowerShell 7.6.5 hosts. The new certified Strict-executable observer validates ABI type/cardinality, executes the artifact without PowerShell, and promotes the compatible `#requires` case against the exact PowerShell 7.6.5 pin. Eighteen cases still require runtime-free observations through their value-ranked semantic owners.
...
Exit gate: **Partial / bounded schema-3 and first runtime-free case complete.** Named profiles govern target identity, binding/lowering, providers, caches, artifacts, diagnostics, and compatible `#requires`; schema-3 observations integrity-bind exact executable/runtime identity, reject contradictory/unbounded evidence, preserve ordered values/cardinality/streams/errors/state and original property order, and promotion fails closed on unpinned or unexplained results. Every promoted family has a native minimized case executed across the three immutable pinned hosts, compatible `#requires` now also has artifact-hash-bound Strict-executable evidence, and upstream patch changes produce review-only evidence without changing a pin. The gate closes when explicit `AutomationNull` identity, the framed nullable/string/culture-sensitive protocol, directly observed process effects, and the remaining 18 runtime-free case observations are complete.
