...
- [x] Carry exact read/write authority, variable names, and module-state read/write-site counts from lowered IR into generated-method metadata, ABI host parameters, IR capabilities, the final disposition ledger, boundary counts, explain output, and census. Ledger schema 2, IR snapshot schema 2, and explanation schema 3 expose the direction explicitly; the compatibility constructor and reviewed JSON round trip prevent the ledger change from silently breaking existing consumers. The pre-ledger public explanation overload is now explicitly obsolete, stays on explanation schema 2/semantic compatibility 1, and fails closed for module-state methods instead of emitting false schema-3 directionality; all current final artifact and CLI paths consume the immutable ledger overload. The ledger labels this exact hosted route `HostedModuleState` without relabeling ordinary invocation-state capture. Metadata collectors and RHS host-requirement discovery share canonical semantic/lowered-tree enumerators rather than rediscovering syntax.
...
- [ ] Partition otherwise-hosted functions into coarse compiled regions only after a canonical region graph records inputs, outputs, mutations, streams, errors, ordering, and boundary cost. Region selection must consume semantic IR and the final ledger; it may not add an AST-to-C# island compiler.
...
Exit gate: **Partial / Current.** Direct Hybrid reads/writes and authored-cast member/index reads are implemented and externally measured through one retained state owner. They improve typed reach while honestly remaining PowerShell-runtime-routed. Untyped/dynamic state, member/index mutation, coarse region compilation, Strict stateful-module ownership, and measured multi-family benefit remain open and should precede another large command-provider expansion.
...
- [ ] Milestone 23 must progress from its live Hybrid read/direct-write/authored-cast access slices to coarse region compilation, Strict stateful-module closure, and measured multi-family benefit without adding a second state or eligibility owner.
