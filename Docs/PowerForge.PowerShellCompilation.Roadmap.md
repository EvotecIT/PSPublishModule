...
- [ ] Preserve enough canonical bound/lowered structure for an otherwise-hosted function to select and emit safe typed regions around its hosted remainder. Selection must occur in semantic/bound IR before the final ledger records the shaped result; it may not use the ledger as a planning input or add an AST-to-C# island compiler. Each promoted region must retain the graph's value, stream, error, ordering, source-map, and boundary identity and must fall back as a whole when those contracts cannot be closed.
  ...
  - [ ] Demonstrate useful computation beyond constant-heavy prefixes before promoting the continuation milestone. Two unrelated complete commands passed output parity, but report generation tied and diagnostic formatting slowed by 11–13%; see the assessment for measured results.
...
Exit gate: **Partial / Current.** The candidate implements bounded scalar prefixes with a retained continuation, post-bind opportunity discovery, and terminal-region promotion through the existing semantic pipeline. Retained functions remain runtime-routed in coverage. Original/generated parity passed in two unrelated complete commands, but measured performance failed the benefit gate. Wider computation, multiple regions, error envelopes, local-call closure, dynamic state, member/index mutation, and Strict stateful-module ownership remain open. No measured multi-family speedup or stable continuation promotion is claimed.
...
2. [ ] Continue Milestone 23 from the implemented scalar continuation to useful computational regions. Prove measured benefit in unrelated real commands before adding wider state, pipelines, or providers.
