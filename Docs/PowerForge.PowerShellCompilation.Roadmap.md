...
- [x] Pass the complete compiler gate with 956/956 tests and all six Strict `net10.0` `win-x64` programs. Build `PowerForge.PowerShell` for both `net10.0` and `net472` with zero warnings and errors.
...
- [x] Select the next bounded optional-scalar or typed-array family across multiple real workflows. Schema-4 `ClosedValueAlternative` admits one exact two-branch fresh-local initializer whose alternatives are a supported stable scalar and its matching one-dimensional stable-scalar array. Nine unchanged PSSharedGoods `Get-Computer*` workflows share the contract; the helper returns one atomic envelope and retained PowerShell restores the selected authored constraint. Wider object/ETS, multidimensional, mismatched, output-producing, previously owned, or compiled-mutating shapes remain fail-closed.
- [ ] Select the next local-call collection-closure family only where several unchanged workflows share the same value, cardinality, ownership, enumeration, error, and continuation contract; do not infer closure from generated C# or add a second collection runtime.
...
Exit gate: **Partial.** The continuation source implements guarded prefixes, later statement-aligned regions, structured scalar/map/vector/list transfers, retained definite assignment, fresh-local and earlier-region ownership, return-or-fallthrough envelopes, distinct no-value/explicit-null return cardinality, one closed stable-scalar `foreach` array capture, post-bind opportunity discovery, complete-body/terminal-region promotion, bounded numeric representation, and integer-range proofs through the existing semantic pipeline. Region admission has no minimum statement count: one-statement and later-region artifacts preserve the same semantic contracts. The unchanged PSSharedGoods `Get-ObjectEnumValues`, `Split-Array`, `Get-ADEncryptionTypes`, `Get-ADTrustAttributes`, `ConvertTo-ImmutableID`, and `Find-DatesPastWeek` functions and CleanupMonster `Get-CloudCacheComputer` supply real dual-host evidence while the retained parent module remains the state and downstream-enumeration owner. General enumeration, local-call collection closure, dynamic state, compiled collection mutation, wider collection/member/index behavior, and Strict stateful-module ownership remain open and are sequenced in M24–26. Further promotion depends on correct reusable support and complete-workload evidence; speedup is not an acceptance gate.
...
6. [x] Select and implement a reusable typed-array family. Schema-4 `ClosedValueAlternative` carries one exact stable scalar versus matching one-dimensional stable-scalar array through a compiler-owned envelope and restores the authored constraint in retained PowerShell; nine unchanged PSSharedGoods workflows share the contract.
7. [ ] Extend M25/M26 with a bounded local-call collection-closure family and module-scale regions only when several unchanged workflows share the complete value, ownership, enumeration, error, and continuation proof; speed is not an admission gate.
8. [ ] Productize CLR consumption and the source development loop through M27/M28 using the existing ABI, project, diagnostics, and source-map owners.
9. [ ] Run M29's provider/platform/public-package qualification as an independent authorized lane. Include additional detection-script states on a disposable target; do not mutate an ambient machine to complete a checkbox.
10. [ ] Optimize already qualified workloads under M30 with fresh measurements and explicit per-workload budgets.
