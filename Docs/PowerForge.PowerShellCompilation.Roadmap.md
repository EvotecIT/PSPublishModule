...
The [next major milestones, 24–30](PowerForge.PowerShellCompilation.NextMilestones.md), define the active delivery tranche: values/collections, complete pipelines, stateful modules, practical CLR libraries, the development loop, distribution/platform qualification, and finally measured acceleration. M24, M25, M26, and M27 are complete; M28 is the next dependency-ordered product goal. The architecture history below remains their foundation.

## Product north star

The current implementation priority is **correctness, preservation of authored behavior, and useful workload coverage**. New support must preserve the program's parameter binding, values and types, output shape, scope, ordering, errors, and continuation behavior on its supported hosts. Select reusable contracts that unlock real scripts and modules; do not choose work primarily because a small helper is easy to accelerate. Runtime retention is an explicit compatibility outcome, not compiled-coverage credit. Speedups and boundary-amortization targets belong to a later optimization phase and are not acceptance gates for new semantic support. Existing benchmark results remain evidence about their measured workloads.
...
- [x] Re-audit the pinned 180/282 Hybrid frontier before broadening M24. The repeated blockers cluster around statement-valued `[pscustomobject]` properties (15 occurrences), native value collections with control flow (14), and dictionary values (10), with overlap; `syntax.unsupported` alone is too coarse to predict coverage. Original PowerShell 7.6 and Windows PowerShell 5.1 probes confirm zero-versus-one-record empty/explicit-null property values. The [M24 packet](PowerForge.PowerShellCompilation.NextMilestones.md) specifies one shared capture owner and dual-host acceptance matrix. This is design/evidence only, not a new emitted-function or Strict claim.
- [x] Route authored `-split`/`-csplit` through the existing Hybrid native operator and index owners. Unchanged `ConvertFrom-X500Address` now emits a complete method, and a synthetic split/index family checks scalar, null, collection, invalid-pattern, cardinality, and error behavior against original execution on both supported hosts. The pinned census rises from 179 to 180/282 emitted functions without losing any of the 38 promoted regions or introducing an input failure. The complete compiler gate passed 1,036/1,036 tests with no failures or skips and built/executed all six Strict `net10.0` `win-x64` programs. The `net472` Release build passed with zero warnings/errors. This is not a new Strict or complete-M24 claim.
- [x] Close the M24 direct map-key/atomic-map-output Hybrid slice through the existing native loop and dictionary owners. The selected PowerShell host owns loop enumeration; bound output analysis qualifies compiler-created ordinary/ordered map literals while preserving borrowed native references; lowering carries host family to the C# map constructor. Unchanged `Get-ProtocolDefaults` now emits a complete method and passes original/generated known, unknown, case-varied, 26-record list, repetition, and reimport checks on PowerShell 7.6/net10 and Windows PowerShell 5.1/net472. Synthetic probes cover culture-sensitive keys, mutation, borrowed identity, and collected cardinality. Non-native map escape remains hosted, and deeper array/ETS contracts are not implied.
- [x] Refresh the pinned full-module census: 179/282 emitted functions across 284 source files and 286 units, 38 promoted regions, 31 retained region candidates, 11 analysis opportunities, and zero input failures. The previous 176-function/39-region checkpoint is historical; one former region is now in a complete method. The complete compiler gate passed 1,032/1,032 tests with no failures or skips and built/executed all six Strict `net10.0` `win-x64` programs. The `net472` Release build passes with zero warnings/errors, and independent read-only review found no actionable issue.

### Earlier continuation checkpoint: 2026-09-19

- [x] Admit direct object-parameter `foreach` through the existing Hybrid host-enumeration owner. Three unchanged PSSharedGoods workflows become complete emitted methods; `Get-ProtocolDefaults` keeps its hosted map-key loop and one promoted prefix.
- [x] Compare unchanged workflows and an injected lazy enumerator on PowerShell 7.6/net10 and Windows PowerShell 5.1/net472. The independent read-only review found no actionable issue; its enumerator-failure proof gap is closed by the added dual-host differential test.
- [x] Pass the complete compiler gate (1,014/1,014, no skips), six Strict `net10.0` `win-x64` programs, the final five focused Release cases, and the `net472` Release build with zero warnings and errors. The pinned module census is 174 emitted functions, 39 promoted regions, 69 candidates, and 12 opportunities with no input failures.

### Continuation checkpoint: 2026-09-18

- [x] Add one closed return-or-fallthrough envelope for statement-aligned conditional returns of parameter-borrowed values. The contract flows through the canonical parser/binder, bound IR, analysis, lowering, generated C#, runtime composition, manifest/census evidence, and JSON round-trip.
- [x] Keep the observable return boundary in retained PowerShell. It owns one-level enumeration, partial success, error-action continuation, stopping, and enumerator disposal instead of moving those behaviors into a generated CLR helper.
- [x] Qualify typed vectors, `System.Array`, `ArrayList`, `List<object>`, atomic Hashtable output, and explicit comma/no-enumerate output on `net10.0`/PowerShell 7.6 and `net472`/Windows PowerShell 5.1. Preserve reuse and input collection state after downstream stopping.
- [x] Promote the unchanged PSSharedGoods `Split-Array` early array return as its third region. The full pinned module reported 47 promoted regions, 70 candidates, and 19 analysis-only opportunities across 284 source files, 286 units, and 282 functions after the additional `foreach` capture slice.
- [x] Retain direct object/ETS returns, arbitrary enumerable contracts, unproved local collection ownership, and compiled collection mutation. Existing injected-enumerator tests remain the executable proof for `MoveNext`, `Current`, disposal, partial records, and continued-statement behavior at the retained PowerShell boundary.
- [x] Pass the complete compiler gate with 988/988 tests and all six Strict `net10.0` `win-x64` programs. The first post-suite Strict attempt materialized only two executable artifacts; a fresh isolated no-cache workspace built and executed 6/6, separating that transient artifact condition from semantic results. Build `PowerForge.PowerShell` for both `net10.0` and `net472` with zero warnings and errors.
- [x] Add one bounded array/foreach transformation through the same ownership and retained-enumeration contracts. A closed non-nullable value-type scalar loop can fill a fresh exactly typed stable-scalar vector in generated code; retained PowerShell performs downstream enumeration. The unchanged `Get-ADEncryptionTypes` and `Get-ADTrustAttributes` workflows pass on both supported hosts, while object/ETS elements, arbitrary enumerators, explicit output shaping, and terminating control flow remain fail-closed.
- [x] Continue M24 with one return-cardinality family. Schema-3 `NoValue` and `NullValue` contracts distinguish a bare return's zero records from `return $null`'s one record in terminal regions and return-or-fallthrough envelopes. Four authored workflow shapes and unchanged `Find-DatesPastWeek`/`Get-CloudCacheComputer` explicit-null paths pass on both supported hosts. The unchanged `ConvertTo-ImmutableID` terminal return raises the pinned module census to 48 promoted regions; candidate and opportunity counts remain 70 and 19.
- [x] Select the next bounded optional-scalar or typed-array family across multiple real workflows. `ClosedValueAlternative`, introduced in schema 4 and carried by current schema-6 transfer evidence, admits one exact two-branch fresh-local initializer whose alternatives are a supported stable scalar and its matching one-dimensional stable-scalar array. Nine unchanged PSSharedGoods `Get-Computer*` workflows share the contract; the helper returns one atomic envelope and retained PowerShell restores the selected authored constraint. Wider object/ETS, multidimensional, mismatched, output-producing, previously owned, or compiled-mutating shapes remain fail-closed.
- [x] Add schema-5 local-call evidence for one exact zero-parameter helper that creates a fresh `ArrayList` and returns that reference as one comma/no-enumerate record. The canonical binder resolves the call; immutable IR carries its zero-argument, result-type, cardinality, ownership, enumeration, and mutation contract; region lowering creates the same fresh list directly without depending on the helper's public success-stream ABI. Dynamic, unresolved, module-qualified, recursive, state-capturing, enumerating, and otherwise unproved calls remain retained.
- [x] Apply the contract without source-name policy to unchanged PSSharedGoods `Get-ObjectPropertiesAdvanced`, `New-SqlQuery`, `New-SqlQueryAlterTable`, and `New-SqlQueryCreateTable`. The pinned census advances from 48 to 51 promoted regions and from 70 to 73 candidates while retaining 141/282 emitted functions and 19 analysis-only opportunities across 284 source files and 286 units. A synthetic artifact matrix on both supported hosts proves atomic output, exact `ArrayList` type, fresh identity, retained mutation, empty error streams, downstream stop/reuse, and remove/reimport.
- [x] Add computed instance/static member reads and computed instance-member assignment through the existing Hybrid native-function owner. Immutable IR carries receiver, name, static target, effects, and required host capabilities; PowerShell's runtime binder owns adapter/ETS and error semantics. A dual-host matrix proves nested member/index access, evaluation order and exactly-once behavior, aliasing, output/errors, stopping, repetition, and remove/reimport. Strict and runtime-free transfer remain closed.
- [x] Apply the family without source-name policy to the complete pinned module. Thirteen unchanged functions move from fallback to emitted with zero emitted-function loss; `New-SqlTableMapping` and `Set-XML` are direct representatives, while `Send-Email` clears this blocker but remains retained for an independent dictionary-escape contract. Two former partial regions become complete functions, so the census moves from 141 to 154 emitted functions and from 51 to 49 promoted regions while candidates remain 73 and opportunities move from 19 to 18.
- [x] Carry retained collection mutation and complete pipeline flow through one existing owner. A complete authored multi-stage pipeline now selects Hybrid native-function binding and one native command region, including when its commands are known intrinsics. PowerShell keeps command resolution, parameter binding, script-block scope, streams, lifecycle, error/stopping, aliases, and mutation authority; generated code keeps the surrounding supported statements. Explicit terminal success-stream sinks such as `Out-Null` stay fail-closed. Strict/runtime-free compilation remains closed.
- [x] Apply the rule without source-name policy to unchanged `Get-MyIpAddress` and `Remove-DuplicateObjects`, which move from fallback to emitted native-bound CLR bodies. `Get-ObjectEnumValues` deliberately retains its two narrower typed regions around hosted mutation. One earlier promoted region is absorbed, producing 156 emitted functions, 48 promoted regions, 73 candidates, and 17 analysis-only opportunities. Dual-host differential tests execute the enum and duplicate-removal workflows plus a synthetic lifecycle/mutation matrix; the DNS workflow is structurally qualified without network access.
- [x] Inventory bounded `foreach`/enumeration, range-and-match control flow, type/conversion expressions, and local-function closure together. The selected local-function slice reuses the exact closed fresh-`ArrayList` projection and native assignment owner: three unchanged SQL consumers become complete methods without admitting general local calls. The census is now 159 emitted functions, 45 promoted regions, 73 candidates, and 17 opportunities. The four remaining `syntax.foreachstatement` sole blockers are still not one safe shape.
- [ ] Take the range-and-match frontier as the next grouped control-flow tranche. Implement distinct canonical contracts in the existing operator binder, bound/lowered IR, and backend: three range-only and three match-only sole blockers plus one shared co-blocked workflow form a seven-function ceiling. Range qualification must preserve inclusive ascending/descending order, endpoint conversion/overflow, single evaluation, slicing cardinality, out-of-range/null behavior, and errors. Match qualification must preserve scalar/collection behavior, string and regex-pattern conversion, case/culture/options, invalid patterns, `$Matches`, errors, and stopping.
...
- [x] Pass the complete compiler category: 885/885 tests on .NET 10, including generated `net10.0`/PowerShell 7.6 and `net472`/Windows PowerShell 5.1 artifact cases.
- [x] Pass all six fixed Strict corpus programs as `net10.0` `win-x64` artifacts.
- [x] Promote one second statement-aligned Hybrid region around retained hosted work when canonical graph inputs and outputs exactly match its established-local or guarded-prefix transfer contracts. Preserve output/error order, stopping, and reuse on both supported hosts.
- [x] Apply that shape to unchanged PSSharedGoods `Get-ObjectEnumValues`: a guarded compiled empty-Hashtable prefix, retained mutation of the same object, and an exact compiled terminal passthrough preserve output, errors, explicit cancellation, identity, state, reuse, and removal/reimport on both supported hosts. The full pinned-module census reports 16 promoted regions.
...
- [x] Qualify the complete unchanged LSA detection script as a Hybrid EXE on a controlled Windows target in its enabled state; source and executable output/exit matched and registry state was unchanged. Disabled, absent-value, and failure paths remain unqualified.
...
- [x] bounded runtime-state intrinsics, including live parent `$script:` reads, direct writes, and authored-cast member/index reads for Hybrid binary modules without copying that state into generated C# statics
...
- [x] one canonical lowered-region graph for every semantically emitted method, with typed/hosted/mixed execution, value inputs/outputs, mutations, streams, errors, authored ordering, stable identity, and deterministic static boundary cost carried through executable and module shaping into the ledger and explain output
- [x] one fail-closed Hybrid terminal-region promotion path that reuses canonical binding, optimization, analysis, lowering, region graphs, and C# emission while keeping the containing function retained and runtime-routed
- [x] machine-readable terminal-region candidate decisions from that same path, including exact source identity, stable rejection reasons, final-selection reconciliation, census output, and a cross-workload retained-decision frontier without emission credit
...
| 23. Compile module-scale state and regions without a second runtime | Complete through M26 | Live Hybrid state, guarded prefixes, multiple non-overlapping additional regions, schema-6 transfer contracts, real module workflows, and independent Strict stateful-library lifetime are qualified; unproved dynamic/borrowed/provider behavior remains outside the accepted subset |
| [24. Values, collections, and error continuation](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-24--preserve-values-collections-and-error-continuation) | Complete | Numeric promotion, cardinality, fresh-vector ownership, enumeration failure, cleanup, and continuation have dual-host workflow proof |
| [25. Complete pipelines and advanced functions](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-25--compile-complete-pipelines-and-advanced-functions) | Complete | Whole commands preserve every record, local-call output, binding, streams, stop, cancellation, and cleanup through admitted owners |
| [26. Stateful modules and multiple Hybrid regions](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-26--compile-stateful-modules-and-multiple-hybrid-regions) | Complete / closes M23 | Hybrid keeps one state owner while selecting every independently safe non-overlapping region; M24 value/local-call contracts compose with those regions, and Strict stateful libraries have an independent runtime-free lifetime owner |
| [27. Practical CLR library consumption](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-27--make-compiled-clr-libraries-practical-to-consume) | Complete | Static and stateful normal C# consumers restore runtime-free packages from local feeds; deterministic package closure, provider execution, XML docs/symbols, and fail-closed ABI compatibility are qualified on `net10.0` and `net472` |
...
- canonical compiler diagnostics, decision trace, reproduction evidence, and manifest survive staging and delivery; manifest schema 13 embeds one immutable unit-disposition ledger whose metrics are consumed by coverage, explain, census, reproduction, and boundary evidence without counting manifest hooks, hosted lifecycle wrappers, or promoted sub-function regions as fully emitted authored units, and binds the exact target-specific NuGet closure lock consumed by a project build;
...
- at that snapshot, the Hybrid packet analyzed 735 authored units, emitted none, and retained or runtime-routed all 735 without omission or rejection; the Strict packet analyzed, bound, and emitted all 18 authored units without fallback, including 11 methods in a realistic four-file application. This deliberately exposed the gap between broad package compatibility and native typed emission measured on 2026-08-30;
...
- at that review snapshot, the fixed public Hybrid packet proved safe compatibility rather than typed benefit because it emitted 0/735 units, while its baseline remained enforced. The then-current opt-in external benefit packet separately proved 4/4 emitted commands across three scenario families, so safe packaging and measured typed benefit remained distinct evidence lanes.
...
Short-circuit logical binding now clones symbol state only for the right operand and marks one direct reference variable non-null when reaching that operand proves it: `$null -ne $value` under `-and`, or `$null -eq $value` under `-or`, including the safe reversed form only for a PowerShell-scalar string or a statically sealed non-enumerable scalar. Null equality has an explicit bound operator and emits `Object.ReferenceEquals`, so user-defined C# equality cannot manufacture a non-null proof. Collection-left values, unsealed reference types, enumerable value types including nullable wrappers, opposite predicates, and wider/dynamic conditions do not refine the right operand. They remain hosted or rejected unless the complete right-hand expression has an independent bounded contract; nullable integral/decimal member comparisons now provide one such runtime-free path for opposite predicates without manufacturing non-null state. Runtime-free libraries execute null, positive, zero, reversed, `-and`, and `-or` paths across net472/net8/net10; the 31-case/93-observation exact-host matrix and 24-case Strict subset include a dedicated minimized case. The fixed external packet retains the non-null refinement and now clears that unit's construction diagnostic too, but independent blockers keep measured emission at 5/196 units and 5/183 functions with zero regressions.

Bounded `New-Object` CLR construction now resolves through one compiler-owned command-family contract and normalizes into the same exact constructor selection, bound invocation IR, lowering, and C# backend used by `[Type]::new(...)`. The accepted shape requires a literal `TypeName`, no redirection or splatting, and either no arguments, one closed scalar literal, or one parenthesized list of closed scalar literals; module qualification and the documented `Args` alias are accepted. `Property`, `ComObject`, `Strict`, dynamic type names, variable or splatted argument lists, nested/empty arrays, array-as-one-argument ambiguity, unsupported target types, non-exact overloads, and runtime-error wrapping remain hosted or rejected. Cross-target net472/net8/net10 libraries, the 31-case/93-observation exact-host matrix, and the 24-case Strict subset prove the bounded contract. The safe literal slice removes 9 of the former 25 `command.new-object` occurrences; 16 wider occurrences remain across four external workloads instead of being misclassified as runtime-free. Independent runtime-scope, lifecycle, pipeline, parameter, filesystem/provider, and member blockers keep the honest packet at 5/196 emitted units and 5/183 emitted functions.

Exact `System.Diagnostics.CodeAnalysis.SuppressMessageAttribute` declarations now have one compile-time-only metadata owner shared by structural analysis and semantic validation. The bounded shape requires the real framework attribute on a script or function parameter block, exactly two literal string constructor arguments, and only literal `Justification`, `MessageId`, `Scope`, or `Target` properties; it is deliberately omitted from the runtime ABI. Dynamic or malformed arguments, unknown or repeated properties, parameter-level placement, and unresolved lookalikes fail closed. A real Strict executable and the existing parameter-metadata oracle prove that suppressions do not change execution or output. The fixed external packet drops from 45 to 34 `parameter.metadata` occurrences and from 38 to 31 affected units while staying at 5/196 emitted units and 5/183 emitted functions with zero regressions, so this is a generic semantic closure rather than a claimed coverage increase.

Standalone `[void] expression` now binds as a statement-level discard rather than a general conversion to `System.Void`. The accepted shape is exactly one single-element statement pipeline whose operand already has a typed contract; return, assignment, expression-container, multi-stage pipeline, and unsupported-operand forms still fail closed. The binder preserves operand evaluation, lowering carries explicit discard intent, and one collision-free generic backend helper owns both `[void]` and value-returning `$null = expression` without emitting success output. Bare values, nested control flow, side-effecting member calls, net472/net8/net10 artifacts, Windows PowerShell 5.1, pinned PowerShell 7.4/7.6, and the complete 31-case/93-observation matrix prove the boundary. The fixed external packet removes one of six `expression.conversion` occurrences and one of six affected units while remaining at 5/196 emitted units and 5/183 emitted functions with zero regressions; independent blockers prevent an emission increase.

Exact static enum-member parameter defaults now use the canonical target-typed literal owner rather than evaluating a general PowerShell expression. The enum type must exactly match the parameter's enum or nullable-enum type, optional parentheses may contain only the single member expression, and one case-insensitive public literal member must resolve from the selected target framework's reference metadata. Its target-metadata value becomes the portable numeric ABI, so a member available only to the compiler's newer host framework fails closed for an older target. Mismatched types, missing members, non-enum static properties, method calls, and wider runtime defaults remain fallback. Generated binary-cmdlet omitted/explicit behavior, explicit net472 rejection of a newer-only member, net472/net8/net10 runtime-free libraries, Windows PowerShell 5.1, pinned PowerShell 7.4/7.6, and the complete 31-case/93-observation matrix prove the boundary. The fixed external packet drops from 8 to 6 `parameter.default` occurrences, from 7 to 5 affected units, and from 4 to 3 affected workloads while staying at 5/196 emitted units and 5/183 emitted functions with zero regressions; independent blockers still prevent an emission increase.

The exact read-only `$ExecutionContext.SessionState.LanguageMode` chain now uses the existing runtime-state classification, bound/lowered IR, and binary-host state dictionary. Generated cmdlets capture the live `PSLanguageMode` at invocation; no profile label or build-time snapshot substitutes for session state. Assignment, locally defined `$ExecutionContext`, other execution-context/session-state members, and runtime-free targets fail closed. Real net472/Windows PowerShell 5.1, net8/pinned PowerShell 7.4.19, and net10/pinned PowerShell 7.6.5 binary modules plus the expanded 31-case/93-observation exact-host matrix prove the boundary. The fixed external packet moves one unit from `runtime.scope` to its independent enum/string comparison blocker: runtime-scope occurrences fall from 192 to 191, affected units from 56 to 55, and visible sole blockers from 5 to 4, while emission stays at 5/196 units and 5/183 functions with zero regressions.

Hosted binary modules now preserve scalar enum-left/string-right equality for `-eq`, `-ne`, `-ceq`, and `-cne` through PowerShell's public `LanguagePrimitives.Equals` contract. The bound operator owns the required language-runtime capability, so optimizer reconstruction cannot silently turn hosted semantics into a runtime-free claim. Reverse operand order, relational operators, arrays, and object-typed right operands remain rejected or hosted. Real net472/Windows PowerShell 5.1, net8/PowerShell 7, pinned PowerShell 7.4.19 and 7.6.5 execution, and the complete 31-case/93-observation matrix cover case-insensitive, case-sensitive, numeric-text, invalid-text, and inequality behavior. The fixed external packet remains at 5/196 emitted units and 5/183 emitted functions with zero regressions; `syntax.unsupported` falls from 106 to 105 occurrences, from 40 to 39 affected units, and from 6 to 5 visible sole blockers, but the affected function still has an independent hosted command boundary.

PowerShell's adapted `Count` member on a statically array-typed value now resolves in the canonical CLR member binder to total CLR `Length`, using the existing null-to-zero receiver contract. The bounded slice covers `[array]`/`System.Array`, concrete one-dimensional array types, case-insensitive member spelling, null and empty inputs, and multidimensional values received through the supported `System.Array` ABI; dynamic object receivers and explicitly multidimensional parameter types outside the current target policy still fail closed. Runtime-free libraries targeting net472, net8, and net10 build and execute in the .NET 10 validation process, the modified semantic case passes the complete 31-case/93-observation exact-host matrix, and its Strict artifact remains identical to the pinned PowerShell 7.6.5 result. The public acceptance packet remains green at 10/10 unrelated modules and 4/4 Strict programs. The fixed external packet remains at 5/196 emitted units and 5/183 emitted functions with zero regressions, and its measured `syntax.memberexpression` frontier remains 15 occurrences across 11 units because the counted sites are outside this statically array-typed slice. No workload identity or input-specific rule participates in eligibility.

Bounded hosted Boolean commands now have one command-family IR path rather than command-name logic in the emitter. The initial `Test-Path` provider accepts only `LiteralPath`/`PSPath` expressions proven to begin with `FileSystem::`, mandatory literal `ErrorAction Ignore`, optional literal `PathType`, and optional `IsValid`. It routes the canonical module-qualified command plus bound values through the existing PowerShell capture delegate, preserving FileSystem-provider, literal-path, path-type, and host-version behavior inside that bounded, error-neutralized contract. The backend renders only the lowered provider contract and arguments. Real net472 and net8 generated binary modules match their source hosts. Other providers, unqualified paths, `Path` wildcard semantics, omitted or wider error policy, the PowerShell 7-only `LP` alias, basic functions, Strict runtime-free artifacts, and array, coercive, redirected, or wider command shapes fail closed. Completed generic traversal covers nested runtime-state and stream-message expressions for host delegates, bound parameters, call graphs, provider evidence, and boundary counts. The fixed packet remains 5/196 units and 5/183 functions with zero regressions, while the one visible sole `command.test-path` blocker remains outside the safe slice.

No-argument `Get-Date` now resolves through a compiler-owned runtime-state command provider and the canonical parser → binder → bound IR → lowering → backend route. The exact accepted shape has no positional or named arguments, splatting, invocation operator, or redirection; it produces one `System.DateTime` by reading `System.DateTime.Now` at runtime, so the result remains local-time `DateTime` state and is never folded into the build. The provider is runtime-free, AOT-compatible, dependency-free, recorded in emitted method and artifact evidence, and closed to package or direct-extension registration. Formatting, authored dates, Unix formatting, wider parameters, aliases, and other command forms remain fallback or rejection. Strict net472/net8/net10 libraries, a real `win-x64` Strict NativeAOT publish/run, and real net472/Windows PowerShell 5.1 plus net8/PowerShell 7 binary modules prove the bounded contract. The public acceptance packet remains green at 10/10 unrelated modules and 4/4 Strict programs. The fixed external packet removes all seven former `command.get-date` occurrences across three affected units but remains at 5/196 emitted units and 5/183 functions with zero regressions because those units retain independent blockers.

Command-resolution closure now has one semantic owner rather than separate name guesses in inference, command islands, and provider binding. The provider registry catalogs identities; one resolver applies authored local-function precedence and target-host disposition, and analyzer/explain, inference, binding, Hybrid, Strict, and provider evidence consume it. Generated PowerShell hosts lower a provider only from its canonical `Module\Command` source identity. Unqualified commands preserve PowerShell's local, required/imported-module, alias, and caller-session lookup through a hosted or retained source path; Hybrid remains compatible and Strict fails closed when fallback is required. Closed runtime-free artifacts may use an unqualified registered provider after local-shadow exclusion. The real built-in ownership split (`Microsoft.PowerShell.Core` for `Where-Object`/`ForEach-Object`; `Microsoft.PowerShell.Utility` for `Select-Object`/`Sort-Object` and the current utility providers) is now test-locked. Direct return, typed assignment, untyped assignment, local shadow, required-module shadow, caller-session shadow, module-qualified success, module-qualified alias deferral, Hybrid fallback, Strict rejection, PS5.1/net472, PS7/net8, and runtime-free artifact tests cover the decision matrix. This closes the duplicate-resolution-brain defect; it does not broaden the supported command set.

The reviewed external baseline was intentionally lowered from 9/196 to 5/196 emitted units and from 9/183 to 5/183 emitted functions when this closure landed. `Locksmith2` moved from four emitted functions to one and the Scoop installer from two to one because their additional candidates depended on unqualified host commands whose exact PowerShell command identity had not been proven. Recovering those four units by restoring registry-first name matching is forbidden; future gains must use canonical qualification, a closed runtime-free intrinsic, or another explicit semantic boundary with exact-host evidence. The refreshed census passes 7/7 with zero regressions, the execution qualification remains 4/4 across four unrelated workloads, and the public packet remains 10/10 Hybrid modules plus 4/4 Strict programs.

Explicit local-function positions now bind through the existing local-call signature, bound invocation, lowering, and backend path. The binder orders declared `[Parameter(Position = n)]` slots by numeric position rather than declaration order, treats parameters without an explicit position as named-only once any explicit slot exists, skips already named slots, preserves authored argument-evaluation order, and honors explicit positions even when `CmdletBinding(PositionalBinding = $false)` disables only implicit positioning. Sparse numeric positions therefore describe order without requiring gap arguments. Named parameter sets, duplicate/invalid position metadata, and surplus positional arguments remain fail-closed through the existing validators and local-call diagnostics. The parameter-binding semantic oracle, net472/net8/net10 runtime-free artifacts, net472/Windows PowerShell 5.1 and net10/PowerShell 7 Hybrid differential execution, and explicit negative tests cover the boundary. The fixed external packet removes all 10 former `parameter.binding` occurrences across five units, including its one visible sole blocker, but that function advances to the independent `binary-module.cmdlet-shape` gate; measured emission remains 5/196 units and 5/183 functions. The final ledger continues to report that analyzer-eligible shaped fallback and its cause. Census products and external baseline schema 2 now carry the stable final-ledger identity and disposition of every authored function. Comparison fails closed when a previously eligible or emitted function regresses, when a function newly becomes runtime-routed, when a previously eligible function newly becomes shaping fallback, or when identities are missing or duplicated; aggregate equality therefore cannot conceal replacement of one emitted function by another. A previously ineligible runtime-routed function may advance to an explicitly attributed shaping fallback without being mislabeled as loss. Source identity, parser health, emitted minima, and fallback maxima remain independent gates. This keeps semantic progress visible without hiding artifact-stage loss or creating a second eligibility authority.

Stable scalar defaults for authored `[array]`/`System.Array` parameters now use the canonical target-typed literal contract. PowerShell's compile-time conversion produces one `object[]` element for a scalar default such as `'All'`; the portable literal ABI records the array and its stable element type, and the parameter prologue preserves PowerShell's distinction between an omitted default and explicitly bound input. Validation applies only to the latter, so an intentional out-of-set sentinel default remains valid while the same explicitly supplied value fails. Authored collection expressions, authored conversions that would preserve another runtime type, general `System.Array` conversion expressions, dynamic expressions, nested arrays, and non-encodable elements still fail closed. Numeric spellings proved unavailable to the selected host profile are promoted to file-level parse failures in Package, Hybrid, and Strict, because retained source would not be a viable fallback on that host. The bounded numeric gate is shared with literal lowering and does not claim complete historical-parser emulation. Runtime-free net472/net8/net10 libraries preserve and deserialize the nested ABI, omitted sentinel defaults, explicit empty arrays, and explicit validation failure; real net472/Windows PowerShell 5.1 and net10/PowerShell 7 binary modules match the same source behavior. The fixed external packet remains at 5/196 emitted units and 5/183 functions with zero regressions; its post-emission function frontier still contains two unrelated `parameter.default` occurrences, so no corpus emission gain is claimed for this slice.

The 2026-09-03 live-module-state slice raises the exact external assessment to 6/196 emitted units (3.06%) and 6/183 emitted functions (3.28%) with zero parser errors or baseline regressions. The newly emitted function is `Locksmith2/Public/Get-LS2Stores.ps1`; this is evidence that the generic `$script:` contract unlocked a real administration function, not a compiler rule for that workload. Its emitted method is still reported as runtime-routed because each read crosses into the retained parent script-module scope. The gain therefore means more typed CLR surface, not runtime independence or measured acceleration. Complete-workload execution remains 0/7 in this census-only lane.

The 2026-09-03 write slice accepts only direct untyped `$script:<safe-identifier> = <typed-value>` statements in Hybrid modules. It emits a distinct write-bound IR statement and writer ABI, preserves the parent script scope as the only value owner, and propagates read and write authority independently through compiled local calls. The assignment value still traverses the canonical semantic expression tree, so bound-parameter presence, runtime state, hosted commands, provider cancellation, streams, and nested local-call requirements cannot disappear merely because the expression is a module-state write. Scope-bound writers preserve null, scalar, array, and object identity/cardinality plus case-insensitive variable names and the native `VariableNotWritable` error for Constant and ReadOnly variables on Windows PowerShell 5.1 and PowerShell 7. The generated bridge rethrows the original exception through the compiled body, allowing an authored compatible `catch` to handle it, and converts only an unhandled tracked state error back to its original `ErrorRecord` at the cmdlet boundary. The focused state cluster passes 21/21 on Windows and 21/21 on the native Linux `net10.0`/PowerShell 7 lane with SDK 10.0.303. Normal removal and failed initialization clear writer registrations. Ledger schema 2, IR snapshot schema 2, and explanation schema 3 record read and write crossings independently; the prior public ledger constructor remains available and JSON round trips preserve the new fields. Compound assignment, increment/decrement, typed targets, unsafe/dynamic names, and member/index mutation still fall back. The exact online and offline external packet remains 6/196 units and 6/183 functions with zero regressions: the new contract closes a common state boundary but no additional function clears all of its independent blockers, so this slice claims semantic reach rather than an emission increase.

The 2026-09-04 typed-access slice permits a member or index read only when the authored expression itself converts one safe live `$script:` variable to an exact supported CLR receiver type, such as `([string]$script:Name).Length` or `([string[]]$script:Items)[0]`. The runtime-state binder still owns the read, the existing conversion and CLR member/index binders own the operation, and the ordinary bound IR, lowering, and backend path emits it; no declaration scan, retained-initializer inference, ETS dispatcher, or AST-to-C# path was added. One canonical module-state-origin fact follows the bound value through conversions, local assignments, success output, and local-call results. Member/index/invocation, loop, pipeline, and lifecycle consumers fail closed unless the receiver is the exact permitted direct conversion; object/ETS receivers, nested conversions, state-derived locals, call arguments/results, and method calls therefore cannot widen the boundary. Hosted command regions and captures consult that same origin before projecting arguments and fall back the enclosing function when an input is state-derived, so a hosted command or retained lifecycle cannot launder the value back into dynamic ETS access. Index reads allocate collision-free target and key temporaries so both evaluate exactly once and in authored order. The review closure also corrected indexed mutation globally: lowering now captures RHS, target, and key exactly once in PowerShell's RHS-first order, including before null or bounds failures, while hosted array/list failures retain their effective `IndexOutOfRangeException`/`ArgumentOutOfRangeException`, FQID, and `OperationStopped` category and null arrays retain `NullArray`. Real artifacts match Windows PowerShell 5.1 and PowerShell 7 for these access, order, typed-catch, and failure-identity cases. Member/index mutation of live module-state-derived objects and every Strict parent-module-state use remain fallback or fail closed. The portable corpus remains 8/8 emitted functions, and the exact offline external packet remains 6/196 emitted units and 6/183 emitted functions with zero regressions. The packet contains typed state declarations but no function cleared through this exact authored-cast access contract, so this is a proven semantic expansion rather than an aggregate emission claim.

The 2026-09-04 terminal-region slice is the first partial-function promotion through that same compiler. The refreshed public packet builds, imports, and invokes 10/10 modules and executes 4/4 Strict programs; it records 13 promoted regions inside retained Pester functions, two already-existing full compiled cmdlets, and 733/735 runtime-routed units. A control run at the pre-slice head confirmed that the two full PSFramework cmdlets were already emitted, so the refreshed baseline does not attribute them to region promotion. The refreshed external assessment remains 6/196 emitted units and 6/183 emitted functions, adds two promoted regions in `Locksmith2` (`Test-IssueExists` and `Show-ConnectionContext`), reports zero parser errors and zero regressions, and still performs 0/7 complete-workload executions. These 15 regions are coverage-discovery evidence, not yet proof of useful acceleration: the public probes do not isolate the promoted Pester functions, the external assessment is census-only, and no cross-family performance result has been recorded.
...
- [ ] Isolate native-Linux compiler validation in its own checkout and intermediate-output tree, and classify every test by real host capability. The current command-resolution/artifact matrix passes 20/20 under Linux with exact SDK 10.0.303 plus the net8 targeting pack, and Windows-only CIM/WMI/CDXML/COM/process fixtures now use the shared `WindowsFact` gate. The aggregate compiler category is not yet a valid native-Linux gate because older external-wrapper, Authenticode, net472, and Windows PowerShell oracle tests still invoke Windows-only APIs or cross-host executables; do not report those environment failures as compiler regressions or repeat the earlier blanket full-suite claim at a new head.
...
Exit gate: **Partial.** The support matrix advertises only the already target-proven `win-x64` and `linux-x64` Strict profiles, keeps macOS/Arm64 and additional deployment profiles experimental, and exposes measured opt-in recommendations plus explicit performance budgets. The Strict Linux artifact/profile proof remains valid, but the broader native-Linux test harness still needs the capability partition above before it can become an aggregate release gate. Physical macOS/Arm64 promotion and a stable-channel release remain separate qualification work.

## Milestone 23 — Compile module-scale state and regions without a second runtime

This is the coverage milestone after the current corrective gate above. Hybrid must preserve one live PowerShell module and accelerate proven code around it; Strict must not inherit credit from that hosted state. Analysis-only discovery can continue while the accepted-code defects are corrected.

- [x] Make the retained parent script-module scope the sole state owner for Hybrid binary modules. Generated methods do not receive a copied dictionary snapshot and do not create a competing C# static value.
- [x] Lower a direct read of `$script:<safe-identifier>` through the canonical runtime-state binder, bound IR, lowering, and backend only when the artifact has the dedicated Hybrid module-state capability. Dynamic names, names requiring non-identifier characters, and member/index access on an untyped state value remain fallback.
- [x] Register scope-bound readers per runspace, preserve scalar, array, object, null, and original PowerShell error behavior, and install cleanup before retained initialization. Failed initialization rolls back both reader and command-dispatch registrations; an early module `return` keeps the removal hook; ordinary removal clears the registrations. If retained initialization replaces `OnRemove`, compose the authored handler with generated cleanup and guarantee cleanup from `finally`, including when the authored handler throws. Local compiled helper calls propagate the same host delegate.
- [x] Carry exact read/write authority, variable names, and module-state read/write-site counts from lowered IR into generated-method metadata, ABI host parameters, IR capabilities, the final disposition ledger, boundary counts, explain output, and census. Ledger schema 2, IR snapshot schema 2, and explanation schema 3 introduced the directional state contract; current ledger schema 4, IR snapshot schema 3, explanation schema 5, and semantic compatibility 4 preserve it while adding canonical promoted-region evidence. The compatibility constructors, schema-2 JSON read test, and reviewed current JSON round trip prevent the ledger change from silently breaking existing consumers. The pre-ledger public explanation overload is explicitly obsolete, stays on explanation schema 2/semantic compatibility 1, and fails closed for module-state methods instead of emitting false directionality; all current final artifact and CLI paths consume the immutable ledger overload. The ledger labels this exact hosted route `HostedModuleState` without relabeling ordinary invocation-state capture. Metadata collectors, graph construction, and RHS host-requirement discovery share canonical semantic/lowered-tree enumerators rather than rediscovering syntax.
- [x] Prove the boundary against Windows PowerShell 5.1 and PowerShell 7, including mutation by retained fallback code, StrictMode missing-variable identity, two-runspace isolation, successful removal, failed-initialization rollback, early-return cleanup ownership, authored-`OnRemove` composition when that handler throws, untyped direct and parenthesized member/index rejection, unsafe-name rejection, Strict binary-module failure, and shallow/full census capability parity.
- [x] Refresh the exact external baseline only after the offline packet reports 6/196 emitted units, 6/183 emitted functions, the new function as both emitted and runtime-routed, and zero regressions.
- [x] Add bounded direct state writes only through the same parent-module owner, with exact null/scalar/array/object assignment, case-insensitive naming, Constant/ReadOnly error identity, authored compatible `catch` handling, read-after-write ordering, per-runspace isolation, local-call propagation, nested RHS host requirements, combined hosted-command/write accounting, and lifecycle cleanup. Only an unhandled tracked state error is converted back to its original PowerShell `ErrorRecord` at the cmdlet boundary. No generated static value store exists. Compound assignment, increment/decrement, typed targets, unsafe names, and member/index mutation remain explicit fallback boundaries.
- [x] Add bounded member/index reads only when an authored conversion over one safe live `$script:` variable supplies the exact CLR receiver type and the existing canonical conversion/member/index binders prove the operation. A canonical bound-value origin follows state-derived conversions, locals, and local-call results; member/index/invocation, loop, pipeline, lifecycle, hosted-region, and command-capture consumers fail closed outside the direct-read contract. Index reads evaluate target/key once in authored order. Indexed mutation evaluates RHS/target/key once in PowerShell's RHS-first order and preserves array/list/null failure and typed-catch identity on Windows PowerShell 5.1 and PowerShell 7. Untyped or inferred object-valued state is not permission for dynamic ETS compilation, and nested/compound receivers, call escapes, method calls, effectful indexes, module-state member/index mutation, and Strict parent-module state remain fallback or fail closed.
- [x] Derive one deterministic coarse region graph from canonical lowered IR for every already-emitted method. Consecutive boundary-free top-level statements form typed regions; direct command boundaries form hosted regions; typed control/value flow containing command or live-module-state crossings forms mixed regions. The graph records stable authored-source identity and span, case-normalized PowerShell symbol/state inputs, downstream-observed outputs, conservative possible receiver mutations, streams, modeled error routes, ordering, exact static boundary sites, and a structural ranking cost that is explicitly not a runtime estimate or complete effect proof. The C# backend, executable/module method metadata, final ledger, CLI explanation, and category-framed semantic fingerprint consume this graph; the former backend-only boundary counter was removed. Selection, movement, or fusion still requires an explicit soundness proof and measured evidence.
- [x] Preserve enough canonical bound/lowered structure for an otherwise-hosted function to select and emit safe typed regions around its hosted remainder. Selection occurs in semantic/bound IR before the final ledger records the shaped result; it does not use the ledger as a planning input or add an AST-to-C# island compiler. Each promoted region retains the graph's value, stream, error, ordering, source-map, and boundary identity and falls back as a whole when those contracts cannot be closed.
  - [x] The first bounded slice promotes one terminal suffix after the final binder-rejected top-level statement in a retained Hybrid function. The suffix is reused from the binder's successful statement results and traverses the ordinary optimizer, semantic analyzer, lowerer, region-graph builder, and C# backend. Promotion requires an always-returning supported transfer value, exact parameter or local inputs, local-only mutation, Success output only, no modeled error route, no hosted/runtime/module-state boundary, no isolated local-call closure, and no named `dynamicparam`/`begin`/`process`/`end`/`clean` lifecycle. Source identity, complete-document and exact selected-text hashes, span, source map, graph, helper member, boundary crossing, ledger/explain/IR/failure-map evidence, artifact/module summary, and census count are retained. A dedicated Hybrid-region target capability is separate from live-module-state authority so future Strict state work cannot inherit this route accidentally. The composer only validates and applies the immutable decision. Strict never receives the region capability, and any source or function-header drift or unclosed contract retains the complete suffix as PowerShell.
  - [x] Expose every candidate considered by that slice from the same binder, analyzer, lowerer, and promotion policy as immutable census evidence. Each decision carries stable source/document hashes, source/span identity, its canonical lowered graph when one exists, promoted versus retained status, and one stable decision code plus human reason. Final method-name shaping reconciles the decision so a helper dropped by a CLR-name collision cannot remain labeled as promoted. Both fixed packet runners now use one shared portable projection and retained-decision aggregator; the public lane also requires the census and delivered artifact ledger to agree on promoted-region count. Full paths are reduced to contained relative paths, and a candidate rejected before lowering retains an explicit null graph instead of breaking strict-mode reporting. Candidate observations remain separate from emitted-unit/function coverage.
  - [x] Rank the first terminal-candidate frontier before expanding its ABI. The current public packet contains 30 candidates across two of ten modules: 13 one-line promoted scalar returns and 17 retained candidates. Five `PSD1001` candidates span Pester and Microsoft.PowerShell.Archive, but each is only a one-line return of a prefix-owned local; the other twelve are one-to-three-line Pester `[Pester.ShouldResult]` constructions represented as `System.Object`. The seven-workload external packet contains four candidates in one workload: two promoted, one one-line live-local return, and one five-line hosted verbose/return tail with structural cost five. These records justify neither general `System.Object` transfer nor live-local/hosted-command crossing: none contains enough independently typed work to plausibly amortize another boundary.
  - [x] Extend canonical candidate discovery beyond an always-returning terminal suffix before extending execution. For every binder-rejected Hybrid function that still contains successfully bound top-level statements, the binder now records maximal statement-aligned runs and a dedicated analysis path reuses the ordinary optimizer, semantic passes, lowerer, and canonical region-graph builder without invoking the backend. The public evidence records exact source identity, statement/span size, continuation shape, separate predecessor/input and continuation/output completeness, typed live inputs, conservatively widened live outputs when an unbound continuation could observe any mutation, streams, errors, ordering, direct local calls with transitive bound-call closure, and state-boundary facts. Evidence is replaced per function on every fixed-point binding attempt, sequence reachability stops at the first terminating statement, and optimizer-flattened regions are mapped back only when the complete overlapping authored statement remains typed; an authored statement containing a mixed boundary is omitted until a substatement identity contract exists. Strict receives no opportunity evidence. Final compilation, promoted-region counts, generated C#, and retained PowerShell are unchanged, and legacy census JSON without the additive field still reads as an empty opportunity set.
    Both fixed packet runners use one path-contained portable projection and aggregate only compiler-owned facts. The public packet reports 367 analysis-only opportunities, including 337 outside the terminal-candidate window across five of ten module families; its largest opportunity spans six top-level statements and nine lines. The external packet reports 59, including 55 outside that window; its largest spans eight statements and 47 lines, but all external opportunities occur in the two related AD CS workloads. After conservatively treating every mutation before an unbound continuation as a possible live output, the combined frontier has only two repeated multi-statement shapes across unrelated families. Both span two statements, have incomplete output-consumer analysis, and require non-stable dictionary or object-array transfer. There are zero complete stable-scalar multi-statement opportunities outside the terminal window. This is useful discovery evidence and a reason not to emit them yet.
    The coverage qualification plan selects compiler-described contracts from real scripts and modules and preserves source identity plus original/generated value, type, output, scope, stream, error, and continuation evidence. Exercise unrelated consumers where the contract recurs, without requiring a speedup or a fixed number of benchmark families. Record emitted, retained, and unsupported behavior separately. No source or workload identity may influence selection. Performance comparisons remain optional supporting evidence until the optimization phase.
  - [x] Broaden the opportunity census beyond binder-rejected functions to functions retained only by later semantic, graph, cmdlet-shaping, or artifact decisions. Preserve the canonical bound/lowered owner and reconcile final disposition without allowing census or ledger output to become a planning input. Discovery still reports analysis-only opportunities and grants no execution credit.
  - [x] Add a bounded non-terminal prefix with ordered stable-scalar transfers and preserved authored type constraints, including nullable constraints on both PowerShell runtimes. Keep selection in canonical bound/lowered owners and reject incomplete effects, scopes, errors, and transfers. Whole-function selection takes precedence over overlapping helpers.
  - [x] Unlock additional real commands or complete workflows through correct reusable semantic contracts. The unchanged pinned PSSharedGoods IP conversion command executes a one-statement scalar prefix, `Get-ObjectEnumValues` keeps a guarded Hashtable initializer plus an exact prefix-owned terminal passthrough around retained mutation, and `Split-Array` executes its conditional parameter-array return, fresh typed-list construction, and no-enumerate terminal return on both supported hosts. Existing report and formatting parity, numeric computation, and bounded-counter evidence inform further work; measured speed does not determine coverage acceptance.
  - [x] Replace the scalar/Hashtable special case with a schema-versioned transfer contract for shape, element kind, direction, ownership/provenance, output enumeration, and mutation. Exact atomic `Hashtable`/`OrderedDictionary`, one-dimensional stable-scalar arrays, and read-only exact `System.Array`/`ArrayList`/`List<T>` references can cross retained boundaries. Retained definite assignment, later fresh-local ownership, additional statement-aligned selection, and authored comma/no-enumerate output share this owner. A return-or-fallthrough envelope now carries parameter-borrowed values without enumerating them in generated code; retained PowerShell owns output, partial success, error continuation, stopping, and disposal. Before the return-cardinality tranche, the full pinned PSSharedGoods census exposed 47 promoted regions, 70 candidates, and 19 analysis opportunities. Candidate execution is limited to real functions, and target-runspace reconstruction leaves the authored declaration installed if its optimized alternative cannot be constructed.
  - [x] Compile one closed stable-scalar `foreach` array capture through the canonical parser, binder, bound IR, analysis, lowering, region graph, generated backend, transfer ABI, and retained continuation. The capture owns an exactly typed fresh vector and implicit success writes only; retained PowerShell owns its later one-level enumeration. The unchanged PSSharedGoods `Get-ADEncryptionTypes` and `Get-ADTrustAttributes` workflows provide dual-host evidence.
  - [x] Distinguish terminal and conditional bare returns from explicit null returns through schema-3 `NoValue` and `NullValue` contracts. Retained PowerShell observes zero records for a bare return and one null record for `return $null`; the unchanged PSSharedGoods `ConvertTo-ImmutableID` terminal return raised that checkpoint's census to 48 promoted regions while AutomationNull, dynamic empty subexpressions, and arbitrary object returns remained retained.
  - [x] Carry one closed scalar-or-matching-vector branch choice through `ClosedValueAlternative`, introduced in schema 4 and preserved in current schema-6 transfer metadata, plus an atomic compiler-owned envelope. Retained PowerShell restores the exact authored constraint before it can enumerate or mutate the value. Nine unchanged PSSharedGoods `Get-Computer*` selectors now include their conditional property selection in promoted prefixes; `Get-ComputerBios` supplies dual-host differential execution. At that checkpoint the census remained at 48 promoted regions because the family extended existing regions rather than adding new ones.
  - [x] Admit one exact zero-parameter local `ArrayList` factory through schema-5 evidence. Canonical resolution and immutable IR record the empty CLR parameter list, exact result type, callee-fresh ownership, no-enumerate cardinality, and retained-only mutation. Region lowering constructs the equivalent fresh list without depending on the retained helper's public stream ABI. Four unchanged PSSharedGoods consumers share this source shape; three net-new regions raise the census to 51 promoted regions and 73 candidates while opportunities remain 19.
  - [x] Reuse that exact factory projection inside complete Hybrid native consumers. The binder lowers the proved call without the helper's native parameter binder and writes the constructed list through invocation-owned native variable storage. `New-SqlQuery`, `New-SqlQueryAlterTable`, and `New-SqlQueryCreateTable` become complete methods, absorbing their three earlier regions; `Get-ObjectPropertiesAdvanced` keeps its narrower region because later ETS behavior remains independently hosted. The refreshed census reports 159 emitted functions, 45 promoted regions, 73 candidates, and 17 opportunities.
  - [x] Admit computed instance/static member reads and computed instance-member assignment only in a Hybrid native-function body. The native binder, immutable IR, lowerer, backend, and PowerShell runtime binder preserve adapters, type tables, static targets, errors, and left-to-right exactly-once receiver/name/index evaluation. Native assignment compiles only the authored target AST and keeps the compiled RHS in IR. Strict, runtime-free libraries, typed regions, null-conditional reads, computed method names, and dynamic object/ETS transfer remain closed. Thirteen unchanged PSSharedGoods functions move from fallback to emitted without source-name policy; two former regions become complete methods, so the refreshed census reports 154 emitted functions, 49 promoted regions, 73 candidates, and 18 opportunities.
  - [x] Route a complete authored multi-stage pipeline through the existing Hybrid native command-region contract whenever at least one stage is a command, the terminal stage is not an explicit success-stream sink such as `Out-Null`, and the pipeline does not begin with a compiler-owned invocation that already supports narrower typed continuation. The active invocation preserves command/alias resolution, parameter binding, script-block scope, stream order, lifecycle, errors, stopping, disposal, and mutations of retained collection references. Known compiler intrinsics do not split ownership of the enclosing pipeline. Strict and runtime-free libraries remain unchanged. Two pinned workflows become complete methods while the enum workflow keeps two promoted regions, moving the census to 156 emitted functions, 48 promoted regions, 73 candidates, and 17 opportunities.
  - [x] Add separate native range and regex-match operators. The canonical operator binder and IR keep the contracts distinct, while generated Hybrid methods delegate both to the loaded PowerShell semantic owner. PowerShell 7 supplies its range operation; Windows PowerShell 5.1 converts the left endpoint before evaluating the right and then uses its native integer range operation with original conversion exceptions. Match preserves scalar/collection behavior, regex objects, culture, case and negation, invalid-pattern failures, and existing `$Matches` state. Range preserves inclusive order, slicing/cardinality, endpoint conversion/overflow, host-specific character behavior, nulls, and single evaluation. Direct `foreach` range iteration remains hosted until the compiler represents PowerShell's lazy range enumerator. Nine unchanged pinned functions become complete emitted methods, including the previously partial `Convert-IPToBinary`; no prior emitted function is lost.
  - [x] Add target-compatible type literals as one bounded expression family. `TypeExpressionAst` binds to one `System.Type` value only when the selected target reference surface contains the type, and the backend emits `typeof(...)`. Missing types and .NET 10-only types under net472 fail closed. At this checkpoint, direct and transitively hoisted type literals consumed by invocations inside observed native `try/catch` remained hosted after the unchanged SID conversions exposed a method-invocation wrapper mismatch. The later direct/catch-all closure is recorded below; hoisted values remain hosted. This checkpoint reports 168 emitted functions, 40 promoted regions, 70 candidates, and 14 opportunities.
  - [x] Add one runtime target-conversion contract for Hybrid native functions. The active PowerShell host resolves the authored type name before operand evaluation and applies its conversion binder; Strict and runtime-free targets remain closed. Synthetic dual-host proof covers null, scalar, array, errors, repeated calls, and reimport. `Convert-DomainToSid`, `Convert-DomainFqdnToNetBIOS`, and `ConvertFrom-NetbiosName` become complete emitted methods without source-name policy. That checkpoint's pinned census reports 171 emitted functions, 40 promoted regions, 70 candidates, and 14 opportunities with no input failures or emitted-function losses.
  - [x] Route direct `foreach` over untyped or `[object]` parameters through existing Hybrid native invocation binding, not a guessed CLR element type. The PowerShell host owns lazy enumeration and iterator storage while the compiler still binds, analyzes, lowers, and emits the loop. Three unchanged PSSharedGoods functions become complete methods and pass original/generated execution on both supported hosts. Typed-source loops keep their established path; the ordered-map `Get-ProtocolDefaults` loop stays hosted with one promoted prefix. The current pinned census is 174 emitted functions, 39 promoted regions, 69 candidates, and 12 opportunities with no input failures.
  - [x] Close one direct type-literal/catch-all invocation family without widening typed-catch or hoisted-value admission. The native catch matcher may select the inner error for typed clauses, but an untyped catch must expose the original method-invocation wrapper through `$_`. The canonical statement-error owner corrects only the base method-invocation record; derived records retain their native metadata. Unchanged `ConvertTo-SID` and `Convert-Identity` pass dual-host original/generated missing-identity, caught-record identity, repetition, and reimport checks as complete Hybrid methods. `ConvertFrom-SID` still stops at nested note-property output, and `Test-ComputerPort`'s `[ref]` call needs separate proof. The pinned census is 176 emitted functions, 39 promoted regions, 30 retained region candidates, and 12 opportunities without input failures; the exact-candidate gate passed 1,024/1,024 tests and all six Strict programs.
  - [x] Close the bounded typed-array construction family with schema-6 mutation lifetime evidence. A typed region exclusively owns one fresh stable-scalar vector until live-out transfer; retained PowerShell then owns enumeration and later mutation. The complete native-consumer factory projection remains limited to the exact source-proved zero-parameter fresh-`ArrayList` shape. Direct `System.Object`/ETS values, arbitrary enumerators, multidimensional or object-element arrays, explicit output-shaping commands, terminating control flow inside captures, and borrowed compiled reference mutation remain outside the transfer policy. Exact `System.Array` and `List<object>` references stay opaque to compiled code; their elements are observed only by the retained PowerShell boundary.
  - [x] Generalize detached continuation selection from the first accepted run to every non-overlapping canonical run. Preserve every structurally valid statement-aligned suffix so the existing promotion policy can reject an unsafe maximal span without hiding a safe inner span; final reconciliation keeps only maximal approved non-overlapping regions. The one guarded prefix remains the sole fresh-local owner. Dual-host artifacts execute four scalar regions and an exact local `ArrayList` factory region while preserving identity, mutation, stopping, reuse, and reimport. A same-run modeled CLR-exception span remains hosted while its safe suffix is promoted.
- [x] Define Strict module initialization and persistent state as a separate runtime-free contract. It lowers deterministic top-level initialization, module lifetime, serialized concurrency, transactional reset, initialization-failure rollback, disposal, and an instance state ABI without consulting the Hybrid bridge or retaining authored source.
- [x] Execute newly supported commands in their real module or application context and prove authored behavior on supported hosts, including failure and continuation paths. The disposition ledger distinguishes complete emitted methods, promoted Hybrid regions, retained PowerShell, and certified runtime-free Strict artifacts.

Exit gate: **Complete for M24-M26.** The continuation source implements guarded prefixes, every independently safe non-overlapping statement-aligned region, schema-6 transfer ownership including fresh stable-vector construction, bounded local collection factories, return/cardinality contracts, Hybrid-native computed members, sequential output, complete hosted multi-stage pipelines, parent-owned Hybrid state, and independent runtime-free Strict state lifetime. Dual-host matrices cover values, identity, streams, partial failure, stopping, cancellation, cleanup, and continued execution. The final checkpoint passes 1,060/1,060 compiler-gate tests with zero failures or skips, builds and executes 6/6 Strict `net10.0` `win-x64` programs, and builds `PowerForge.PowerShell` for `net472` Release with zero warnings/errors. The same-input pinned census remains 188/282 emitted functions and 36 promoted regions because it contains no third eligible disjoint run; no coverage increase is claimed. Borrowed compiled reference mutation, runtime-free dynamic object/ETS access, arbitrary enumeration, computed method names, null-conditional access, and provider-bound live dependencies remain fail-closed or later work. Performance is not an admission gate.
...
1. [x] Integrate the assignment-constraint correctness fix and preserve its original/generated regression proof.
2. [x] Generalize the M24/M26 transfer ABI across exact atomic maps, stable-scalar vectors, read-only list-like references, retained definite assignment, fresh locals, additional statement-aligned regions, and no-enumerate terminal output. Qualify the unchanged `Split-Array` workflow on both supported hosts.
3. [x] Close the first M24 branch-level return envelope while keeping enumeration failure, partial success, disposal, stopping, and continued execution in retained PowerShell.
4. [x] Continue [M24](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-24--preserve-values-collections-and-error-continuation) with one bounded stable-scalar array/foreach capture through the same transfer and retained-enumeration owners.
5. [x] Close terminal and conditional bare-return versus explicit-null cardinality through the shared transfer, control-flow, generated-backend, and retained-invocation owners.
6. [x] Select and implement a reusable typed-array family. `ClosedValueAlternative`, introduced in schema 4 and carried by current schema-6 transfer evidence, transfers one exact stable scalar versus matching one-dimensional stable-scalar array through a compiler-owned envelope and restores the authored constraint in retained PowerShell; nine unchanged PSSharedGoods workflows share the contract.
7. [x] Add one bounded local-call collection-closure family only after several unchanged workflows share the complete value, ownership, enumeration, error, and continuation proof. The exact zero-parameter fresh-`ArrayList` factory now serves four consumers; speed was not an admission gate.
8. [x] Add grouped object/member/index support as one Hybrid-native computed-member family. Thirteen unchanged PSSharedGoods functions become emitted; Strict and typed/runtime-free transfer remain closed.
9. [x] Route retained collection mutation and complete pipeline flow through the existing Hybrid native command-region owner. Two unchanged workflows become complete methods, while a third retains its narrower typed-continuation path, with dual-host execution or bounded structural proof; no new pipeline engine or type whitelist is introduced.
10. [x] Inventory the next grouped batch across bounded `foreach`/enumeration, range-and-match control flow, type/conversion expressions, and local-function closure. The exact fresh-collection factory projection closes three complete SQL consumers through the existing native assignment owner, without a source-name rule or general call-ABI widening.
11. [x] Qualify the grouped range-and-match tranche through two explicit operator contracts. Dual-host proof preserves endpoint conversion/overflow, inclusive order, character behavior, scalar/collection regex matching, `$Matches`, errors, state, and stopping; nine unchanged functions become complete methods.
12. [x] Add bounded target-compatible type literals and keep the observed native-catch wrapper case hosted. Runtime-free and hosted synthetic contracts prove `System.Type` values, target availability, and fail-closed resolution; the pinned SID workflow proves the retained error boundary on both hosts.
13. [x] Inventory the conversion-only and foreach-only residuals and take runtime target conversion as the largest shared conversion batch. Three unchanged directory-conversion workflows become emitted; `ConvertFrom-X500Address` remains the only conversion-only residual because its blocker is indexed output/cardinality.
14. [x] Inventory the four foreach-only residuals and admit the shared object-parameter host-enumeration contract. Three complete methods are qualified; the ordered-map key loop and dictionary escape remain separate, and no runtime-free arbitrary-enumerator claim follows.
15. [x] Close the first observed native-catch cohort only for one direct type-literal argument under catch-all handling. Two unchanged conversions become complete Hybrid methods with dual-host execution; retain hoisted, `[ref]`, and other unproved invocation shapes.
16. [x] Qualify direct map-key enumeration together with host-family dictionary construction, identity, escape, and streamed record behavior in unchanged `Get-ProtocolDefaults`; keep independent indexed-output and note-property blockers separate. This is a bounded Hybrid slice, not completion of M24's wider exit gate.
17. [x] Qualify `-split`/`-csplit` plus indexed return as one Hybrid family through the existing native operator and index owners. The unchanged `ConvertFrom-X500Address` becomes a complete method with dual-host differential proof; the wider M24 value/cardinality/error exit gate remains open.
18. [x] Add the first shared M24 statement-valued `if`/`elseif`/`else` contract for `[pscustomobject]`, plain/ordered maps, and `@()` through native collection binding and conditional IR. Synthetic original/generated artifacts preserve zero/null/one/many/nested records, branch mutation, direct and nested errors under `SilentlyContinue`/`Stop`, and error-variable contents on both supported hosts; escaping control flow stays authored. The same pinned PSSharedGoods module moves from 180/282 to 188/282 complete emitted functions without losing an emitted function. Newly complete registry translation and random-name workflows receive original/generated dual-host acceptance with state, error, cancellation, and reimport observations. This is not the complete M24/M25 exit gate.
19. [x] Finish that M24 conditional-value family with condition-side failure, prior-destination, injected enumeration/cleanup, wider non-success streams, and stopping proof. The dual-host closure matrix preserves warning/verbose/information observations, caught construction failures without overwriting the prior destination, and injected `GetEnumerator`/`MoveNext`/`Current`/`Dispose` failure traces with partial output and error identity. Strict admission remains independent. The first observed-catch follow-up admits only a directly bound `System.Type` local with closed linear type-literal/alias writes under catch-all handling; dual-host synthetic success/failure proof did not change the 188/282 complete-method census. A following `[ref]` slice admits one trailing direct local only for static CLR calls whose matching overloads take a by-reference final parameter; source/generated dual-host tests prove writeback on success and invalid `TryParse` input, while unsupported storage and argument-alias shapes stay hosted. Its full gate passed 1,050/1,050 tests and six Strict programs; the same-input census remains 188/282 complete methods and 36 promoted regions. The expanded conditional closure and grouped M24/M25 matrix are included in the final 1,056/1,056 gate and six-program Strict pass. Mixed type values, general reference aliasing, and the live UDP `Test-ComputerPort` workflow remain separate boundaries.
20. [x] Close M25 complete-command coverage through the existing output, command-region, statement-error, and local-call owners. Complete workflows with multiple output-producing calls and retained pipeline stages preserve streams, partial failure, stopping, cancellation, cleanup, and later caller continuation on both supported hosts; unknown dynamic observers still fail closed.
21. [x] Productize M27 CLR consumption through the existing ABI, package, provider, and generated-project owners. Static TimeSpan and stateful reservation packages restore into ordinary C# consumers without PowerShell or compiler references; ABI comparison, deterministic source/PDB/package identity, dependency closure, provider cancellation/cleanup, and `net10.0`/`net472` consumption are qualified. The final gate passed 1,063/1,063 tests and 6/6 Strict programs, the `net472` build had zero warnings/errors, and the same-input census remained 188/282 emitted functions with 36 promoted regions.
22. [ ] Deliver M28's source-first run/watch/diagnose/debug loop through the existing project, cache, diagnostics, and source-map owners.
23. [ ] Run M29's provider/platform/public-package qualification as an independent authorized lane. Include additional detection-script states on a disposable target; do not mutate an ambient machine to complete a checkbox.
24. [ ] Optimize already qualified workloads under M30 with fresh measurements and explicit per-workload budgets.
...
The fresh external baseline remains 6/196 emitted units and 6/183 functions. Its two promoted terminal regions and 59 analysis-only opportunities are discovery evidence, not proof of region speedup. See the current assessment for the additional repository scan, reproduced defects, and fresh Windows Strict execution. Earlier bounded provider, corpus, and target-host results remain valid only within their recorded scope and evidence dates.

Do not translate syntax directly in an emitter, clone PowerShell's internal compiler, add repository-specific provider branches, or infer platform support from cross-publish alone.
...
| Hosted dispatch | original function, fine generated cmdlets, coarse cmdlet/typed region, Hybrid boundary | calls per second, crossing cost, allocations | Later optimization target; semantic coverage acceptance depends on preserved behavior, not amortized crossing cost |
...
The 1.5x/3x compute figures are later optimization targets, not current semantic-support gates or universal marketing promises. They apply to pure typed kernels large enough to dominate process and command-dispatch startup. If a semantic helper misses a target, preserve its behavior and record the cost while continuing correctness and coverage work. Do not weaken semantics to hit a benchmark.
...
The semantic compiler does not have a competing AST/emitter eligibility brain. Profiles are effective and exact-host proven for every promoted family, the external-provider route is executable and trust-bound, corpus policy has one owner, and the fixed benefit lane invokes emitted commands across three unrelated scenario families. The remaining product risks are broad module-scale semantic coverage, Strict stateful-module ownership, the last external-provider ecosystem exercise, physical target promotion, and authorized public lifecycle proof. Those gaps must close through the existing owners rather than creating parallel behavior as coverage expands.

## Product maturity gate — Partial / Milestones 16 and 18–23
...
- [ ] Milestone 23 must progress from its live Hybrid state and first terminal-region slice to broader continuation/error/transfer-safe compilation, real-workload coverage, and Strict stateful-module closure without adding a second state or eligibility owner. Performance optimization is a later phase.
