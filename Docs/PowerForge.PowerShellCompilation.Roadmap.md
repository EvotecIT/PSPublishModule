...
Exit gate: **Partial.** Bounded WMI/CIM/CDXML, administration-style, native/process, COM, and signed external binary-module examples are preserved through declared Hybrid routes; one direct typed local CIM query proves an adapter foundation, and the generic executable-provider trust boundary is now available. Generated Strict management-provider execution and the remote management operation/failure matrix remain required. Deterministic planning and hosted execution are not presented as native compilation.
