...
| 18. Establish versioned semantic oracles | Partial / minimized host-case foundation | Named profiles govern target identity and compilation; observations carry integrity-bound executable/runtime identity, promotion rejects missing pins or unexplained differences, and 19 native cases execute across all three host profiles; 13/19 cases have matching runtime-free results while 6 remain open |
...
Exit gate: **Partial / bounded schema-3 and thirteen runtime-free cases complete.** Named profiles govern target identity, binding/lowering, providers, caches, artifacts, diagnostics, compatible `#requires`, the profile-fixed PowerShell major version, bounded compiler-owned dictionary flow, compile-time-safe literal conversion, stable-string interpolation, local function graphs, typed/defaulted parameters, exact/alias/abbreviated parameter binding, index/member assignment targets, typed integral compound arithmetic, comparison operators, and logical operators; schema-3 observations integrity-bind exact executable/runtime identity, reject contradictory/unbounded evidence, preserve ordered values/cardinality/streams/errors/state and original property order, and promotion fails closed on unpinned or unexplained results. Every promoted family has a native minimized case executed across the three immutable pinned hosts; those thirteen features now also have artifact-hash-bound Strict-executable evidence, and upstream patch changes produce review-only evidence without changing a pin. The gate closes when explicit `AutomationNull` identity, the framed nullable/string/culture-sensitive protocol, directly observed process effects, and the remaining 6 runtime-free case observations are complete.
