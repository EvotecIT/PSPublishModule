...
| 18. Establish versioned semantic oracles | Partial / minimized host-case foundation | Named profiles govern target identity and compilation; observations carry integrity-bound executable/runtime identity, promotion rejects missing pins or unexplained differences, and 19 native cases execute across all three host profiles; immutable reviewed pins and matching runtime-free results remain open |
...
- [ ] Pin the exact executable host artifact for every promoted profile—patch/build, edition, OS, architecture, culture, feature switches, executable hash, and upstream commit/release identity—instead of a broad version range or executable name. The schema-2 harness now records and integrity-hashes all of these fields, accepts an exact executable path, and rejects replay or promotion when the recorded host-artifact identity differs; the reviewed immutable pin set for every promoted profile remains open.
- [ ] Preserve and compare structured output value/type/null/`AutomationNull`/cardinality/enumeration state, original property order, all stream records, structured error identity/category/termination, exit code, culture/encoding, and declared filesystem/process effects without reducing them to formatted text or a constant empty effect list. Schema 2 now preserves original selected-property order, cross-stream sequence, information tags, error identity/category/type/termination, success type/sequence, exit code, culture, and filesystem effects. Explicit no-output/null/`AutomationNull` cases, encoding/cardinality detail, and observed process effects remain open.
- [ ] Add a minimized executable oracle case and exact observed-host matrix for every promoted semantic feature. Nineteen native `.ps1` resources now cover all currently promoted feature families, provenance resolves to real applicable case IDs, and the complete 19-case matrix executes on Windows PowerShell 5.1, PowerShell 7.4.18, and PowerShell 7.6.x. The gate remains open until immutable reviewed host pins and corresponding runtime-free CLR observations are recorded for every case/profile claim.
...
Exit gate: **Partial / exact-host and minimized-case foundation complete.** Named profiles govern target identity, binding/lowering, providers, caches, artifacts, diagnostics, and compatible `#requires`; schema-2 observations integrity-bind exact executable/runtime identity, preserve ordered structured streams/errors and original property order, and promotion fails closed on unpinned or unexplained results. Every promoted family now has a native minimized case executed across the three host profiles. The gate closes when the immutable host-pin set, corresponding runtime-free observations, complete null/cardinality/encoding/process-effect envelope, and automated upstream-change lane are complete.
