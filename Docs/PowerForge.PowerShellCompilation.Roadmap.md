...
The [next major milestones, 24–31](PowerForge.PowerShellCompilation.NextMilestones.md), define the delivery tranche: values/collections, complete pipelines, stateful modules, practical CLR libraries, the development loop, broader complete-workload coverage, ordinary command forms, distribution/platform qualification, and measured acceleration. M24–M29 have completed bounded local gates. M29a now addresses common authored forms that still fall back before M30 distribution work. Interactive source debugging is a deferred developer-tooling qualification, not a gate on compiler coverage. The architecture history below remains their foundation.
...
M24–M28 have completed bounded value, pipeline, state, region, local-feed consumption, and source-first development-loop gates. M28 includes executable run/watch through the existing project and cache owners, Strict library NuGet packaging through the independent-rebuild packer, grouped project diagnostics from canonical decisions and bound local calls, and a Hybrid module consumer quickstart on both supported Windows PowerShell hosts. Failed rebuilds do not execute stale artifacts, and launch rechecks this attempt's authenticated output inventory. Source paths/PDBs and `debug-plan` are implemented, but interactive debugger behavior is unqualified. M29 subsequently completed its pinned local portfolio gate with 30 newly emitted and differentially executed module functions, three module workflows, and an offline standalone workflow; this is not general language coverage. M29a prioritizes everyday command shapes using fresh blocker and complete-workflow evidence. The [assessment](PowerForge.PowerShellCompilation.Assessment.md) identifies revisions, findings, and executable evidence; the [next milestones](PowerForge.PowerShellCompilation.NextMilestones.md) own the active checklist.
...
| Strict CLR library | Candidate with clean-consumer proof, callable ABI schema 4 for static libraries and schema 5 for stateful libraries, and fail-closed managed-closure certification | No for a certified artifact | Managed DLL | Direct use from C# and other CLR hosts |
...
`[OutputType()]` is useful contract evidence but not proof by itself because ordinary PowerShell does not enforce it. Authored `[OutputType([void])]` is advisory metadata and does not suppress success output actually produced by a function body. It is not a local-call, lifecycle-output, or recursive value contract; body inference remains authoritative, and direct self-recursion still requires one verified non-void contract. The canonical metadata policy separates authored output declarations from the optional single-CLR-type semantic contract. Hybrid binary cmdlets can preserve non-whitespace literal names and resolved CLR type lists, including repeated attributes and one literal parameter-set association per attribute, on advisory-metadata-capable targets. Strict runtime-free targets still require a target-compatible single semantic type; unsupported advisory collections, unresolved type expressions, dynamic names, and mixed forms fail closed. Validation attributes narrow accepted input but do not silently change the underlying type. Profile-guided observations and census data may prioritize work; they never specialize a Strict artifact around values seen during training.
...
| 6. Define runtime-free artifact contract and managed ABI | Complete | Strict publication is fail-closed and ABI schemas 4/5 carry value state, output/null/cardinality, streams, and binding semantics |
...
| [24. Values, collections, and error continuation](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-24--preserve-values-collections-and-error-continuation) | Complete / bounded qualified families | Numeric promotion, cardinality, fresh-vector ownership, enumeration failure, cleanup, and continuation have dual-host workflow proof |
| [25. Complete pipelines and advanced functions](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-25--compile-complete-pipelines-and-advanced-functions) | Complete / bounded qualified families | Whole commands preserve every record, local-call output, binding, streams, stop, cancellation, and cleanup through admitted owners; stopping fixtures wait for native engine acknowledgment |
| [26. Stateful modules and multiple Hybrid regions](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-26--compile-stateful-modules-and-multiple-hybrid-regions) | Complete / bounded M23 contract | Hybrid keeps one state owner while selecting approved regions from each canonical non-overlapping run; M24 value/local-call contracts compose with those regions, and Strict stateful libraries have an independent runtime-free lifetime owner |
| [27. Practical CLR library consumption](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-27--make-compiled-clr-libraries-practical-to-consume) | Complete / bounded local-feed consumption | Static/stateful consumers, deterministic rebuilds, cancellation-safe publication, canonical NuGet metadata, and required ABI/provider gate families; consumer upgrades require restore/rebuild |
| [28. Source-first development loop](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-28--deliver-a-source-first-development-loop) | Complete / bounded local workflows | Run, incremental rebuild, explain/diagnose, package, and two-host module consumption use the existing project service; interactive debugging is deferred and unqualified |
| [29. Complete script and module coverage](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-29--broaden-complete-script-and-module-coverage) | Current / multi-wave | At least 30 additional executed complete functions on the fixed packet, plus two module workflows and one script/application workflow with differential proof |
| [30. Distribution and qualified platforms](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-30--qualify-distribution-providers-and-platforms) | Partial / external gates | M19/M21/M22 close for the declared release set and actual supported targets |
| [31. Acceleration of proven workflows](PowerForge.PowerShellCompilation.NextMilestones.md#milestone-31--accelerate-proven-workflows) | Planned / later optimization | Useful complete workloads receive measured performance improvements after semantic qualification |
...
Exit gate: **Complete for bounded M24–M26 contracts.** The continuation implements guarded prefixes, approved regions selected from non-overlapping canonical runs, schema-6 transfer ownership including fresh stable-vector construction, bounded local collection factories, return/cardinality contracts, Hybrid-native computed members, sequential output, complete hosted multi-stage pipelines, parent-owned Hybrid state, and independent runtime-free Strict state lifetime. Dual-host matrices cover values, identity, streams, partial failure, stopping, cancellation, cleanup, and continued execution. The earlier M26 checkpoint recorded 1,060 passing compiler cases and six Strict Windows programs; the assessment records the later corrective qualification. At that checkpoint, the pinned census remained 188/282 emitted functions and 36 promoted regions because it contained no third eligible disjoint run. The assessment records subsequent workload expansion separately. Borrowed compiled reference mutation, runtime-free dynamic object/ETS access, arbitrary enumeration, computed method names, null-conditional access, and provider-bound live dependencies remain fail-closed or later work. Performance is not an admission gate.
...
1. [ ] Complete M29 against the pinned multi-module portfolio: retain the 196/282 PSSharedGoods baseline and its qualified eight `-as` functions, add PowerInfoblox, another Evotec module, two independent third-party modules, and a standalone script/application. Investigate every per-function regression before refreshing a baseline. Gain at least 30 differentially executed complete functions across three module packets and four recurring blocker families, including PowerInfoblox and a third-party module; prove three module workflows and one script/application workflow. Keep Hybrid fallback, Strict rejection, and co-blockers explicit. The PSSharedGoods-only 226/282 target is a trend, not the ecosystem gate.
...
Keep generic `Write-*` stream identity, preferences, ordering, and continuation intact. Basic functions that collide with generated common parameters retain their authored surface until the wrapper contract is proved. Do not translate syntax in emitters, create product-specific compiler policy, or use a census count as proof of complete execution.
...
The semantic compiler does not have a competing AST/emitter eligibility brain. Profiles are effective and exact-host proven for promoted families at their recorded checkpoints, the external-provider route is executable and trust-bound, corpus policy has one owner, and the fixed benefit lane invokes emitted commands across three unrelated scenario families. Remaining product work covers broader semantic families, the source-first development loop, integration, host servicing, the last external-provider exercise, physical target promotion, and public upgrade/lifecycle proof. Keep those changes in the existing owners.

## Product maturity gate — Partial / development loop and distribution qualification
...
- [x] M23 closes through the bounded M24–M26 continuation/transfer, full-module workflows, and independent Strict stateful lifetime contracts. Broader unsupported families remain explicit boundaries.
