...
NuGet packing requires Strict library mode, emitted source, and explicit package metadata. Signed compiler inputs are not supported by the independent-rebuild packer. Cancellation before publication or a failed integrity/ABI/rebuild check preserves the prior package; a completed publication returns success. Ctrl+C cancels the CLI rebuild and its owned process tree. M30 covers signed distribution, public feeds, long-path qualification, clean-machine deployment, and additional platforms.
