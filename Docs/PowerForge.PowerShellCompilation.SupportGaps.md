...
The first implementation target is the bounded object-valued `switch` family because it has pure representative paths in two modules. A simple string-looking source variable is not sufficient proof: native function binding deliberately stores variables as `Object`, and an unsafe cast would change pipeline and comparison behavior. The compiler must either prove a narrower condition from the authored parameter/local contract or keep the function hosted. The per-function ledger and same-input explain/build evidence decide whether the new rule actually promotes a function.
