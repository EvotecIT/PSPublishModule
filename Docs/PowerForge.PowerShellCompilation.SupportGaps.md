...
| PSScriptTools `[OutputType('Name')]`, multiple types, or `ParameterSetName` | Advisory `Get-Command` output metadata, not a value guarantee | The current single-resolved-CLR-type metadata contract rejects many unchanged functions. The next design must preserve authored metadata separately from inferred return values. |
