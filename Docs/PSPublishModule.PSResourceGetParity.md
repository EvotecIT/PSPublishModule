...
| `Uninstall-PSResource` | `Uninstall-ManagedModule` | Supported for modules | Name/wildcard or typed installed input, exact/range version targeting, prerelease, scope/custom root, profile/edition-aware cross-root dependency and loaded-module safety, atomic batch preflight, planning, `WhatIf`, and `Confirm`. |
