...
- Markdown rendering also injects default `loading=\"lazy\"` and `decoding=\"async\"` on `<img>` tags when missing, so markdown image syntax (`![](...)`) gets sane defaults even before optimize runs.
