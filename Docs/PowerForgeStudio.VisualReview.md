...
| Build & Run | Validated contracts, reviewed local actions and the available build action are visible; the inspector follows the current working copy and captured build, while result paths fit compact rows. Recognized secret arguments are removed from planning errors. Native JSON-only and script-only builds completed through this page; the script-only plan and build wrote separate evidence files. The compact headless result keeps the receipt and output dock readable. | Check compact native progress and error presentation after a failed build. |
