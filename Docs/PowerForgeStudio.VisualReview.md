...
- [x] Selecting a project, including through Ctrl+K, expands its working-copy tree and top-level Build folder so scripts and JSON configuration are visible beside Overview. Script nodes use a distinct vector gear. A manually collapsed Build folder stays collapsed on re-selection and after restoring saved tree state; completing a pending directory read does not reopen it. Wide and compact headless captures were inspected in `Artifacts/StudioValidation/tree-selected-expansion/`.
