...
| Build & Run | Validated contracts, reviewed local actions and the available build action are visible; the inspector follows the current working copy and captured build, while result paths fit compact rows. Recognized secret arguments are removed from planning errors. A native JSON-only build produced a NuGet package and scrolled its completed receipt into view. The compact headless result keeps the receipt and output dock readable. | Verify script-backed planning separately. |
