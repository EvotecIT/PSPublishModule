...
| Project GitHub | Issues, PR details, checks, files and reviewed actions are functional; the context panel follows the selected item. PR patch excerpts use the same diff viewer without implying that omitted upstream content is available. Compact PR files scrolls the page to show the complete visible hunk in a useful diff pane. | Reduce the empty space in the list/detail split and check compact discussion in the final native pass. |
