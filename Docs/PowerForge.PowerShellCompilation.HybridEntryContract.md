...
A same-body Hybrid `net10.0` explanation confirms why the private method matters. `param([string] $Pattern = 'yyyy'); Get-Date -Format $Pattern` has an eligible but hosted script root. Moving the body into a basic `Get-FormattedDate` function remains hosted under `binary-module.cmdlet-shape`: the command-host behavior cannot be exposed through that function's loose common-parameter contract. Adding `[CmdletBinding()]` makes the named method emit with one hosted command region, but would change the source's binding semantics. The root compiler must lower to a private entry method without manufacturing authored advanced-function metadata or using the public cmdlet descriptor as its eligibility test.
