...
Install the reviewed PowerForge CLI from the pinned engine checkout and run the publisher script directly from that root-owned checkout. Give each lane its own capture account, mode-0700 work directory beneath `/var/lib/powerforge-<lane>-backup`, mode-0600 write deploy key scoped to the backup repository, and a root-owned known-hosts file that the account can read. The root-owned systemd environment file at `/etc/powerforge/server-backup/<lane>.env` supplies these paths and values:
