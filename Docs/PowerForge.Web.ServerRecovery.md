...
`server bootstrap-plan` generates a reviewable markdown plan, JSON plan, and LF-normalized shell script draft for rebuilding a fresh Ubuntu host. The preflight checks the declared Ubuntu release and CPU architecture instead of continuing on a mismatched host. `packages.dotnetSdks` accepts the historical .NET Core apt bands through 3.1, with bare `1`, `2`, and `3` normalized to their `.0` bands, and modern major or major.0 values from 5 onward. PowerForge configures the official Microsoft Ubuntu package repository once before installing normalized `dotnet-sdk-<major>.<minor>` packages or PowerShell; the PowerShell path currently requires an x64 target with an explicit Ubuntu release. Managed accounts are created before owned directories. Repositories may declare `sshIdentityFile` and `sshKnownHostsFile`; PowerForge then applies both to clone and fetch through a strict `GIT_SSH_COMMAND` with `IdentitiesOnly` and host-key checking. Both paths must also appear in `bootstrapRequiredFiles`, so a private clone fails closed until its isolated key and pinned host keys have been restored. Set repository `ref` to an immutable commit or tag when recovery must restore an exact engine or application revision after cloning the configured branch. A repository using `refCaptureCommandId` also fails closed when a source manifest has not yet been hydrated; executable recovery plans must use the captured manifest. A managed file may declare a repository-owned `source`; PowerForge installs it only after repositories are available, rejects any target that overlaps a repository, rejects symlinked or canonically escaping sources at runtime, and rejects sources or targets that overlap declared secrets or encrypted capture paths. Set `validation` to `sudoers` for a root-owned `0440` file directly below `/etc/sudoers.d`; bootstrap validates the candidate and the complete sudoers policy, restoring the previous file if validation fails or the replacement is interrupted. Secret steps are rerunnable presence guards: they stop with an operator-facing restore instruction while state is missing and continue without exposing values after restoration.
