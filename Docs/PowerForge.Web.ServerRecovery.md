...
`server deploy` runs the manifest's deploy commands over SSH while holding every declared operation lock for the complete command set. Use `--dry-run` to print the resolved remote commands without executing them, and `--fail-on-failure` when automation should exit non-zero if a required deploy command fails.
