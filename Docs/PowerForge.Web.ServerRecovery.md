...
When a manifest contains both capture and deploy work, declare the same exact `/var/lock/*.lock` paths used by the host deployment commands in `operationLocks`. PowerForge creates and verifies root-owned mode-644 regular lock files, acquires locks in deterministic order, holds every declared lock for the complete capture, engine-owned `server deploy`, or generated bootstrap mutation window, and fails the operation if the remote lock session disappears. Command-owned deployment is the explicit exception described above. Apache `sites` and `conf` entries may declare `enabled: true` or `enabled: false`; bootstrap and inspect then manage both file content and activation state without caller-side `a2ensite` or `a2enconf` shell steps. Generated HTTPS sites begin disabled, then the shared certificate-aware activation helper enables them only when certificate material is available.
