...
`-Cleanup OldVersions` is executable, not plan-only. Delivery completes first, repair inventories the same estate again, and cleanup is replanned from that current state so a newly installed latest version can make its former fallback eligible for removal in the same run. Every removal still names one exact installed directory. The managed uninstall engine merges current-runspace loaded-module evidence even when `-IncludeLoaded` was omitted and validates dependencies across the relevant visible roots in the same edition/profile estate. Exact-path, `ShouldProcess`, `WhatIf`, and confirmation safety remain active. Repair stops cleanup after an operational failure, inventories the same roots one final time, and returns `ExecutionSucceeded`, `Converged`, and post-apply inventory, plan, and test evidence.
...
- [x] Add repair/maintenance coverage for current-runspace loaded-module safety, exact-path cleanup execution, post-delivery cleanup replanning, cross-root dependency blocking, stale-estate revalidation, artifact-plus-destination inventory, multiple editions/profiles/physical roots, and post-apply convergence.
