...
Focused tests cover local-feed presence and removal, a GitHub release probe, and refusal to probe an unclassified receipt. Private module-owned feeds with JSON `PackageBuild` or referenced `ProjectBuild` lanes now reload only the current feed address for an in-memory probe. The address must uniquely match the saved secret-free destination; changed or ambiguous lanes fail before network access. Script-backed modules still fail closed when their saved destination omitted URL credentials or query values, because verification does not rerun the module script. No external release or feed was changed by these tests.
