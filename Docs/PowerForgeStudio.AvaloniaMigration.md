...
Required next storage work: the existing publication receipt primary key is (session_id, root_path, target_name, target_kind). Two destinations for the same named target/kind collide. Finalization rolls back and retains pending evidence, but cannot save that valid multi-destination result until the schema supports distinct receipts. Migrate this shared table transactionally and prove existing receipt preservation, multiple destinations, idempotent initialization, and failure rollback before connecting publication UI.
