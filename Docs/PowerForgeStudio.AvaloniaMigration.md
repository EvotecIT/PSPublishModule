...
Focused tests cover local-feed presence and removal, a GitHub release probe, and refusal to probe an unclassified receipt. Private module-owned feeds whose saved destination omits credential-bearing URL parts still need a safe configuration rehydration path; the current verification fails closed rather than probing an unauthenticated or different endpoint. No external release or feed was changed by these tests.
