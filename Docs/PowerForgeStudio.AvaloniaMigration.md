...
The automation inspector now selects an attention item on first load, preserves a selected item by ID across refresh, and selects another visible item when a filter excludes the previous one. It shows the provider's current state, verified next run (or explicitly says none is verified), last run and result beside the schedule. A short “Last run” table heading remains readable when compact Details is open. Focused behavior checks and wide/compact Avalonia renders are in `Artifacts/StudioValidation/automation-inspector-20260923/`; GitHub cron text is still never converted into an invented next-run time. The full 84-test Avalonia suite passed after a bounded cleanup retry for a transient Windows lock on a linked-worktree test fixture; its isolated behavior test also passed.

A selected GitHub schedule can now open its local workflow YAML in the owning project's Files view. Studio rechecks that the file still exists beneath the discovered project and workspace, requires the `.github/workflows` location and matching project identity, and rejects linked path components before navigating. A missing source leaves the Automations route in place with an error. Windows Task Scheduler rows do not offer this file action. The route uses the existing document/file surface, so it does not launch a script, browser or provider mutation. Focused behavior checks and wide/compact renders are in `Artifacts/StudioValidation/automation-source-20260923/`; the full 84-test Avalonia suite passed.
...
Workspace-root registration beyond the folder picker, exclusions, groups, retention, runtime/terminal profiles, keyboard mappings, themes, scaling, provider binding edits and diagnostic export remain planned settings. Non-active recent roots can now be forgotten from Settings; active roots and roots used by retained profiles stay listed. Forgetting changes only the machine-local recent list and preserves the directory, files and saved explorer state. Native Windows keyboard, pointer, scaling and file-picker behavior remain unverified.
...
The Changes page now rereads Git state when opened and when the Studio window regains focus after its first activation. A refresh retains the selected staged or working-copy path if it still exists, reloads its patch, and clears selection when the path disappears. It skips focus refresh during an active Git read or mutation and does not scan Git while another page is visible. A real-repository Avalonia test covers an external file addition, retained and removed selection, inactive-page behavior and refresh on returning to Changes; wide and compact renders are in `Artifacts/StudioValidation/git-external-refresh-20260923/`. Native window reactivation remains to be checked; the headless host did not raise an activation event when a window was hidden and shown.

A later refresh correction keeps the previous Git rows and patch visible while the replacement read runs. It dates that evidence, disables Git mutations during the read, and, if the read fails, marks the prior snapshot stale and keeps actions disabled until a successful retry. The selected path at completion wins if the user changed selection during the read. A controlled real-repository test pauses and fails the Git status probe, checks the retained row and disabled action, then verifies recovery and a refreshed patch. The rendered stale state is in `Artifacts/StudioValidation/git-stale-refresh-20260923/`. All 86 Avalonia tests passed after this correction.
...
Within an approved unified release, WinGet submission receipts identify each attempted package ID, version and manifest. The shared submitter passes cancellation to the command runner and stops before starting another package; the release result retains completed entry outcomes when cancellation follows a command. It also stops at the first failed package, so later packages have no attempted receipt. Any failed or cancelled WinGet command requires reconciliation before replay; completed commands may already have caused a remote effect. A successful command records submission only. Studio verification reports upstream manifest acceptance and catalog availability as unverified until a provider check exists.

WinGetCreate may emit a `microsoft/winget-pkgs` pull-request URL even when its process exits with an error. PowerForge extracts only that repository's numeric PR URL and drops query or fragment text from captured output. Studio saves the recognized link on the package receipt and offers Open WinGet PR for the selected receipt, including a failed command that needs reconciliation. Interactive authentication keeps the console streams attached for prompts, so its PR URL is not captured; the operator must find that submission upstream. The link records where to inspect upstream review; it does not turn submission or a merged PR into verified catalog availability.

Controlled tests cover a successful command, an error exit after a PR link, a lookalike host, cancellation, receipt persistence and the safe Open WinGet PR action. Wide and compact submitted/failed receipt renders were inspected in `Artifacts/StudioValidation/winget-review-link/`. No live WinGet submission or catalog readback was performed.
...
### Recent workspace root management
...
### Disposable NuGet feed publication and readback (2026-09-22)

A disposable .NET library is now restored and packed, then passed through Studio's default project-build publication adapter to a local NuGet feed. The test confirms the reviewed destination, the published receipt and exact package bytes, runs Studio's default verification service against the recorded publication checkpoint, and restores a consumer from that feed. The shared verification host resolves absolute local paths and `file:` feed URIs, checks the package identity and compares SHA-256 with the approved digest captured at signing. A missing or changed feed package fails verification. If the source artifact has been removed, the saved digest still permits byte-level verification; an older checkpoint without that digest cannot pass local-feed verification after source removal.

Relative local feed paths are resolved from the source package directory, matching the push client. This proves the local package path without external credentials. The signing receipt in the test is an integrity checkpoint for the disposable fixture, not cryptographic signing. Authenticated HTTP feeds, public NuGet availability, GitHub releases, PowerShell repositories and native release-page interaction still require separate proof. A native Windows launch this day rendered the populated project tree and Overview against the worktree; the task-owned process was closed before returning to code work.

### Module-owned package verification

Module-owned package publication records both NuGet packages and GitHub releases under the `ModulePackages` release lane. Verification routes a published `.nupkg` receipt with a saved package identity through the NuGet verifier, and a named GitHub release receipt with a tagged release URL and saved asset inventory through the GitHub verifier. The package route matches the `ModuleBuild` signing receipt by artifact path so the approved digest survives the module lane's `UnifiedRelease` publish label. Per-project GitHub publication saves one receipt per result with its actual release URL, retaining successful and failed results separately. A published receipt that identifies neither stays failed and unverified. The original lane identity remains on saved verification receipts and in the journal.

Focused tests cover local-feed presence and removal, GitHub release asset inventory readback, and refusal to probe an unclassified receipt. New GitHub receipts retain the expected asset names and byte sizes across checkpoint and database storage. Verification reads the published tagged release from GitHub's API and fails when an expected asset is missing or has a different size; historical URL-only receipts remain unverified. This readback checks metadata, not byte-for-byte content, and the current unauthenticated route covers public GitHub releases. Private module-owned feeds with JSON `PackageBuild` or referenced `ProjectBuild` lanes now reload only the current feed address for an in-memory probe. The address must uniquely match the saved secret-free destination; changed or ambiguous lanes fail before network access. The shared NuGet verifier carries a service-index query to a package URL only when the package endpoint has no query of its own and remains on the same origin. A signed package-base URL keeps its own query, and a cross-origin package endpoint receives no query from the index. Script-backed modules still fail closed when their saved destination omitted URL credentials or query values, because verification does not rerun the module script. No external release or feed was changed by these tests.

### Recent GitHub release downloads

The project Releases page can refresh a read-only GitHub catalog for the selected working copy. It shows at most ten recent releases, each asset's reported download count and the sum when GitHub provides a complete asset inventory. A full or incomplete listing is labeled as such, and refresh failures preserve the prior snapshot with an observation time and stale-data notice. Switching working copies clears the previous project's counts and discards late responses. The view uses the existing bounded GitHub client and authentication source; it does not publish, download assets or store credentials.

Controlled HTTP tests cover the bounded request, paging indication and missing download counts. A headless Releases test covers the displayed totals, compact scrolling, working-copy switch race and stale-data notice after a failed refresh. The wide and compact renders are in `Artifacts/StudioValidation/github-release-downloads/`. A read-only live GitHub API query found current PowerForge release assets, but the new Studio panel was not used to change a remote release. Native Windows input for this panel remains unverified.

### Changed-project tree filter

The project tree now has the Changed view from the approved workspace concept. Selecting it reads local Git state for every discovered primary checkout and registered non-bare worktree, then retains only project groups with observed staged, unstaged or untracked files. The scan uses four-repository concurrency, a bounded Git command timeout, 64K-character status and 256K-character worktree-list capture limits, cancellation and no remote access. An oversized status remains conservative changed evidence instead of becoming false-clean. Its dated summary reports working copies that could not be checked instead of treating them as clean. The prior observation remains visible during refresh. Switching workspaces cancels and versions the old scan, clears a catalog from the prior root and disables Changed until discovery installs the new catalog.

Real disposable Git tests cover changes in a primary checkout and linked worktree, exclusion of a clean repository, and a linked working copy backed by a bare common repository without a false partial-scan warning. A controlled process test proves the capture limits, conservative oversized-output handling and absence of branch enumeration. Avalonia tests cover first-scan cancellation, preservation of unfiltered projects before initial evidence, partial results, stale evidence during refresh, late-result replacement and a paused workspace switch that cannot scan the prior catalog. Wide and compact headless renders were inspected in `Artifacts/StudioValidation/changed-project-filter/`. Native Windows input for this chip remains unverified.

### Reversible project archiving

The project header and context panel now expose Archive project for the selected repository. Archive state is a machine-local explorer preference inside the existing `workspace-roots.json`; Studio does not move, delete, clean or rewrite the repository. Archived projects leave the active Favorites and Other projects groups and appear below them in a counted, collapsed Archived group. Their favorite state is retained, so Restore project returns a favorite to Favorites. The title-bar repository jump can still locate an archived project, expands its Archived parent so selection remains visible, and exposes its Restore action.

Persistence tests cover JSON round trips, compatibility with older records, preservation through session/profile writes and unknown top-level fields. A headless Avalonia test archives a real local Git repository, verifies that its README remains present, reopens Studio from saved state, renders wide and compact trees, restores the project and confirms its favorite placement. The inspected renders are in `Artifacts/StudioValidation/archived-projects/`. Native Windows input for these actions remains unverified.

### Native shell status and project actions (2026-09-22)

The Release executable launched against a disposable one-repository workspace. Native Windows UI Automation selected that repository, read the observed `main`, `Clean`, `1 repository` and `Local workspace` evidence, invoked Project actions and found Add favorite and Archive project. No menu action was executed. The transient status text is configured as a polite automation live region and checked through the headless automation peer; spoken screen-reader output was not verified. The persistent branch, Git and repository fields remain ordinary read-only evidence.

A subsequent native Windows pass selected a disposable JSON-only .NET project, opened Build & Run, inspected its plan and built a real `NativeBuildFixture.1.0.0.nupkg` with PowerForge publication disabled. The completed build receipt and package path were observed through UI Automation. The first capture left the result below the center viewport, so Build & Run now scrolls the completed receipt into view. The final native capture in `Artifacts/StudioValidation/native-build/evidence/native-build-completed.png` shows the result, artifact rows, context and output together. A compact headless capture in `Artifacts/StudioValidation/native-build/final-headless/build-result-compact.png` shows the receipt and output dock remain readable without the inspector. Planning guidance also states that script-backed release JSON can invoke trusted PowerShell to export a plan and may have side effects; the tested fixture itself used JSON only. This evidence does not cover script-backed planning, signing or external publication.

The task-owned process closed after validation and the disposable repository was removed. `Artifacts/StudioValidation/status-strip/native-status-strip.png` captures the same native window, and `native-status-strip.json` records the named UI Automation elements and menu items. Windows `PrintWindow` omitted the final composited footer pixels even though UI Automation reported every named footer element within the window bounds and not off-screen. Wide and compact headless captures therefore remain the pixel evidence for the complete footer; the native run proves the real Windows accessibility and flyout path.

### Script-only project planning and build (2026-09-22)

Studio now invokes a discovered `Build-Project.ps1` directly for plan and build-only requests when no `Build/project.build.json` exists. A disposable script-only repository completed both actions through the native Windows Build & Run page, writing separate plan and build markers; the rendered result is in `Artifacts/StudioValidation/script-only-build/native-script-build.png`. Studio removes the previous plan before inspection and rejects a fresh plan whose PowerForge `Success` field is false, even if the script exits with code zero. Focused planner and command-host tests cover these cases. Studio Releases still requires reviewable JSON configuration for project publication. The disposable repository, fake module, generated plan and native test script were removed after validation.

### Native JSON-to-local-feed release (2026-09-22)

A native Windows run selected a disposable JSON-only .NET repository and completed Build & Run, Prepare, Sign, Inspect publication targets, explicit destination approval, Publish and Verify through the Releases page. The project JSON pointed only to a disposable local NuGet feed, with GitHub publication disabled. Studio's build step produced `NativeReleaseFixture.1.0.0.nupkg` without publishing. Its separate release-signing step used a one-day disposable self-signed code-signing certificate in the current-user store; `dotnet nuget sign` succeeded with the expected untrusted-root warning. The target review named the local feed before approval. Studio recorded Published and Verified receipts, and an independent SHA-256 comparison confirmed that the source and feed packages matched. No public registry or GitHub release was changed.

The first native capture showed that publication controls followed long progress and signing-receipt lists. Releases now places publication destinations and actions immediately after the release steps and saved-history controls; detailed artifacts, progress and receipts follow. A final native pass completed the same local-feed flow after that layout change, and the compact Windows captures were inspected at `Artifacts/StudioValidation/native-release-flow/evidence/native-release-review.png` and `native-release-verified.png`. Inspection now scrolls the destination review into view; a compact headless render at `release-destination-review-compact.png` shows the target, approval checkbox and Publish button together. All 78 Avalonia tests passed, including a focused check for that scroll behavior. The build JSON's `SignPackages=false` disables signing during Build & Run; this first native release pass selected the separate Releases certificate through the host environment. Native proof here covers the local NuGet feed and a test certificate, not a trusted production signature, authenticated feed, public NuGet listing, GitHub publication or PowerShell repository. The scroll refinement has headless visual proof; the native captures precede it.

### JSON-selected Studio release signing (2026-09-23)

Releases now takes `CertificateThumbprint`, `CertificateStore` and `TimeStampServer` from an unchanged project build JSON for project artifacts, falling back to host signing settings when JSON has no thumbprint. Other build adapters in a mixed release use the host setting. It validates the JSON against the build checkpoint before and after reading the certificate selection, and accepts only `CurrentUser` or `LocalMachine` as a certificate store. Changed or unreadable configuration requires a rebuild before signing. The page identifies the certificate source and prerequisite for each adapter, checks the local store during preparation, and disables Sign artifacts when any required certificate is unavailable. Signing rechecks every required certificate before changing any artifact. Certificate and credential values are not displayed. The full Studio gate passed with zero build warnings, 78 Avalonia tests and 903 shared Studio tests (one opt-in smoke test skipped); a repeated Windows temp-directory teardown race in an unrelated editor test was resolved with a bounded cleanup retry.

A native Windows run then built a disposable JSON-only .NET package, prepared its release, signed it with a one-day self-signed certificate selected solely through `Build/project.build.json`, reviewed the local NuGet feed, published there and verified matching package bytes. The Studio process had no `RELEASE_OPS_STUDIO_SIGN_THUMBPRINT`. The native prepared and verified captures are in `Artifacts/StudioValidation/json-signing/evidence/`. A separate compact headless capture under `Artifacts/StudioValidation/signing-readiness/` shows the unavailable-certificate message with Sign disabled. This proves JSON certificate selection for the local project package path; it does not establish a trusted production signature or public package availability.

### Local build folders in the project tree (2026-09-23)

Workspace discovery now includes immediate child folders with a supported PowerForge JSON or PowerShell build contract, even when they have no `.git` entry. A selected local folder opens Files and Build & Run, shows its own Local files node and project count, and reports that Git is optional for local build work. Project Changes, History and GitHub stay disabled until that folder owns a Git working copy; neither its Git status nor its remote can be borrowed from a parent checkout. The Changed tree filter skips local folders, and a workspace containing only local folders disables that filter. Activity and the project tree use the same discovery set, so their project counts agree. Activity does not raise a Git Guard warning merely because a local build folder has no Git entry. A workspace root containing a child Build folder is not misidentified as one large project.

Focused catalog and Avalonia tests cover mixed Git/local/plain folders, a directly selected local project, JSON entrypoint discovery, route guards, Changed filtering without parent-Git inheritance, and absence of a false Activity Git Guard warning. Native Windows UI Automation selected a disposable local JSON project, observed one project and disabled Git tabs, and opened Build & Run without executing the build. Wide and compact native captures are in `Artifacts/StudioValidation/local-project-tree/native-evidence/`. This pass validates project discovery, navigation and rendering, not package output or publication.

The real JSON-build test now reaches Build & Run through discovery and selection of a no-Git project. A separate native Windows run selected a disposable no-Git `NativeBuild` folder, generated a build-only plan, executed it and showed the completed receipt and live phases. The resulting `NativeBuildFixture.1.0.0.nupkg` contained `lib/net10.0/NativeBuild.dll`; the package and temporary project were removed after verification. The final capture and result are in `Artifacts/StudioValidation/local-json-build/evidence/`. No signing, feed push or GitHub publication was requested.

A no-Git integration path now continues from project discovery and the real JSON build through Prepare, destination inspection, explicit publication approval, a real push to a disposable local NuGet feed, and Verify. The feed package's SHA-256 matched the built package, and the feed was empty before Publish. This test uses a captured artifact-integrity checkpoint in place of a certificate signature; the separate native JSON-signing run above remains the proof for cryptographic signing. Releases keeps saved history available for a local folder, disables GitHub release refresh, and explains that GitHub metrics require a Git working copy. Compact headless captures of the completed release and the local-project GitHub state are in `Artifacts/StudioValidation/local-release-flow/curated/`. The test cleans its package, feed and release database; no public destination is contacted.
