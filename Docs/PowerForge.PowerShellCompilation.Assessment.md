...
| Language semantics | Bounded typed values, calls, control flow, closed collection transfers, zero-record versus explicit-null return contracts, conditional return and scalar/vector-alternative envelopes, commands, lifecycle, and closed stable-scalar `foreach` array captures through canonical IR | AutomationNull/dynamic empty-output closure, dynamic promotion, general enumeration, multiple success outputs, and broader pipeline/value contracts |
...
- The complete bounded compiler gate passed **975/975 tests** with zero failures or skips, then built and executed **6/6 Strict `net10.0` `win-x64` programs**.
...
- The pinned complete PSSharedGoods census passed without input failures: **284 source files**, **286 units**, **282 functions**, **141 emitted functions**, **51 promoted regions**, **73 candidates**, and **19 analysis opportunities**. Current schema-5 transfer evidence carries the `ClosedValueAlternative` continuation introduced in schema 4 for nine promoted regions. Schema-5 local-call evidence promotes unchanged prefixes in `Get-ObjectPropertiesAdvanced`, `New-SqlQuery`, `New-SqlQueryAlterTable`, and `New-SqlQueryCreateTable`; three are net-new promoted regions and one replaces or extends an already selected region. These are distinct coverage measures, not a PowerShell-language coverage percentage or a performance claim.
...
- [ ] Select the next grouped consumer family from real workflows—object/member/index access, retained collection mutation, or complete pipeline flow—and carry it through the same canonical owners; avoid one-type-at-a-time exceptions.
