...
Updated: 2026-09-17. The active continuation is preserved on `fix/compiler-audit-milestones` without a pull request. The current compiler support matrix is `net10.0`/PowerShell 7.6 plus `net472`/Windows PowerShell 5.1; `net8.0` is retired from forward-looking compiler targets because Microsoft support ends on 10 November 2026. Region selection is semantic rather than benchmark-driven: one-statement and additional statement-aligned regions are valid when their value, ownership, direction, output, mutation, error, ordering, and continuation contracts close. The transfer ABI now admits stable scalars, exact atomic `Hashtable`/`OrderedDictionary` references, one-dimensional stable-scalar arrays, read-only exact `System.Array`/`ArrayList`/`List<T>` references, and terminal `NoValue`/`NullValue` cardinality while continuing to reject direct `System.Object`/ETS values, arbitrary enumerators, unproved local collection ownership, and compiled collection mutation. A structured return-or-fallthrough envelope lets retained PowerShell execute an authored conditional return without the generated helper enumerating its value; it now distinguishes a bare return with zero success records from an explicit null return with one record. A closed non-nullable value-type scalar `foreach` may fill a fresh, exactly typed one-dimensional stable-scalar array capture when every success value and loop effect is proven. The unchanged PSSharedGoods `Split-Array`, `Get-ADEncryptionTypes`, `Get-ADTrustAttributes`, `ConvertTo-ImmutableID`, and `Find-DatesPastWeek` workflows and CleanupMonster `Get-CloudCacheComputer` preserve the admitted behavior on both supported hosts. The pinned 286-unit module census reports 48 promoted regions, 70 candidates, and 19 analysis opportunities. Region reconstruction also fails closed at target-runspace import: when the runspace cannot construct an optimized retained function, its authored declaration remains installed. The 2026-09-06 and 2026-09-07 sections below retain their original evidence dates, revisions, and pre-retirement measurements. None of these results establishes current public-package availability.
...
| Language semantics | Bounded typed values, calls, control flow, closed collection transfers, zero-record versus explicit-null return contracts, conditional return envelopes, commands, lifecycle, and closed stable-scalar `foreach` array captures through canonical IR | AutomationNull/dynamic empty-output closure, dynamic promotion, general enumeration, multiple success outputs, and broader pipeline/value contracts |
...
- The complete bounded compiler gate passed **935/935 tests** with zero failures or skips, then built and executed **6/6 Strict `net10.0` `win-x64` programs**.
- Focused original/generated collection and value-cardinality workflows pass on `net10.0`/PowerShell 7.6 and `net472`/Windows PowerShell 5.1. They cover atomic maps, stable-scalar vectors, `System.Array`, `ArrayList`, `List<object>`, zero-record bare returns, one-record explicit null returns, conditional return/fallthrough, ordinary and comma/no-enumerate output, downstream stopping, retained output/error continuation, remove/reimport, `Split-Array` cardinality, stable-scalar `foreach` capture in `Get-ADEncryptionTypes` and `Get-ADTrustAttributes`, and repeated invocation. Pinned `Find-DatesPastWeek` and `Get-CloudCacheComputer` explicit-null paths match original execution on both hosts. Existing injected-enumerator tests cover partial records plus `MoveNext`, `Current`, disposal, and error-action continuation at the retained PowerShell boundary.
...
- The pinned complete PSSharedGoods census passed without input failures: **284 source files**, **286 units**, **282 functions**, **141 emitted functions**, **48 promoted regions**, **70 candidates**, and **19 analysis opportunities**. The additional promoted region is the unchanged `ConvertTo-ImmutableID` terminal bare return. These are distinct coverage measures, not a PowerShell-language coverage percentage or a performance claim.
