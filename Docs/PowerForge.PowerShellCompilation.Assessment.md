...
| Language semantics | Bounded typed values, calls, control flow, closed collection transfers, commands, and lifecycle through canonical IR | Dynamic promotion, general collection enumeration/failure continuation, multiple success outputs, and broader pipeline/value contracts |
| Module-scale compilation | Live Hybrid state, guarded prefixes, additional statement-aligned regions, structured scalar/map/vector/list transfer contracts, and real same-function multi-region workflows | Control-flow envelopes, compiled collection mutation, local-call closure, and remaining Strict persistent-state/lifetime work |
...
- The complete bounded compiler gate passed **913/913 tests** with zero failures or skips, then built and executed **6/6 Strict `net10.0` `win-x64` programs**.
- Focused original/generated collection workflows pass on `net10.0`/PowerShell 7.6 and `net472`/Windows PowerShell 5.1. They cover atomic ordered maps, stable-scalar vectors, empty `ArrayList`/`List<object>` output, retained collection mutation, `Split-Array` cardinality and comma/no-enumerate output, and repeated invocation.
...
- The pinned complete PSSharedGoods census passed without input failures: **284 source files**, **286 units**, **282 functions**, **141 emitted functions**, **44 promoted regions**, **67 candidates**, and **19 analysis opportunities**. These are distinct coverage measures, not a PowerShell-language coverage percentage or a performance claim.
...
- [x] Replace the scalar/Hashtable special case with a closed collection-region ABI. The ten-crossing PSSharedGoods frontier now yields nine typed regions across atomic maps, stable-scalar vectors, and read-only list-like references; `Split-Array` supplies a real two-host multi-region workflow while its control-flow-dependent parameter-array branch remains explicitly retained.
- [ ] Close enumeration failure/disposal, partial-success continuation, and control-flow envelopes as one M24 capability batch. Keep dynamic objects, ETS, arbitrary enumerators, and compiled collection mutation fail-closed until their complete contracts exist.
