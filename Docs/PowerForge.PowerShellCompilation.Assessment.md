...
Updated: 2026-09-15. The active continuation is preserved on `fix/compiler-audit-milestones` without a pull request. Its reconciled candidate passes 885/885 compiler-category tests and all six Strict `net10.0` `win-x64` corpus programs. The canonical gate now caps xUnit collection concurrency at four and allows 120 seconds for its shared external-process probe, while still killing the full process tree on timeout. The current compiler support matrix is `net10.0`/PowerShell 7.6 plus `net472`/Windows PowerShell 5.1; `net8.0` is retired from forward-looking compiler targets because Microsoft support ends on 10 November 2026. Region selection is semantic rather than benchmark-driven: one-statement regions are valid when their contracts close, and the first wider boundary admits only a fresh guarded empty Hashtable plus exact read-only terminal passthrough. The unchanged PSSharedGoods `Get-ObjectEnumValues` workflow now proves two regions around retained mutation, including errors, explicit cancellation, identity, state, reuse, and removal/reimport on both supported hosts. The 2026-09-06 and 2026-09-07 sections below retain their original evidence dates, revisions, and pre-retirement measurements. None of these results establishes current public-package availability.
...
| Module-scale compilation | Live Hybrid state, guarded prefixes, terminal/complete-body helpers, exact scalar transfers, and a real same-function two-region Hashtable workflow | Wider/additional regions, mutable collection transfer, local-call closure, and remaining Strict persistent-state/lifetime work |
