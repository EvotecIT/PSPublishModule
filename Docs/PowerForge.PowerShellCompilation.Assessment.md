...
The Strict executable debugger fixture built and ran from a compact project path, returning `7`. Its generated C# has `#line` directives for authored lines, but the PDB records their stable 64-character document names under a temporary absolute build path, without a source checksum. The separate portable `source-map.json` relates those documents to `main.ps1`; a debugger mapping must account for the actual PDB document path. A 262-character generated executable path in a deeper ordinary user-profile temp directory failed to launch on Windows even though the DLL executed through `dotnet`; M29 retains long-path qualification. No authored-source breakpoint, step, local, stack, or exception observation is credited yet. PDB presence and static mapping are not counted as debugger proof.
