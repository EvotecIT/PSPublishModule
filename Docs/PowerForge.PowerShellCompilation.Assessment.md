...
Updated: 2026-09-17. The active continuation is preserved on `fix/compiler-audit-milestones` without a pull request. The current compiler support matrix is `net10.0`/PowerShell 7.6 plus `net472`/Windows PowerShell 5.1; `net8.0` is retired from forward-looking compiler targets because Microsoft support ends on 10 November 2026. Region selection is semantic rather than benchmark-driven: one-statement and additional statement-aligned regions are valid when their value, ownership, direction, output, mutation, error, ordering, and continuation contracts close. The transfer ABI now admits stable scalars, exact atomic `Hashtable`/`OrderedDictionary` references, one-dimensional stable-scalar arrays, and read-only exact `System.Array`/`ArrayList`/`List<T>` references while continuing to reject direct `System.Object`/ETS values, arbitrary enumerators, unproved local collection ownership, and compiled collection mutation. A structured return-or-fallthrough envelope now lets retained PowerShell execute an authored conditional return without the generated helper enumerating its value; retained PowerShell therefore owns partial records, error continuation, stopping, and enumerator disposal. A closed non-nullable value-type scalar `foreach` may now fill a fresh, exactly typed one-dimensional stable-scalar array capture when every success value and loop effect is proven. The generated region returns the array as a transfer value; retained PowerShell still performs the authored downstream enumeration. The unchanged PSSharedGoods `Split-Array`, `Get-ADEncryptionTypes`, and `Get-ADTrustAttributes` workflows preserve original/generated behavior on both supported hosts. The pinned 286-unit module census reports 47 promoted regions, 70 candidates, and 19 analysis opportunities. Region reconstruction also fails closed at target-runspace import: when the runspace cannot construct an optimized retained function, its authored declaration remains installed. The 2026-09-06 and 2026-09-07 sections below retain their original evidence dates, revisions, and pre-retirement measurements. None of these results establishes current public-package availability.
...
| Language semantics | Bounded typed values, calls, control flow, closed collection transfers, conditional return envelopes, commands, lifecycle, and closed stable-scalar `foreach` array captures through canonical IR | Dynamic promotion, general enumeration, multiple success outputs, and broader pipeline/value contracts |
...
- The complete bounded compiler gate passed **925/925 tests** with zero failures or skips, then built and executed **6/6 Strict `net10.0` `win-x64` programs**.
- Focused original/generated collection workflows pass on `net10.0`/PowerShell 7.6 and `net472`/Windows PowerShell 5.1. They cover atomic maps, stable-scalar vectors, `System.Array`, `ArrayList`, `List<object>`, conditional return/fallthrough, ordinary and comma/no-enumerate output, downstream stopping, retained mutation ownership, `Split-Array` cardinality, stable-scalar `foreach` capture in `Get-ADEncryptionTypes` and `Get-ADTrustAttributes`, and repeated invocation. Existing injected-enumerator tests cover partial records plus `MoveNext`, `Current`, disposal, and error-action continuation at the retained PowerShell boundary.
...
- The pinned complete PSSharedGoods census passed without input failures: **284 source files**, **286 units**, **282 functions**, **141 emitted functions**, **47 promoted regions**, **70 candidates**, and **19 analysis opportunities**. These are distinct coverage measures, not a PowerShell-language coverage percentage or a performance claim.
...
- [ ] Select the next coherent collection/value family from real workflows and carry it through the same canonical owners; avoid one-type-at-a-time exceptions.
