...
Updated: 2026-09-18. The active continuation is preserved on `fix/compiler-audit-milestones` without a pull request. The current compiler support matrix is `net10.0`/PowerShell 7.6 plus `net472`/Windows PowerShell 5.1; `net8.0` is retired from forward-looking compiler targets because Microsoft support ends on 10 November 2026. Region selection is semantic rather than benchmark-driven: one-statement and additional statement-aligned regions are valid when their value, ownership, direction, output, mutation, error, ordering, and continuation contracts close. The transfer ABI now admits stable scalars, exact atomic `Hashtable`/`OrderedDictionary` references, one-dimensional stable-scalar arrays, read-only exact `System.Array`/`ArrayList`/`List<T>` references, terminal `NoValue`/`NullValue` cardinality, one closed branch-dependent scalar-or-vector continuation local, and one source-proved zero-argument local call that returns a fresh atomic `ArrayList` while continuing to reject direct `System.Object`/ETS values, arbitrary enumerators, unproved local collection ownership, and compiled collection mutation. Compiler-owned envelopes keep conditional returns and closed scalar/vector alternatives atomic until retained PowerShell restores the authored statement or type constraint; generated helpers do not enumerate those values early. A closed non-nullable value-type scalar `foreach` may fill a fresh, exactly typed one-dimensional stable-scalar array capture when every success value and loop effect is proven. Hybrid native-function bodies may now read or assign computed instance members and read computed static members through PowerShell's own adapter binders; this is explicitly host-bound and does not widen Strict, typed-region transfer, or runtime-free library eligibility. The unchanged PSSharedGoods `Split-Array`, `Get-ADEncryptionTypes`, `Get-ADTrustAttributes`, `ConvertTo-ImmutableID`, `Find-DatesPastWeek`, `New-SqlTableMapping`, and `Set-XML` workflows, nine `Get-Computer*` property-selector workflows, four local-factory consumers, and CleanupMonster `Get-CloudCacheComputer` preserve the admitted structural contracts. The pinned 286-unit module census now reports 154 emitted functions, 49 promoted regions, 73 candidates, and 18 analysis opportunities. Two previously partial functions became complete native-bound CLR bodies, so promoted-region count fell without losing authored coverage. Region reconstruction also fails closed at target-runspace import: when the runspace cannot construct an optimized retained function, its authored declaration remains installed. The 2026-09-06 and 2026-09-07 sections below retain their original evidence dates, revisions, and pre-retirement measurements. None of these results establishes current public-package availability.
...
| Language semantics | Bounded typed values, calls, control flow, closed collection transfers, zero-record versus explicit-null return contracts, conditional return and scalar/vector-alternative envelopes, commands, lifecycle, closed stable-scalar `foreach` array captures, and host-bound computed member access through canonical IR | AutomationNull/dynamic empty-output closure, runtime-free dynamic objects/ETS, general enumeration, multiple success outputs, and broader pipeline/value contracts |
| Module-scale compilation | Live Hybrid state, guarded prefixes, additional statement-aligned regions, structured scalar/map/vector/list transfer contracts, one bounded local collection-factory call, return-or-fallthrough continuation, and real same-function multi-region workflows | General local-call collection closure, compiled collection mutation, and remaining Strict persistent-state/lifetime work |
...
- The complete bounded compiler gate passed **983/983 tests** with zero failures or skips, then built and executed **6/6 Strict `net10.0` `win-x64` programs**.
...
- The pinned complete PSSharedGoods census passed without input failures: **284 source files**, **286 units**, **282 functions**, **154 emitted functions**, **49 promoted regions**, **73 candidates**, and **18 analysis opportunities**. Thirteen unchanged functions moved from fallback to emitted native-bound CLR bodies with no emitted-function loss. `New-SqlTableMapping` and `Set-XML` are direct computed-member representatives; `Send-Email` clears its computed-member blocker but remains retained for an independent dictionary-escape contract. Two former partial regions and one analysis-only opportunity were absorbed by complete functions, explaining the lower region/opportunity counts. These are distinct coverage measures, not a PowerShell-language coverage percentage or a performance claim.
...
- [ ] Select the next grouped consumer family from the refreshed frontier—retained collection mutation or complete pipeline flow—and carry it through the same canonical owners; avoid one-type-at-a-time exceptions.
