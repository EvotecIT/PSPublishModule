...
Updated: 2026-09-19. The active continuation remains on `fix/compiler-audit-milestones` without a pull request. The forward-looking support matrix is `net10.0`/PowerShell 7.6 and `net472`/Windows PowerShell 5.1; `net8.0` is retired because its Microsoft support ends on 10 November 2026. Admission is semantic, not benchmark-driven: a generated method or region must preserve values, ownership, output, mutation, errors, ordering, and continuation. The transfer ABI still rejects direct `System.Object`/ETS values, arbitrary enumerators, unproved collection ownership, and compiled-owned collection mutation.

Hybrid native functions now select the existing PowerShell-owned lazy `foreach` contract for direct iteration over an authored untyped or `[object]` parameter. This does not make arbitrary enumeration runtime-free: the active host still owns enumerator acquisition, loop-variable storage, stopping, and authored cleanup. Typed collection loops keep their established route, and direct range iteration remains hosted pending its distinct lazy-range proof. Three pinned PSSharedGoods functions now emit complete methods. `Get-ProtocolDefaults` retains one promoted ordered-map prefix and a hosted map-key loop; exploratory whole-function admission also exposed a separate dictionary-literal escape guard. The complete pinned module census reports 174 emitted functions, 39 promoted regions, 69 candidates, and 12 analysis opportunities across 286 units, with no input failures.

Other qualified Hybrid paths remain narrow. Complete authored pipelines use native command regions so PowerShell owns discovery, binding, script-block scope, streams, lifecycle, errors, stopping, aliases, and retained collection mutation. Explicit success-stream sinks such as terminal `Out-Null` still fail closed, and compiler-owned invocations feeding hosted pipelines retain their typed-continuation route. An exact zero-parameter fresh-`ArrayList` factory is admitted through canonical IR and invocation-owned storage. Range and regex-match operations delegate to the active PowerShell host; target-compatible type literals lower to `System.Type`, and Hybrid can resolve selected target-incompatible conversion types in that host. Type-consuming invocations inside observed native `try/catch` remain hosted until method-wrapper identity is closed. Region reconstruction also leaves authored declarations installed when target-runspace import cannot construct an optimized alternative. The dated 2026-09-06 and 2026-09-07 sections below are historical evidence, not current package or release claims.
...
| Module-scale compilation | Live Hybrid state, guarded prefixes, additional statement-aligned regions, structured scalar/map/vector/list transfer contracts, one bounded local collection-factory call, complete hosted pipelines with retained collection mutation, return-or-fallthrough continuation, and real same-function multi-region workflows | General local-call collection closure, compiled-owned collection mutation, complex lifecycle closures, and remaining Strict persistent-state/lifetime work |
