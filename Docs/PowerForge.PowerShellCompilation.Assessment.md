...
Updated: 2026-09-18. The active continuation is preserved on `fix/compiler-audit-milestones` without a pull request. The current compiler support matrix is `net10.0`/PowerShell 7.6 plus `net472`/Windows PowerShell 5.1; `net8.0` is retired from forward-looking compiler targets because Microsoft support ends on 10 November 2026. Region selection is semantic rather than benchmark-driven: one-statement and additional statement-aligned regions are valid when their value, ownership, direction, output, mutation, error, ordering, and continuation contracts close. The transfer ABI continues to reject direct `System.Object`/ETS values, arbitrary enumerators, unproved local collection ownership, and compiled-owned collection mutation. Complete authored multi-stage pipelines use the existing Hybrid native command-region owner: the active PowerShell invocation preserves command discovery, parameter binding, script-block scope, streams, lifecycle, errors, stopping, aliases, and mutation of retained collection references while generated code continues around the pipeline. Explicit success-stream sinks such as terminal `Out-Null` retain their established fail-closed path instead of being treated as an ordinary output-producing pipeline. A compiler-owned invocation feeding a hosted pipeline keeps its narrower typed-continuation route rather than being replaced by whole-function hosting. The exact zero-parameter fresh-`ArrayList` factory contract now also closes complete Hybrid native consumers: canonical IR constructs the proved value and the existing native assignment owner stores it in invocation-owned PowerShell state, without calling the helper's parameter binder or introducing a name rule. The unchanged PSSharedGoods `New-SqlQuery`, `New-SqlQueryAlterTable`, and `New-SqlQueryCreateTable` workflows become complete native-bound CLR bodies; `Get-ObjectPropertiesAdvanced` retains its narrower promoted region. The pinned 286-unit module census now reports 159 emitted functions, 45 promoted regions, 73 candidates, and 17 analysis opportunities. Three former factory-call regions are absorbed by those complete methods. Region reconstruction also fails closed at target-runspace import: when the runspace cannot construct an optimized retained function, its authored declaration remains installed. The 2026-09-06 and 2026-09-07 sections below retain their original evidence dates, revisions, and pre-retirement measurements. None of these results establishes current public-package availability.
...
- The pinned complete PSSharedGoods census passed without input failures: **284 source files**, **286 units**, **282 functions**, **159 emitted functions**, **45 promoted regions**, **73 candidates**, and **17 analysis opportunities**. `New-SqlQuery`, `New-SqlQueryAlterTable`, and `New-SqlQueryCreateTable` move from their prior promoted-prefix state to complete native-bound CLR bodies; their three factory-call regions are absorbed. `Get-ObjectPropertiesAdvanced` retains its narrower factory-call region because its later ETS behavior has a separate owner. The remaining grouped frontier contains four `syntax.foreachstatement` sole blockers; three range-only plus one range/match co-blocked function; three match-only plus that same co-blocked function; three type-expression sole blockers; and five conversion sole blockers with heterogeneous causes. These are distinct coverage measures, not a PowerShell-language coverage percentage or a performance claim.
...
- [ ] Take range-and-match as the next grouped control-flow goal. Keep their semantic contracts separate, but qualify them together because three range-only and three match-only sole blockers plus one shared co-blocked workflow can close as one seven-function tranche. Preserve `$Matches`, regex conversion/errors, inclusive range order, endpoint conversion/overflow, slicing cardinality, single evaluation, and stopping.
