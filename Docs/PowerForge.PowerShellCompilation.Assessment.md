...
Updated: 2026-09-18. The active continuation is preserved on `fix/compiler-audit-milestones` without a pull request. The current compiler support matrix is `net10.0`/PowerShell 7.6 plus `net472`/Windows PowerShell 5.1; `net8.0` is retired from forward-looking compiler targets because Microsoft support ends on 10 November 2026. Region selection is semantic rather than benchmark-driven: one-statement and additional statement-aligned regions are valid when their value, ownership, direction, output, mutation, error, ordering, and continuation contracts close. The transfer ABI continues to reject direct `System.Object`/ETS values, arbitrary enumerators, unproved local collection ownership, and compiled-owned collection mutation. Complete authored multi-stage pipelines now use the existing Hybrid native command-region owner: the active PowerShell invocation preserves command discovery, parameter binding, script-block scope, streams, lifecycle, errors, stopping, aliases, and mutation of retained collection references while generated code continues around the pipeline. Explicit success-stream sinks such as terminal `Out-Null` retain their established fail-closed path instead of being treated as an ordinary output-producing pipeline. A compiler-owned invocation feeding a hosted pipeline keeps its narrower typed-continuation route rather than being replaced by whole-function hosting. This is a host-bound execution contract, not a new typed transfer or runtime-free pipeline engine, and it does not widen Strict. The unchanged PSSharedGoods `Get-MyIpAddress` and `Remove-DuplicateObjects` workflows become complete native-bound CLR bodies; `Get-ObjectEnumValues` retains its two promoted typed regions around the hosted mutation. The pinned 286-unit module census now reports 156 emitted functions, 48 promoted regions, 73 candidates, and 17 analysis opportunities. One prior promoted region is absorbed by a complete method and the other new method moves directly from fallback to emitted. Region reconstruction also fails closed at target-runspace import: when the runspace cannot construct an optimized retained function, its authored declaration remains installed. The 2026-09-06 and 2026-09-07 sections below retain their original evidence dates, revisions, and pre-retirement measurements. None of these results establishes current public-package availability.
...
- The pinned complete PSSharedGoods census passed without input failures: **284 source files**, **286 units**, **282 functions**, **156 emitted functions**, **48 promoted regions**, **73 candidates**, and **17 analysis opportunities**. `Get-MyIpAddress` and `Remove-DuplicateObjects` move from fallback to emitted native-bound CLR bodies with no emitted-function loss; `Get-ObjectEnumValues` retains its two narrower promoted regions. One former region and one pipeline opportunity are absorbed. The remaining `syntax.pipeline` frontier is five occurrences in three functions with one visible sole blocker. These are distinct coverage measures, not a PowerShell-language coverage percentage or a performance claim.
...
- [ ] Form the next grouped goal from the remaining real consumers. Assess bounded `foreach`/enumeration, range-and-match control flow, type/conversion expressions, and local-function closure as contract families; do not select one AST node or CLR type solely because its diagnostic count is high.
