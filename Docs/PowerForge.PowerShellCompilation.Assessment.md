...
| Language semantics | Bounded typed values, calls, control flow, closed collection transfers, conditional return envelopes, commands, and lifecycle through canonical IR | Dynamic promotion, enumeration inside compiled transformations, multiple success outputs, and broader pipeline/value contracts |
...
- The complete bounded compiler gate passed **918/918 tests** with zero failures or skips, then built and executed **6/6 Strict `net10.0` `win-x64` programs**.
- Focused original/generated collection workflows pass on `net10.0`/PowerShell 7.6 and `net472`/Windows PowerShell 5.1. They cover atomic maps, stable-scalar vectors, `System.Array`, `ArrayList`, `List<object>`, conditional return/fallthrough, ordinary and comma/no-enumerate output, downstream stopping, retained mutation ownership, `Split-Array` cardinality, and repeated invocation. Existing injected-enumerator tests cover partial records plus `MoveNext`, `Current`, disposal, and error-action continuation at the retained PowerShell boundary.
...
- The pinned complete PSSharedGoods census passed without input failures: **284 source files**, **286 units**, **282 functions**, **141 emitted functions**, **45 promoted regions**, **68 candidates**, and **19 analysis opportunities**. These are distinct coverage measures, not a PowerShell-language coverage percentage or a performance claim.
...
- [ ] Recover one bounded array/foreach transformation through that same owner. Keep dynamic objects, ETS, arbitrary enumerators, local-call collection closure, and compiled collection mutation fail-closed until their complete contracts exist.
