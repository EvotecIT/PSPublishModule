...
- Monotonic version state and version-bearing output names are immutable by default. Set `Installers[].Versioning.AllowOutputOverwrite: true` only for an intentional non-release rebuild: PowerForge reuses the latest completed version when the configured line and floor still match, serializes competing rebuilds, and releases the state reservation when the run finishes. A newer date floor or major/minor line still advances to a new version.
