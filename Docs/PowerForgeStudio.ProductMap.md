...
- Activity: recent saved release states appear under Releases alongside readiness signals. Selecting a journal row can open its exact checkpoint in the project's Releases tab when that working copy remains available inside the workspace. Completed rows are history, while failed or waiting rows remain attention signals. The bounded cross-project GitHub scan checks non-archived favorites first, then repositories with local readiness attention or changes, and places archived projects last. Deferred repositories remain explicitly partial; open a project's GitHub tab to inspect it directly.
