...
- Releases: prepared plan, signing, destination review, publication, verification, saved history for the selected working copy and local-save recovery. NuGet verification can use the saved package ID/version when the local archive is gone; this checks the registry endpoint, not the original archive bytes. If the archive remains, its identity must match the saved receipt. Publication receipts, saved progress and visible diagnostics omit URL credentials and query values; older saved release evidence is scrubbed on database upgrade. To verify a private project-build feed whose URL carried those values, Studio reloads the current matching project configuration for the in-memory probe; if that configuration is missing or points elsewhere, the release stays unverified. A multi-project release can appear in each participating project's history, but its shared progress is shown only in the full journal because those events cannot be assigned safely to one project.
