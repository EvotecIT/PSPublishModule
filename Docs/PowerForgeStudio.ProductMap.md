...
- Build & Run: discovered contracts, reviewed plan, running progress, cancellation, artifacts and output. [Project tasks](PowerForgeStudio.ProjectTasks.md) expose declared PowerShell or executable commands and shortcuts for conventional build scripts, each with a reviewed selection and result.
...
- Automations: a selected GitHub schedule can open its current local `.github/workflows` file in the owning project's Files view after path and link checks. When the bounded GitHub scan observed a scheduled run ID, the inspector can open that exact run on GitHub in the system browser. The action is absent for definition-only rows or runs without an ID. Windows Task Scheduler rows retain their provider-owned evidence without pretending a local source file or GitHub run exists.
